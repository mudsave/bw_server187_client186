...
protected:
	virtual void fini()
	{
		...
		{
			BaseApp::instance().intNub().deregisterFileDescriptor( iter->first );
			++iter;
		}
		...
		{
			BaseApp::instance().intNub().deregisterWriteFileDescriptor( iter->first );
			++iter;
		}
	}
};
		...
		PyObject * pFileOrSocket, PyObject * pCallback, bool isRead )
{
	...
	bool result = isRead ?
		BaseApp::instance().intNub().registerFileDescriptor( fd, pHandler ) :
		BaseApp::instance().intNub().registerWriteFileDescriptor( fd, pHandler );
...
bool deregisterFileDescriptorCommon( PyObject * pFileOrSocket, bool isRead )
{
	...
	bool result = isRead ?
		BaseApp::instance().intNub().deregisterFileDescriptor( fd ) :
		BaseApp::instance().intNub().deregisterWriteFileDescriptor( fd );
...
PyObject * address()
{
	const Mercury::Address & addr = BaseApp::instance().intNub().address();
