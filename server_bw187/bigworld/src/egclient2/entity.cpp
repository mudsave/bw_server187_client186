...
std::map<ObjectID, Entity *> Entity::entities_;

extern ObjectID g_playerID;

/**
 *	Constructor.
 */
Entity::Entity( ObjectID id, const EntityType & type, const Vector3& pos,
...
void Entity::handlePropertyChange( int messageID, BinaryIStream & data )
{
	...
	if (pOwner == NULL)
	{
		ERROR_MSG( "Entity::handlePropertyChange: "
	...
	// print it out
	DataDescription * pDD =
		type_.description().clientServerProperty( path[path.size()-1] );
	std::cout << "Setting " << pDD->name();

	for (int i = path.size()-2; i >= 0; i--)
		std::cout << " subelt " << path[i];

	PyObject * pStr = PyObject_Str( &*pNewValue );
	std::cout << " to " << PyString_AsString( pStr ) << std::endl;
	Py_DECREF( pStr );
}
