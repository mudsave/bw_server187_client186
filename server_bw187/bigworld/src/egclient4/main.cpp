...
public:
	 ...
	 */
	virtual void onEntityEnter( ObjectID entityID, SpaceID spaceID, ObjectID )
	{
		...
		BinaryIStream & data, bool oldVersion )
	{
		if (oldVersion)
		{
			WARNING_MSG( "MyServerMessageHandler::onEntityCreate: "
				"versioning not supported in egclient." );
		}
		...
		else
		{
			...
			{
				ERROR_MSG( "MyServerMessageHandler::onEntityCreate: "
					"object type %d doesn't exist for egclient\n", (int)type );
				if (!g_spaceID)
				{
					// HACK: force a quit
	 ...
	 */
	virtual void onEntityMove( ObjectID entityID, SpaceID spaceID,
			ObjectID vehicleID,
	...
	virtual void onRestoreClient( ObjectID id,
		...
		BinaryIStream & data, bool oldVersion )
	{
		INFO_MSG( "MyServerMessageHandler::onRestoreClient: id=%d (%d)\n",
		...
		IF_NOT_MF_ASSERT_DEV( id == g_playerID )
		{
			ERROR_MSG( "MyServerMessageHandler::onRestoreClient: " 
				"received ID (%d) does not match player ID (%d)!",
				id, g_playerID );
			return;
		}
		...
		entityDesc.readStream( data, EntityDescription::CLIENT_DATA,
			pDict.getObject() );
	}
...
// in cstdmf/dprintf.cpp
// hack to get printing to work even though this is a "client"
extern bool g_shouldWriteToConsole;

int main( int argc, char * argv[] )
{
	// hack: make sure that debug messages are shown
