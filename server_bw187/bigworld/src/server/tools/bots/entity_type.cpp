 ...
 *	Constructor (private)
 */
EntityType::EntityType( uint32 idx,
...
bool EntityType::init( const std::string &clientPath, MD5::Digest& digest )
{
	...
	for (int i = 0; i < numEntityTypes; i++)
	{
		...
		MF_ASSERT( types_.size() == currDesc.clientIndex() || !succeeded );
		types_.push_back(
				new EntityType( i, currDesc, (PyTypeObject *)pClass ) );
	}
 ...
 *	this entity type. It streams the properties from the input stream.
 */
Entity * EntityType::newEntity( const ClientApp & clientApp, ObjectID id, const Vector3 & pos,
