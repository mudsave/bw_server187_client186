 ...
 *	Constructor.
 */
ClientApp::ClientApp( Mercury::Nub & mainNub,
	const char * tag, PyTypeObject * pType ) :
	PyObjectPlus( pType ),
	serverConnection_(),
	spaceID_( 0 ),
	playerID_( 0 ),
	...
	pDest_( NULL )
{
	...
	{
		char randName[10];
		sprintf(randName, "_%08x", rand());
		username.append(randName);
	}
 ...
 */
void ClientApp::onBasePlayerCreate( ObjectID id, EntityTypeID type,
 ...
 */
void ClientApp::onCellPlayerCreate( ObjectID id,
 ...
 */
void ClientApp::onEntityEnter( ObjectID id, SpaceID spaceID, ObjectID )
{
	...
	if (hasScripts_)
	{
		if (id != playerID_)
		{
			EntityMap::iterator iter = this->cachedEntities_.find( id );
			if (iter != this->cachedEntities_.end())
			{
 ...
 */
void ClientApp::onEntityLeave( ObjectID id, const CacheStamps & stamps )
{
 ...
 *	requestEntityUpdate.
 */
void ClientApp::onEntityCreate( ObjectID id, EntityTypeID type,
	SpaceID spaceID, ObjectID vehicleID, const Position3D & pos,
	float yaw, float pitch,	float roll,
	BinaryIStream & data, bool oldVersion )
{
	...
	if (hasScripts_)
	{
		if (oldVersion)
		{
			WARNING_MSG( "ClientApp::onEntityCreate(%d): "
						"versioning not supported in bot.\n", id );
		}

		// Make sure it doesn't already exist.
		EntityMap::iterator cachedIter = this->cachedEntities_.find( id );
		if (cachedIter != this->cachedEntities_.end())
		{
		    TRACE_MSG( "ClientApp::onEntityCreate(%d): entity %d is in cache\n", playerID_, id );
			cachedIter->second->position( pos );
			cachedIter->second->updateProperties( data );
			this->entities_[ id ] = cachedIter->second;
			this->cachedEntities_.erase( cachedIter );
		}
		else if (this->entities_.find( id ) != this->entities_.end())
		{
		...
		else
		{
			...
			{
				ERROR_MSG( "ClientApp::onEntityCreate(%d): "
						"entity type %d doesn't exist for egclient\n", playerID_, (int)type );
				if (!spaceID_)
				{
					// HACK: force a quit
// 					HACK_MSG( "ClientApp::onEntityCreate(%d): faking "
// 							  "ctrl-c to force a quit\n", playerID_ );
					//g_ctrlC = true;
				}
				return;
		    }
 ...
 */
void ClientApp::onEntityProperties( ObjectID id, BinaryIStream & data,
		bool oldVersion )
{
	...
	if (hasScripts_)
	{
		...
		{
			ERROR_MSG( "ClientApp::onEntityProperties(%d): entity(id = %d) not found\n",
						playerID_, id );
		}
	}
	else
 ...
 */
void ClientApp::onEntityProperty( ObjectID entityID, int propertyID,
	BinaryIStream & data, bool oldVersion )
{
 ...
 */
void ClientApp::onEntityMethod( ObjectID entityID, int methodID,
	BinaryIStream & data, bool oldVersion )
{
 ...
 */
void ClientApp::onEntityMove( ObjectID entityID, SpaceID spaceID, ObjectID vehicleID,
...
void ClientApp::onEntitiesReset( bool keepPlayerOnBase )
{
	...
	iter = this->cachedEntities_.begin();
	while (iter != this->cachedEntities_.begin())
	{
		Entity * pCurr = iter->second;
		++iter;
		pCurr->destroy();
	}
	this->cachedEntities_.clear();
...
bool ClientApp::tick( float dTime )
{
	...
	// if there's no login in progress then assume we are online,
	// and so process any pending network input, then send a move
	if (pLoginInProgress_ == NULL)
	{
	...
	return (pLoginInProgress_ != NULL) || serverConnection_.online();
}
...
void ClientApp::destroy()
{
	if (!isDestroyed_)
	{
		...
		if (pModule && (playerID_ != 0))
		{
			...
			// flush out the call before we die
			serverConnection_.send();
		}
		...
		for (EntityMap::iterator iter = cachedEntities_.begin();
				iter != cachedEntities_.end(); iter++)
		{
			iter->second->destroy();
		}
