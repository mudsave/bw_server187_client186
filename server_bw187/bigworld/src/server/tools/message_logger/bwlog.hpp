...
protected:
	...
	public:
		...
		bool getEntry( const EntryAddress &addr, Entry &result,
			Segment **ppSegment = NULL, bool warn = true );
...
public:
	...
	public:
		Result( const Entry &entry, BWLog &log, const UserLog &userLog,
			const Component &component, const char *message );
	private:
		~Result() {}
	public:
		double time_;
	...
	public:
		...
		PyObject* pyGetAttribute( const char *attr );
		PyObject *next();
		const char *interpolate( LoggingStringHandler &handler,
			RangePtr pRange );
