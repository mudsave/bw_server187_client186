...
/* huangshanquan  2009-05-06 change begin :write mysql log*/
{
	appPath_ = "";
	messagemysql_ = new MessageMysql();
}
/* huangshanquan 2009-05-06 change end*/

BWLog::~BWLog()
{
	delete messagemysql_;
	messagemysql_ = NULL;
...
bool BWLog::init( const char *root, const char *mode, const char *config )
{
	...
	while ((dirent = readdir( rootdir )) != NULL)
	{
		...
		sprintf( fname, "%s/%s/uid", root, dirent->d_name );
	...
	/* huangshanquan  2009-05-05 add begin:write mysql log*/
	if (mode_ == "a+")
	{
		if(!messagemysql_->init(appPath_))
			return false;
	}
	/* huangshanquan 2009-05-05 add end*/
...
bool BWLog::readConfig( const char *config )
{
	...
	else if (stat( "/etc/bigworld.conf", &statinfo ) == 0)
	{
		...
		else
		{
			...
			/* huangshanquan  2009-05-25 add begin*/
			appPath_ = bwconf.toolsDir_ + "/message_logger";
			/* huangshanquan 2009-05-25 add end*/
		}
	}
	...
	const Mercury::Address &addr, MemoryIStream &is )
{
	...
	if (is.error())
	{
		ERROR_MSG( "Log message from %s was too short (%d bytes)\n",
			(char*)addr, len );
		return false;
	}
	...
	if (pUserLog == NULL)
	{
		...
		{
			ERROR_MSG( "Couldn't resolve uid %d (%s).  UserLog not started.\n",
	...
	/* huangshanquan  2009-05-05 add begin:write mysql log*/
	if (messagemysql_ != NULL && \
		messagemysql_->isconnect() && \
		MESSAGE_PRIORITY_SCRIPT == entry.messagePriority_)
	{
		std::string dbmesg;
		char daybuf[256];
		time_t  timep;
		...
		sprintf(daybuf, "%d-%d-%d %d:%d:%d", 
				(1900+tp->tm_year), (1+tp->tm_mon), tp->tm_mday,
				tp->tm_hour, tp->tm_min, tp->tm_sec);

		MemoryIStream args( is.data(), is.remainingLength() );

		pHandler->streamToString( args, dbmesg );

		messagemysql_->putMessage(entry.messagePriority_ ,
									dbmesg,
									pUserLog->log_.componentNames_.resolve( component.typeid_ ),
									daybuf,
									pUserLog->log_.hostnames_.resolve( component.addr_.ip ));
	}
	/* huangshanquan 2009-05-05 add end*/
...
Mercury::Reason BWLog::resolveUid( uint16 uid, uint32 addr, std::string &result )
{
		...
		Mercury::REASON_SUCCESS)
	{
		...
		ERROR_MSG( "UserMessage query to %s failed: %s\n",
...
{
	char buf[ 256 ];
	sprintf( buf, "%s%02d (id:%d) %s", msg_.componentName_.c_str(), appid_,
		id_, (char*)addr_ );
	...
	pText_( NULL )
{
	...
	sprintf( buf, "%s/entries.%s", userLog_.path_.c_str(), suffix_.c_str() );
	...
	sprintf( buf, "%s/args.%s", userLog_.path_.c_str(), suffix_.c_str() );
	...
	{
		sprintf( buf, "%s/text.%s", userLog_.path_.c_str(), suffix_.c_str() );
...
bool BWLog::Range::iterator::operator<( const iterator &other ) const
{
	...
	if (pRange_->direction_ == FORWARDS)
	{
		// TODO: rewrite this to include metaOffset_
		return segmentNum_ < other.segmentNum_ ||
			segmentNum_ == other.segmentNum_ && entryNum_ < other.entryNum_;
	}
	else
	{
		return segmentNum_ > other.segmentNum_ ||
...
void BWLog::Range::iterator::step( int direction )
{
	...
	if (entryNum_ < 0 || entryNum_ >= this->segment().nEntries_)
	{
			...
			segmentNum_ >= (int)pRange_->userLog_.segments_.size())
		{
			...
			{
				ERROR_MSG( "BWLog::Range::iterator::step: "
					"Tried to seek backwards past the end of the log\n" );
				return;
			}
		}
...
int BWLog::Range::iterator::operator-( const iterator &other ) const
{
	...
	if (segmentNum_ == other.segmentNum_)
	{
		return d * (entryNum_ - other.entryNum_) +
			   d * (metaOffset_ - other.metaOffset_);
	}
	else
	{
		// Count first and last segments first
		int count = d == FORWARDS ?
...
{
	char buf[ 32 ];
	sprintf( buf, "%d:%d:%d", segmentNum_, entryNum_, metaOffset_ );
	return std::string( buf );
}
	...
	MemoryIStream &is, bool useOldFormat ) const
{
	...
	Result *pResult = new Result(
		entry, log_, *this, component, msg.c_str() );
...
PyObject* BWLog::UserLog::py_getEntry( PyObject *args )
{
	...
	// have been rolled.
	if (!this->getEntry( addr, entry, &pSegment, false ))
	{
		Py_RETURN_NONE;
	}
	...
	return new Result( entry, log_, *this, *pComponent, msg.c_str() );
}
...
{
	sprintf( s_pathBuf_, "%s/%s", dir, filename );
	return s_pathBuf_;
}
...
const char * BWLog::Hostnames::resolve( uint32 addr )
{
	...
	if (it == this->end())
	{
		...
		char line[ 2048 ];
		sprintf( line, "%s %s", ipstr, hostname );
...
bool BWLog::ComponentNames::handleLine( const char *line )
{
	...
	if (this->size() >= this->MAX_COMPONENTS)
	{
		CRITICAL_MSG( "BWLog::ComponentNames::handleLine:"
			"Dropping component '%s'; max number of components reached",
			this->MAX_COMPONENTS );
	}
...
const char * BWLog::ComponentNames::resolve( int ttypeid ) const
{
	...
	else
	{
		ERROR_MSG( "BWLog::ComponentNames::resolve:"
			"Cannot resolve unknown typeid (%d) from %d known records\n",
			ttypeid, this->size() );
		return NULL;
	}
}
	...
	if (adit != addrMap_.end())
	{
		...
		else
		{
			ERROR_MSG( "BWLog::Components::erase: "
				...
				pComponent->str().c_str(), (char*)addr );
		}
...
BWLog::QueryParams::QueryParams( PyObject *args, PyObject *kwargs, BWLog &log ) :
	...
	good_( false )
{
	static char *kwlist[] = { "uid", "start", "end", "startaddr", "endaddr",
							  "period", "host", "pid", "appid",
							  "procs", "severities",
							  "message", "interpolate", "casesens", "direction",
							  NULL };

	double start = LOG_BEGIN, end = LOG_END;
	const char *host = "", *message = "", *cperiod = "";
	PyObject *startAddr = NULL, *endAddr = NULL;

	if (!PyArg_ParseTupleAndKeywords( args, kwargs, "H|ddO!O!ssHHiisibi",
			kwlist,
			&uid_, &start, &end,
			&PyTuple_Type, &startAddr, &PyTuple_Type, &endAddr,
			&cperiod, &host, &pid_, &appid_, &procs_, &severities_,
			&message, &interpolate_, &casesens_, &direction_ ))
	{
		return;
	}
	...
	{
		pRegex_ = new regex_t();
		int reFlags = REG_EXTENDED | REG_NOSUB | (casesens_ ? 0 : REG_ICASE);
		int reError;
		char reErrorBuf[ 256 ];

		if ((reError = regcomp( pRegex_, message, reFlags )) != 0)
		{
			regerror( reError, pRegex_, reErrorBuf, sizeof( reErrorBuf ) );
...
PyObject* BWLog::Query::next()
{
	...
	for (int i=0; pRange_->getNextEntry( entry ); i++)
	{
		...
		const char *matchText = pHandler->fmt().c_str();

		if (pParams_->interpolate_ == PRE_INTERPOLATE)
			matchText = this->interpolate( *pHandler, pRange_ );

		// Filter
		if ((pParams_->addr_ && (pComponent->addr_.ip != pParams_->addr_)) ||
			(pParams_->pid_ && pComponent->msg_.pid_ != pParams_->pid_) ||
			(pParams_->appid_ && pComponent->appid_ != pParams_->appid_) ||
			(pParams_->procs_ != -1 &&
				!(pParams_->procs_ & (1 << pComponent->typeid_))) ||
			(pParams_->severities_ != -1 &&
				!(pParams_->severities_ & (1 << entry.messagePriority_))) ||
			(pParams_->pRegex_ &&
				regexec( pParams_->pRegex_, matchText, 0, NULL, 0 ) != 0))
		{
			continue;
		}
...
const char * BWLog::Query::interpolate( LoggingStringHandler &handler,
	RangePtr pRange )
{
	...
	msg.clear();
	handler.streamToString( argsStream, msg );
	return msg.c_str();
}
...
BWLog::Result::Result( const Entry &entry, BWLog &log,
	const UserLog &userLog, const Component &component, const char *message ) :
	PyObjectPlus( &s_type_ ),
	time_( entry.time_ ),
...
const char *BWLog::Result::formatOld()
{
	...
	sprintf( timestr+n, ".%03ld", now.tv_usec/1000 );

	snprintf( s_linebuf_, sizeof( s_linebuf_ ),
