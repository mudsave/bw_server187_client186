...
bool Logger::init( int argc, char * argv[] )
{
	...
	if (interfaceName_ == Mercury::Nub::USE_BWMACHINED)
	{
		...
		INFO_MSG( "Discovered network interface %s\n", interfaceName_.c_str() );
	}

	if (interfaceName_.empty())
	{
		WARNING_MSG( "Logger::init: Assuming monitoringInterface of eth0\n" );
		interfaceName_ = "eth0";
	}

	INFO_MSG( "Using network interface '%s'\n", interfaceName_.c_str() );
	...
	for (int i=1; i<argc; ++i)
	{
		...
		if (argv[i][0] == '-')
		{
			...
			else if (arg == "-l")
			{
				...
				{
					unsigned long loggerID = strtoul( argv[i], NULL, 0 );
					
					static const LoggerID MAX_LOGGER_ID = 
						std::numeric_limits< LoggerID >::max();

					if (loggerID > (unsigned long) MAX_LOGGER_ID)
					{
						ERROR_MSG( "LoggerID must be in range 0-%u\n", 
							MAX_LOGGER_ID );
						return false;
					}
					loggerID_ = LoggerID( loggerID );

					INFO_MSG( "Logging messages destined only for LoggerID: "
							"%u\n",
						loggerID_ );
				}
			}
	...
	// Register birth and death listeners.
	{
		...
		if (lm.sendAndRecv( this->socket(), LOCALHOST ) !=
			...
			char * data, int dataLen, const Mercury::Address & addr )
{
	...
	{
		INFO_MSG( "Registering %s at %s (uid:%d, loggerID:%u)\n",
