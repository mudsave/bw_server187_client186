...
bool EntityLoader::startup()
{
	...
	{
		ERROR_MSG( "EntityLoader: Failed to parse entities.xml\n" );
		return false;
	}
		...
		DataSectionPtr pProperties )
{
	...
	else
	{
		...
		bundle << ObjectID( 0 ) << entityTypeID << DatabaseID( 0 );
		...
		Mercury::UnpackedMessageHeader&, BinaryIStream& data, void*)
{
	if (component_ == ON_CELL)
	{
		ObjectID objectID;
		data >> objectID;
		INFO_MSG( "Created entity %d on cell\n", objectID );
	}
	else
	{
		EntityMailBoxRef ref;
		data >> ref;
		INFO_MSG( "Created entity %d on base at %s\n",
