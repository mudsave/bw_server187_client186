...
// Version 36:setgid() before fork() so users have permissions on their procs
// Version 37:Take caching into account in system memory calculations
// Version 38:Handles changes to system time (fix in cstdmf/time_queue.*)
...
{
	int nCpus, cpuSpeed;
	std::vector< MaxStat > cpu;
	MaxStat mem;

	Stat< uint64 > packTotIn, packDropIn, packTotOut, packDropOut;
...
bool updateSystemInfoP( SystemInfo &si );
