...
{
	id_ = cluster_.machined_.callbacks().add(
...
void Cluster::ClusterTimeoutHandler::cancel()
{
	...
	cluster_.machined_.callbacks().cancel( id_ );
	id_ = 0;
}
...
void Cluster::FloodTriggerHandler::handleTimeout( TimeQueueId id, void *pUser )
{
...
TimeQueue::TimeStamp Cluster::FloodTriggerHandler::delay() const
{
	...
	TimeQueue::TimeStamp max =
		std::max( min+1, AVERAGE_INTERVAL * cluster_.machines_.size() * 2 );
...
void Cluster::FloodReplyHandler::handleTimeout( TimeQueueId id, void *pUser )
{
...
void Cluster::BirthReplyHandler::handleTimeout( TimeQueueId id, void *pUser )
{
