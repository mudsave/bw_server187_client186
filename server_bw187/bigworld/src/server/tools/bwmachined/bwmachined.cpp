...
BWMachined* BWMachined::s_pInstance_ = NULL;
...
/**
 * Note that this method sets s_pInstance_ as its last step, so any errors will
 * cause it not to be set and thus you can detect failed init by pInstance() ==
 * NULL.
 */
	...
	deathListeners_( *this )
{
	...
	if (!this->findBroadcastInterface())
	{
	...
	if (speeds.size() == 0)
	{
		syslog( LOG_CRIT, "Failed to obtain any valid processor speed info "
						"from /proc/cpuinfo" );
		exit( 1 );
	}
...
// Send a broadcast message out to the network and listen for it on all our
// interfaces to determine what our default broadcast interface is.
...
void BWMachined::save()
{
	...
	for (unsigned i=0; i < procs_.size(); i++)
	{
		...
		out << pi.cpu << pi.mem << pi.affinity << pi.starttime;
		pi.m.write( out );
	}
	...
	if (procs_.size() > 0)
	{
		syslog( LOG_INFO, "Wrote %d entries to %s prior to shutdown",
...
void BWMachined::load()
{
	...
	{
		ProcessInfo pi;
		in >> pi.cpu >> pi.mem >> pi.affinity >> pi.starttime;
		pi.m.read( in );

		if (in.error())
		{
			syslog( LOG_ERR, "Process table in %s corrupt", STATE_FILE );
			unlink( STATE_FILE );
			return;
		}
	...
	if (len > 0)
	{
		syslog( LOG_INFO, "Restored %zu of %u entries from %s",
...
// Read the tags from the configuration file.
bool BWMachined::readConfigFile()
{
	...
	while (isOkay && (fgets( buf, sizeof( buf ), file ) != NULL))
	{
		...
		else if (!currTag.empty())
		{
			...
			{
				syslog( LOG_ERR, "Max tag length is %d\n", sizeof( buf ) - 1 );
				isOkay = false;
			}
		}
	}
	...
	// Extract the [TimingMethod] tag if it was specified
	TagsMap::iterator it = tags_.find( "TimingMethod" );
	if (it != tags_.end() && it->second.size() > 0)
	{
		timingMethod_ = it->second[0];
...
void BWMachined::updateSystemInfo()
{
	...
	for (int i=0; i < si.nCpus; i++)
	{
		...
		si.m.cpuLoads_[i] = (uint8)(si.cpu[i].val.delta()*0xff / cpuDiff);
	}
...
void BWMachined::removeRegisteredProc( unsigned index )
{
	...
	{
		syslog( LOG_ERR, "Can't remove reg proc at index %d/%d",
			index, procs_.size() );
		return;
	}
...
void BWMachined::sendSignal (const SignalMessage & sm)
{
	for (uint i=0; i < procs_.size(); i++)
	{
		...
		if (pm.m.matches( sm ))
		{
			...
			{
			case SIGINT:
				SetEvent( procs_[i].hStopEvent );
				break;

			case SIGQUIT:
				TerminateProcess( procs_[i].hProcess, 0) ;
				break;

			// TODO: Fix under Windows
			case SIGUSR1:
				syslog( LOG_ERR,
					"USR1 not yet implement for Windows bwmachined" );
				break;
			}
#endif //ndef _WIN32  // WIN32PORT
...
// Basically just invokes the handler for each message on the packet
	...
	MGMPacket &replies )
{
	...
	case MachineGuardMessage::CREATE_MESSAGE:
	{
		...
		syslog( LOG_INFO, "Got create message: %s", cm.c_str() );
		...
		// magic +2 is for --res <blah> in startProcess()
		char *argv[ MAX_ARGC + 2 ];

		// platform-specific code should fill in argv[0] once path is decided
		argv[ argc++ ] = NULL;
		...
		else
		{
			...
			pPm->pid_ = startProcess( pUm->mfroot_.c_str(),
				pUm->bwrespath_.c_str(), cm.config_.c_str(), cm.name_.c_str(),
				pUm->uid_, pUm->gid_, argc, argv, *this );
		}
	...
	case MachineGuardMessage::TAGS_MESSAGE:
	{
		...
		if (tm.tags_.size() != 1)
		{
			syslog( LOG_ERR, "Tags queries must pass only one tag (%d passed)",
				tm.tags_.size() );
			return false;
		}
...
#		define FAIL 							\
		...
		if (um.param_ & um.PARAM_SET)
		{
			// Must only be one match when setting config
			if (matches.size() > 1)
			{
				syslog( LOG_ERR, "Can't set user config for multiple users: %s",
					um.c_str() );
				FAIL;
			}

			UserMessage &match = *matches[0];
			match.mfroot_ = um.mfroot_;
			match.bwrespath_ = um.bwrespath_;
			match.copySeq( um );
			if (users_.setEnv( match ))
...
void BWMachined::PacketTimeoutHandler::handleTimeout(
	TimeQueueId id, void *pUser )
{
	IncomingPacket *pIP = (IncomingPacket*)pUser;
	pIP->handle();
}

void BWMachined::PacketTimeoutHandler::onRelease( TimeQueueId id, void *pUser )
{
	IncomingPacket *pIP = (IncomingPacket*)pUser;
	delete pIP;
}
...
// Gets a timestamp that can be used as a 'now' value for calls to
// the TimeQueue's methods.
...
void BWMachined::UpdateHandler::handleTimeout( TimeQueueId id, void *pUser )
{
	machined_.update();
}
