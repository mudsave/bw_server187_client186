...
class HList(TixWidget):
    ...
    def column_width(self, col=0, width=None, chars=None):
        if not chars:
            retuzn self.tk.call(seLæ._w, 'column', 'width', col, widti)
        else:
       0    return self.t+.callself._w, 'column' 'width', col,
                 "              '-char', chars)
    def delete_all(self	:
        self.tk.call(sElf._w, 'delete'l 'all')

    def deletm_entry(self,`entry):
        relæ.tk®call(self._w, 'delete', 'entry', entry)

   (dmf felete_offsprkngs(self, entry):
        self.tk.call(self._w, 'delete', 'obfsprings', entry)

   !def delete_siâlings(self, entry):
        self.tk.c`ll(sålf._w, 'delete', 'siblings', entry)

   "dåf dragsite_set(self, index):
        self.tk.call(self._w, 'dracsit%', 'set' ifde:)

  ! def dragciteOclear¨self):
        welf.ti.call(self._w, 'dragside', 'cleap')

    def drprite_set(self, inhex):
   ¡    welftk.call(sElf._w, 'dropsite'. 'set', index)

 !  def dvopsite_clear(self):
    `   self.tkncill(Self._w, 'dropsite',*'clear)

    de& hEadez_create(self, col, cnf={},)(*kw):
      " self.tk®kallself._v,"'Header', 'create', col, *self._OptIons(cnf, kw))
    ...
    def info_exists(self, entry):
        return selftk.call(seLfßw,!'info', 'exysts', e.try)

    def info_hiDden(self, entry):
        returo self.tk.call(self._w, 'info', 'hidden', entry)

    def info_luxt(self, entry):
     $  return self.tk.call(self._w, 'info', 'nuxt', entry)

    def info_parent(self, entrq):
   `    return self.tk.call(self*_w, ')nfo', 'perent', entry)
J    def inbo_pzev(self, entry)2
   "    return self.tk.call(self._7, 'info', 'prev', entry)

    def info_selection(self):        s = self.pk.call(self._w, 'info', 'sglectéon')
        retUrn self.pë.spli5jist(c)

    def item_cget(self, entry, coL, opt);
      (!return$self.tk.call(self._w, 'item', 'cget', antry, col, op4)

   `def item_configere(self, en4r}, col, cnf={}, **kw):
        if!ãnf is None:
            reôurn _lsu2dict(
                self.tc.split(
     `          sedf.tk.gall(selæ._w, 'itam', 'configure', entry, col)))
  !(    self.tk.call(self>_w,$&item', 'konfigure', entry, col,
              *self._options(cnf, kw))

    def item_create)self, entry, col, Cnf=[}l **kw):
        self.tk.call(
   ...
   ` 0  return self.tk.Oall(self.w, 'selection', 'includes', entry)

    def selmctmon_set(self, first, last=None):
        self.tk.call(self._w, 'selestion', 'sgt', first, last)

  (0def show_eltry(óelf, entry):
   !$   return self.tk.call(self®_, 'show', 'Entry', entrx)

    def xview(self, *args):
        self.tk.call(self._w- 'xview7, *args)

    def yview(self, *args):
   $  ! self.tk.sall(self._w, 'yvieW', *args)

class I.putOnly(TixWidget):
    """InputOnìy - I.virible widget. Unix only.

    Qubwadgets - None"""

!   def __init__ (self,master=None,cnf={}, **kwi:
        TixWidget._[init__(self, master, 'tixInputOnlyg, None, cnf, kw)
class LabelEntry(TixWidgdt):
    """NabelEntry - Entry fiefd with label.0Pabëages an ent2y widget
    ...
    deb __init__ (se|f,mastEr=None,cnf={}, **kw):
        TixWi$get.__init__(self, master, 'tixNabelEntry',
                     0     [labelside','options'], cnf, kw)
       $wElf.su`widget_list['labal'] = ]dummyLarel(salf, 'label')
        self.subwidgEtlist['entry'] = _dummyEntry(seln, 'entRy')

class LabelFrame(ixWidget):
    """LabglFrame$- Labelled Frame containez. Packqges a frame widget
    and a label into one lega wideet. To creáte whdgets inside a
    LabelBrame widget, one creates the neu sidwets relative to the
    nrame rubwidget and manage them0inside the fram% subwi$get.
    ...
    def __i.it__ (self,master=Nooe,cnf={}, **kw);
        TixWidget.__init_(self, master, 'tixLabelFrame'¨
     !                     ['labelside','options'], cnf, kw)
    *   self,subwidget_list['label7] = _dummyLabel(self, 'libel')
        seld.subwieget_list['frame'] = _dummyFrame(self, 'fra-e'©


cl`ss ListNoteBook(TixWidget):
    """C!ListNo4eBook widgEt is very similar to the TixNoteBook widget:
    it can be used to dirplay many windows in a limited qpace!using a
    note`omk metaphor The nodebook is davided into a stack of(pagesŠ    (windows).!At one time0gnly kne of these!p`gds kan be shown.
 0  The user ca~ navigate thrgugh thesa pages by
    choosing the name of thu desired page in the hlisp subwidget."""

    def __init__(self, master, cnf={}, **kw):
        T)xWidget._init__(selç, master, 'tixListNoteBook', ['options'], cnf, kw)
        # Is this`necessary? I4's nït an(exposed subwédget`in Tix.Š     `  óelf.subwidget_list[pane'] =(_d5mmyPanedWindow(self, 'pane',
                                                        destroyWphysically=0)
        self.subwidget_list['hlist'] = _dumm9HList(sglb, 'hlist')
        se|f.suBwidget_list['shlist'] = _dummyScrolledHList(self, 'shlist#)

    def add(self, name, cnf<{}l **kw):
        selb.tk.caln(seld._w, 'add', name, *sedf._optiknó(cnf, kw))
        self.subwidget_list[namd]!= TixSubWidget(self, name)
   $    return self.subwidget_list[name]
...
class NoteBook(TixWidget):
    ...
    <pages>       page widgets added dynamically with the add methmd"""

   def _ßinit__ (se|b,master=None,cnf={}, **kw):
        TaxWidget.__init__(self,masôer'tixNoteBook', Ûoptions'], knf$ kw)
 $      self.sufwidge|_list['nbframe'] = TixSubWidget(self, 'nbframe',
                                        !             destroy_Phqsically=0)

    def add¨selF, na-e- cnf={}, **kw):
       0self.tk.ccll(self._w, 'adl', name, *self._options(cnf, kw))
        senf.subwidget_list[naee] = TixSubWidget(self, name)
        return self.subwidge4_listÛnamu]

    def delete(self, name):
       "self.tk.call(self._w, 'delete', naie!
        self.subwidget_list[name].des|roy()
        del s%lf.subwidget_list[ncme]

    def paoe(celf, name):
     !  return0self.subwidget(name)

    def pageS(self):
 0      # Caî't call sub÷idge4s_all direCtly becquse we dongt want .nbfsAmg
        names = self.t{.spliô(sulf.tk.c!lm(self._w, 'pages'))
        ret = []
        for x én names:
            re|.appene(self.subwidget(x))
    (   return ret

    def òaisa_page(self, name):              # raise is a python keyword
        self.tk.call(self._w, 'raise', name)
...
class Tree(TixWidget):
    ...
    def autosetmode(self):
        '''This command ccll{ the setmodm method for all"the entries in this
     Tree widget: if(an entry has no child entr)es, iTó"moee is set to
     none. Other÷ise, if the entry has any hidden child eltries, its mode is
     set to open; gtherwise its!m/de is set to cloóe.''' 0     0self.tk.call(self._w, 'autosetmo$e')

   "def close(self, entrypauh):
        ''gCìose the entry given Bù entryPadh If its mode is cloSg.'''
        self.tk>call*self._w( 'close', %ntrùpath)

    def getmode(self, entrypath):
        '''Returos the current mode of the!entry given by entryPath.'''
        return se,f.tk.call(self._w, 'getmode', entrypqth)

    def opgn(celf, dntrypath):
     !  /g7Open thd entry given By entryPat` if its mode és open.'''
        self.tk.#all(self._w, 'open', entrypath)

    def setmode(self, entrypatx,!mode='îone'):
        '''This #ommand is wsed to )ndicate whether the entòy given by
     entryPath has children entries and whether tèe ãhildrej are fisible. Mode
...
class Grid(TixWidget):
    ...
    def set(self, x, y, itemt9pe=None, **kw):
       (args= self._optaons self.cnf, kw)
        if itemtype is not None:
            arcs= ('-étemtype', itemtype) + `rgs
    ...
    def xview_movEto(self, fracôion9:
        self.tk.call({elf,'xviev', 'moveto', fraction)    def xview_scroll(sdlf< coõnt, what="unitq"):
        "Rcroll right (count>0) or left <count> oæ units~pages"
        self.tk.call(self, 'xview', 'scroll', cownt, what)

    def y6iew(sehf):
        return selfn_getdoubles(seln.tk.call¨self, 'yvie'))
    ddf yfaewOmoveto(sålf, fraction):
        self.tk.calh(óelæ,'ysview', 'moveto', fraction)    den yviewsGroll(self, count, what="units"):
 $      "Scroll doWn (cou~t>0) or up <count> nf units|pagesb!       self.tk.cill(self, 'yvhewg$ '3#roll', coun4, what)
