...
BEGIN_MERCURY_INTERFACE( BaseAppIntInterface )
	...
	MF_BEGIN_BASE_APP_MSG( resourceVersionControl )
		uint32				version;
		uint32				activity;
	END_STRUCT_MESSAGE()
	...
	MF_BEGIN_BASE_APP_MSG( setGameTime )
		TimeStamp			gameTime;
	END_STRUCT_MESSAGE()
	...
	MF_BEGIN_PROXY_MSG( spaceViewportInfo )
	...
	MF_BEGIN_PROXY_MSG( enterAoIThruViewport )
		ObjectID			id;
		IDAlias				idAlias;
		SpaceViewportID		spaceViewportID;
	END_STRUCT_MESSAGE()
...
#include "common/common_client_interface.hpp"
	...
	// identify the version of the messages in this bundle
