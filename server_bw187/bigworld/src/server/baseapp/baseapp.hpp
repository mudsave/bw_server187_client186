...
public:
	...
	// message concerning versions of resources
	void resourceVersionControl(
		const BaseAppIntInterface::resourceVersionControlArgs & args );
	void sendAckForStep( int step );	// helper to send the reply
	void resourceVersionTag(
		const BaseAppIntInterface::resourceVersionTagArgs & args );
	void resourceVersionTag(
		const BaseAppExtInterface::resourceVersionTagArgs & args );
	...
	void setGameTime( const BaseAppIntInterface::setGameTimeArgs & args );
	...
	Mercury::Address backupAddrFor( ObjectID entityID ) const
	{
	...
	// This enum is shared by BaseApp and Proxy.
	enum InsProfileSelector
	{
		insRunning,
		insIn,
		insOut
	...
	Base * findBase( ObjectID id ) const;

	typedef std::map<ObjectID, Base *> Bases;
	...
	ProfileGroup &	pro()			{ return pro_; }

	uint32 latestVersion() const		{ return latestVersion_; }
	uint32 impendingVersion() const		{ return impendingVersion_; }
	uint32 imminentVersion() const		{ return imminentVersion_; }
	uint32 previousVersion() const		{ return previousVersion_; }
	bool sendResourceVersionTag() const	{ return sendResourceVersionTag_; }
	bool enableEntitiesHold() const		{ return enableEntitiesHold_; }
	...
	int dbRequestTimeout() const	{ return dbRequestTimeout_; }
...
private:
	...
	public:
		...
		std::string & getDataFor( ObjectID entityID )
		{
			if (usingNew_)
		...
		/**
		 *	Whether we can switch to the new backup upon receiving it.
		 */
		...
		/**
		 *	Setter to indicate that we can switch to the new backup upon
		 *	receiving it.
		 */
	...
	private:
		...
		private:
			...
			std::map< ObjectID, std::string > data_;
		};
	...
	private:
		...
		public:
			QueueElement( TimeStamp expiryTime,
					ObjectID proxyID, SessionKey loginKey ) :
			...
			ObjectID proxyID() const	{ return proxyID_; }
