...
void intSignalHandler( int /*sigNum*/ )
{
	...
	if (pMgr != NULL)
	{
		BaseAppMgrInterface::shutDownArgs shutDownArgs;
		pMgr->shutDown( shutDownArgs );
	}
}
...
// Static data
BaseAppMgr * BaseAppMgr::pInstance_ = NULL;
...
BaseAppMgr::BaseAppMgr( Mercury::Nub & nub ) :
	...
	shutDownServerOnBadState_(
		BWConfig::get( "shutDownServerOnBadState", false ) ),
	...
	hasMultipleBaseAppMachines_( false )
{
	...
	if (!BWConfig::get( "updater/enabled", DEFAULT_UPDATER_ENABLED ))
	{
		latestVersion_ = 0;
	}

	BWConfig::update( "baseAppMgr/shouldShutDownOthers",shouldShutDownOthers_ );
	...
	DataSectionPtr pBaseAppMgrConfig = BWConfig::getSection( "baseAppMgr" );

	if (pBaseAppMgrConfig)
	{
		std::vector< int > appID;
		pBaseAppMgrConfig->readInts( "reservedBaseApp", appID );

		std::vector< int >::iterator iter;
		for (iter = appID.begin(); iter != appID.end(); iter++)
		{
			reservedBaseApps_.insert( *iter );
		}
	}
...
BaseApp * BaseAppMgr::findBestBaseApp() const
{
	...
	while (iter != baseApps_.end())
	{
		...
		if (currLoad < lowestLoad && reservedBaseApps_.count( iter->second->id() ) == 0)
		{
			lowestLoad = currLoad;
...
void BaseAppMgr::addWatchers()
{
	...
	// the usual profiles
	static const char	* pgLabels[] = { "running", NULL };
// 	pRoot->addChild( "profiles",  new SequenceWatcher<ProfileGroup>(
// 		pro_, pgLabels ) );
// 	pRoot->addChild( "profiles/*", &ProfileVal::watcher() );
	ProfileWatcher *pw = new ProfileWatcher( pro_, pgLabels );
	pRoot->addChild( "profiles", pw );
 ...
 *	timer events.
 */
int BaseAppMgr::handleTimeout( int /*id*/, void * arg )
{
	// Are we paused for shutdown?
	if ((shutDownTime_ != 0) && (shutDownTime_ == time_))
		return 0;

	switch (reinterpret_cast<int>( arg ))
	{
...
private:
		...
		BinaryIStream& data, void * /*arg*/)
	{
		...
		if (ref.addr.ip != 0)
		{
			...
			bundle << baseAppMgr.latestVersion() <<
				baseAppMgr.impendingVersion();
		}
		else
		{
...
private:
		...
		BinaryIStream& data, void * /*arg*/)
	{
		...
		this->prependData( bundle );

		bundle.addBlob( data.retrieve( header.length ), header.length );
...
private:
			...
			BinaryIStream & data )
	{
		...
		// Make sure we have received word from the update server
		if (baseAppMgr.latestVersion_ == uint32(-1))
		{
			ERROR_MSG( "BaseAppMgr::createEntity: "
				"No word yet from update server\n" );
			baseAppAddr.port = 1;  // TODO: Remove magic numbers and using port field

			Mercury::Bundle	bundle;
			bundle.startReply( header.replyID );
			bundle << baseAppAddr;
			bundle << "Update server not yet ready.";
			BaseAppMgr::instance().nub().send( srcAddr, bundle );
			return;
		}
		...
		if (pBest->load() > baseAppMgr.baseAppOverloadLevel_)
		{
	...
	Mercury::ReplyID replyID, const BaseAppMgrInterface::addArgs & args )
{
	...
	if (!cellAppMgr_.channel().isEstablished())
	{
		WARNING_MSG( "BaseAppMgr::add: "
			"Not allowing BaseApp at %s to register before the CellAppMgr\n",
			srcAddr.c_str() );
...
//	TRACE_MSG( "BaseAppMgr::add:\n" );
	...
	this->commonAdd( *pBaseApp );
	pBaseApp->send();
}


/**
 *	This helper method performs code that is common to both add and addBackup.
 */
void BaseAppMgr::commonAdd( Mercury::ChannelOwner & channelOwner )
{
	// Tell it about the latest version if we already know it
	// (must be in a separate bundle so it has a chance to add its interface)
	if (latestVersion_ != uint32(-1))
	{
		Mercury::Bundle & b = channelOwner.bundle();
		BaseAppIntInterface::resourceVersionControlArgs & rvca =
			BaseAppIntInterface::resourceVersionControlArgs::start( b );
		rvca.version = latestVersion_;
		rvca.activity = 0;
	}

	// If we're in the middle of an update let it know about that also
	if (updaterStep_ != -1)
	{
		MF_ASSERT( updaterStep_ < 4 );	// must be early

		Mercury::Bundle & b = channelOwner.bundle();
		for (int i = 1; i <= updaterStep_; i++)
		{
			BaseAppIntInterface::resourceVersionControlArgs & rvca =
				BaseAppIntInterface::resourceVersionControlArgs::start( b );
			rvca.version = coordinatingVersion_;
			rvca.activity = i;
		}
	}
}
	...
	const BaseAppMgrInterface::addBackupArgs & args )
{
	...
	BaseAppID id = this->getNextID();
	TRACE_MSG( "BaseAppMgr::addBackup: %s id = %d\n",
		(char *)args.addr, id );
			...
			BinaryIStream & data )
{
	...
	DEBUG_MSG( "BaseAppMgr::recoverBaseApp: %s, id = %d\n",
...
void BaseAppMgr::del( const BaseAppMgrInterface::delArgs & args,
					 const Mercury::Address & addr )
{
	TRACE_MSG( "BaseAppMgr::del: %d\n", args.id );

	// First try real Base Apps.

	if (this->onBaseAppGone( addr, false ))
	{
		DEBUG_MSG( "BaseAppMgr::del: now have %d base apps\n",
				baseApps_.size() );
	}
	else if (this->onBackupBaseAppGone( addr ))
	{
		DEBUG_MSG( "BaseAppMgr::del: Now have %d backup base apps\n",
			backupBaseApps_.size() );
	}
	else
	{
		ERROR_MSG( "BaseAppMgr: Error deleting %s id = %d\n",
			(char *)addr, args.id );
	}
}
...
/**
 *	This method checks and handles the case where a Base App may have stopped.
 */
bool BaseAppMgr::onBaseAppGone( const Mercury::Address & addr,
							   bool shouldRestore )
{
	...
	if (iter != baseApps_.end())
	{
		INFO_MSG( "BaseAppMgr::onBaseAppGone: baseapp%02d @ %s\n",
		...
		{
			ERROR_MSG( "BaseAppMgr::onBaseAppGone: "
		...
		if (shouldHardKillDeadBaseApps_)
		{
			...
			INFO_MSG( "BaseAppMgr::onBaseAppGone: Sending SIGQUIT to %s\n",
					(char *) baseApp.addr() );
			if (!sendSignalViaMachined( baseApp.addr(), SIGQUIT, nub_ ))
			{
				ERROR_MSG( "BaseAppMgr::onBaseAppGone: Failed to send "
						"SIGQUIT to %s\n", (char *) baseApp.addr() );
			}
		}
		...
		if (shouldRestore)
		{
			...
			this->onBackupBaseAppGone( pBackup->addr() );
		...
		else
		{
			...
			{
				controlledShutDown = true;
				NOTICE_MSG( "BaseAppMgr::onBaseAppGone: "
			...
			else if (baseApp.backupHash().empty())
			{
				...
				if (baseApp.newBackupHash().empty())
				{
					ERROR_MSG( "BaseAppMgr::onBackupBaseAppGone: "
							"No backup for %s\n", (char *)iter->first );
				}
				else
				{
			...
			// Tell all the baseapps that the dead one is gone.
			MemoryOStream args;
			args << iter->first << baseApp.backupHash();
			this->sendToBaseApps( BaseAppIntInterface::handleBaseAppDeath,
				args );
 ...
 *	stopped.
 */
bool BaseAppMgr::onBackupBaseAppGone( const Mercury::Address & addr )
{
	...
	if (iter != backupBaseApps_.end())
	{
		...
		while (iter != backedUpSet.end())
		{
			...
			{
				WARNING_MSG( "BaseAppMgr::onBackupBaseAppGone: "
...
{
	this->shutDown( shouldShutDownOthers_ );
}
...
/**
 *	This method processes a message from the DBMgr that sets us up for
 * 	restoration of entities from the database.
 */
void BaseAppMgr::prepareForRestoreFromDB( const Mercury::Address & addr,
			const Mercury::UnpackedMessageHeader & header,
			BinaryIStream & data )
{
	MF_ASSERT( !hasStarted_ );

	// Save time for new BaseApps
	data >> time_;

	INFO_MSG( "Restore from database: game time=%.1f\n",
	...
	// Send game time to BaseApps
	{
		MemoryOStream payload;
		payload << time_;

		this->sendToBaseApps( BaseAppIntInterface::setGameTime, payload );
		this->sendToBackupBaseApps( BaseAppIntInterface::setGameTime, payload );
	}
}
...
void BaseAppMgr::startTimer()
{
	...
	{
		hasStarted_ = true;
		int gtid = nub_.registerTimer( 1000000/updateHertz_,
				this,
...
public:
	...
	virtual void prependData( Mercury::Bundle & bundle )
	{
		bundle << BaseAppMgr::instance().numBaseApps();
	}
};
			...
			BinaryIStream & data )
{
	...
	else
	{
		...
		Mercury::Bundle & bundle = this->dbMgr().bundle();
		bundle.startReply( header.replyID );
		// -0x10 indicates the the CellAppMgr is not ready.
		// TODO: Improve scheme for reporting status.
		bundle << this->numBaseApps() << -0x10;
		this->dbMgr().send();
	}
}
...
void BaseAppMgr::handleBaseAppDeath( const Mercury::Address & addr )
{
	...
	if (!this->onBaseAppGone( addr, true ))
	{
		this->onBackupBaseAppGone( addr );
	}
}
...
/**
 *	This method handles the resource version control message.
 */
void BaseAppMgr::resourceVersionControl(
	const BaseAppMgrInterface::resourceVersionControlArgs & args )
{
	// record the step that the updater has decreed
	if (args.activity >= 1 && args.activity < 256)
	{
		coordinatingVersion_ = args.version;
		updaterStep_ = (int)args.activity;
	}
	...
	{
	case 0:
		DEBUG_MSG( "BaseAppMgr::resourceVersionControl: "
			"Latest version is %d\n", args.version );
		latestVersion_ = args.version;
		impendingVersion_ = args.version;
		break;
	case 1:
		DEBUG_MSG( "BaseAppMgr::resourceVersionControl: "
			"Going to step 1 for new version %d\n", coordinatingVersion_ );

		// TODO: get from srcAddr instead of looking it up!
		nub_.findInterface( "UpdaterInterface", 0, updaterAddr_ );

		this->sendUpdaterStepAck( 1 );
		break;
	case 2:
		// all components will now assume that the update is afoot
		impendingVersion_ = args.version;
		this->sendUpdaterStepAck( 2 );
	case 3:
		// the new version is now imminent - clients are prevented from logging
		// on until they have downloaded all the new resources
		// (handled by BaseApp)
		this->sendUpdaterStepAck( 3 );
		break;
	case 4:
		// no new processes are permitted to enter the system
		allowNewBaseApps_ = false;
		this->sendUpdaterStepAck( 4 );
		break;
	case 5:
		// all components are told to load the new resources
		// the bundle version identifier for the old version is inserted into
		// every bundle (that contains entity-related messages,
		// i.e. script/property updates)
		// (TODO: the bundle stuff)

		// no new client logins are permitted
		// (we do not prevent clients from logging in,
		// rather their proxy prevents them from enabling entities)

		this->sendUpdaterStepAck( 5 );
	case 6:
		// all components are told to switch to the new resources
		latestVersion_ = args.version;
		this->sendUpdaterStepAck( 6 );

		DEBUG_MSG( "BaseAppMgr::resourceVersionControl: "
			"Gone to step 6 and version is now %d\n", latestVersion_ );

		break;
	case 7:
		// client logins are permitted once more
		// (at the proxy enabling entities level)

		// the bundle version identifier is no longer sent between
		// internal (server) components
		// (TODO: the bundle stuff)

		this->sendUpdaterStepAck( 7 );
		break;
	case 8:
		// all components discard any machinery they kept around to handle
		// messages of the old version
		// (nothing to do here)
		this->sendUpdaterStepAck( 8 );
		break;
	case 9:
		// new server processes are permitted to enter the system
		allowNewBaseApps_ = true;
		updaterStep_ = -1;	// the update is over! yay!
		this->sendUpdaterStepAck( 9 );
		DEBUG_MSG( "BaseAppMgr::resourceVersionControl: "
			"Gone to step 9 and update is complete\n" );
		return;	// don't send this to anyone else
	default:
		ERROR_MSG( "BaseAppMgr::resourceVersionControl: "
			"Received unknown activity %d\n", args.activity );
		break;
	};

	// and send it to all our proxies
	MemoryOStream payload;
	payload << args;

	this->sendToBaseApps( BaseAppIntInterface::resourceVersionControl,
			payload );
	this->sendToBackupBaseApps( BaseAppIntInterface::resourceVersionControl,
			payload );
}


/**
 *	Helper method to send an ack to the updater.
 */
void BaseAppMgr::sendUpdaterStepAck( int step )
{
	Mercury::Bundle bundle;
	UpdaterInterface::ackStepArgs asa = { step };
