...
static const char* DATABASE_FILENAME = "entities/db.xml";
static const char* DATABASE_INFO_SECTION = "_BigWorldInfo";
static const char* DATABASE_LOGONMAPPING_SECTION = "LogOnMapping";


// -----------------------------------------------------------------------------
// Section: OldXMLDatabase
// -----------------------------------------------------------------------------
class OldXMLDatabase : public IDatabase::IOldDatabase
{
	XMLDatabase&		xmlDb_;
	const EntityDefs& 	entityDefs_;

public:
	OldXMLDatabase( XMLDatabase& xmlDb, const EntityDefs& entityDefs ) :
		xmlDb_(xmlDb), entityDefs_(entityDefs)
	{}
...
void OldXMLDatabase::getEntity( IDatabase::IGetEntityHandler& handler )
{
	xmlDb_.getEntity( handler, entityDefs_ );
}

void OldXMLDatabase::putEntity( const EntityDBKey& ekey,
	EntityDBRecordIn& erec, IDatabase::IPutEntityHandler& handler )
{
	xmlDb_.putEntity( ekey, erec, handler, entityDefs_ );
}
		...
		bool /*isFaultRecovery*/, bool /*isUpgrade*/ )
{
	...
	pDB_ = BWResource::openSection( DATABASE_FILENAME );
	...
	if (!pDB_)
	{
		ERROR_MSG( "XMLDatabase::startup: Could not open %s: Creating it.\n",
				DATABASE_FILENAME );
		pDB_ = BWResource::openSection( DATABASE_FILENAME, true );
	}
...
bool XMLDatabase::shutDown()
{
	if (pDB_)
	{
		BWResource::instance().save( DATABASE_FILENAME );
...
{
	this->getEntity( handler, *pEntityDefs_ );
}
...
{
	this->putEntity( ekey, erec, handler, *pEntityDefs_ );
}

/**
 *	Internal implementation of IDatabase::putEntity that allows for different
 * 	EntityDefs.
 */
void XMLDatabase::putEntity( const EntityDBKey& ekey, EntityDBRecordIn& erec,
							 IDatabase::IPutEntityHandler& handler,
...
bool XMLDatabase::commit()
{
	if (pDB_)
	{
		return BWResource::instance().save( DATABASE_FILENAME );
	}
 ...
 *	Override from IDatabase.
 */
void XMLDatabase::putIDs( int count, const ObjectID * ids )
{
 ...
 *	Override from IDatabase.
 */
void XMLDatabase::migrateToNewDefs( const EntityDefs& newEntityDefs,
	IMigrationHandler& handler )
{
	if (!pNewEntityDefs_)
	{
		// Name property can't change at the moment or else we need to repopulate
		// nameToIdMaps_ and do custom implementation for get/putEntity by name
		// in OldXMLDatabase
		MF_ASSERT( newEntityDefs.hasMatchingNameProperties( *pEntityDefs_ ) );
		pNewEntityDefs_ = &newEntityDefs;

		// There's nothing to migrate at the moment. XML database will work with
		// the new definitions just fine.
		handler.onMigrateToNewDefsComplete( true );
	}
	else
	{
 ...
 *	Override from IDatabase.
 */
IDatabase::IOldDatabase* XMLDatabase::switchToNewDefs(
	const EntityDefs& oldEntityDefs )
{
	OldXMLDatabase*	pOldXMLDb;
	if (pNewEntityDefs_)
	{
		pEntityDefs_ = pNewEntityDefs_;
		pOldXMLDb = new OldXMLDatabase( *this, oldEntityDefs );
