...
#define MF_RAW_VARLEN_CELL_APP_MSG( NAME )									\
	MERCURY_HANDLED_VARIABLE_MESSAGE( NAME, 2,								\
			RawVarLenMessageHandler< CellApp >,								\
			&CellApp::NAME )
...
#define MF_RAW_SPACE_MSG( NAME ) 											\
	MERCURY_HANDLED_VARIABLE_MESSAGE( NAME, 2, RawMessageHandler<Space>,	\
			&Space::NAME )
...
MF_BEGIN_INTERFACE( CellAppInterface )
	...
	// CellApp messages
	// -------------------------------------------------------------------------
	MF_RAW_VARLEN_CELL_APP_MSG( addCell )
		// SpaceID spaceID;
	...
	MF_BEGIN_CELL_APP_MSG( resourceVersionControl )
		uint32				version;
		uint32				activity;
	MF_END_MSG()

	MF_BEGIN_CELL_APP_MSG( resourceVersionTag )
		uint8				tag;
	MF_END_MSG()
	...
	MF_RAW_VARLEN_CELL_APP_MSG( onloadTeleportedEntity )

	MF_BEGIN_CELL_APP_MSG( cellAppMgrInfo )
...
#define SPACE_GEOMETRY_LOADED_BOOTSTRAP_FLAG	0x01
	...
	// The arguments are as follows:
	// 	ObjectID		The id for the new entity
	...
	MF_BEGIN_CELL_MSG( retireCell )
		bool isStart;
	MF_END_MSG()

	MF_VARLEN_CELL_MSG( removeCell )

	MF_VARLEN_CELL_MSG( notifyOfCellRemoval )

	MF_RAW_CELL_MSG( ackCellRemoval )
	...
	MF_BEGIN_ENTITY_MSG( setSpaceViewportAck, WITNESS_ONLY )
		ObjectID			id;
		SpaceViewportID		spaceViewportID;
	MF_END_MSG()

	MF_BEGIN_ENTITY_MSG( setVehicleAck, WITNESS_ONLY )
		ObjectID			id;
		ObjectID			vehicleID;
	MF_END_MSG()

	MF_BEGIN_ENTITY_MSG( gatewaySubscribe, REAL_ONLY )
		ObjectID			gatewaySrcID;
		Mercury::Address	cellHost;
		SpaceID				spaceID;
		float				range;
	MF_END_MSG()
