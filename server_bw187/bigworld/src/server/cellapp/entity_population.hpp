...
class EntityPopulation :
	public std::map< ObjectID, Entity * >,
...
public:
	...
	*/
	...
	void addObserver( ObjectID id, Observer * pObserver ) const;
	bool removeObserver( ObjectID id, Observer * pObserver ) const;

	void notifyObservers( Entity & entity ) const;

	// TODO: Rename these calls/fields to be 'real location'.
	void rememberRealChannel( ObjectID id, CellAppChannel & channel );

	void forgetRealChannel( ObjectID id )
	{
	...
	CellAppChannel * findRealChannel( ObjectID id ) const;

private:
	typedef std::map< ObjectID, CellAppChannel* > RealChannels;
	virtual void handleCellAppDeath( const Mercury::Address & addr );
	void forgetRealChannel( RealChannels & channels,
		const Mercury::Address & addr );

	typedef std::multimap< ObjectID, Observer * > Observers;
