 ...
 *  being accessable through "getStealthFactor" and "setStealthFactor".
 *
...
public:
	...
	void setToInitialState( ObjectID id, Space * pSpace );
	~Entity();
	...
	void writeGhostDataToStream( BinaryOStream & stream );

	void offload( CellAppChannel * pChannel );
	...
	uint8 physicsCorrected() const	{ return physicsCorrected_; }
	...
	static void disableBackups()		{ s_backupsDisabled_ = true; }
	static bool backupsDisabled()		{ return s_backupsDisabled_; }
...
public:
	...
	void setSpaceViewportAck(
		const CellAppInterface::setSpaceViewportAckArgs & args );
	void setVehicleAck(
		const CellAppInterface::setVehicleAckArgs & args );

	void gatewaySubscribe( const CellAppInterface::gatewaySubscribeArgs& args );
	...
	PY_AUTO_METHOD_DECLARE( RETVOID, destroySpace, END );
	void destroySpace();
	...
	bool getEntitiesInRange( EntityReceiver & receiver, float range,
			PyObjectPtr pClass = NULL, PyObjectPtr pActualPos = NULL );
	void findEntitiesInSquare( float range, EntityVisitor & visitor ) const;

	void fakeID( ObjectID id );
	...
	bool physicallyPossibleGatewayTraversal( Entity * pGatewaySrc,
		Entity * pGatewayDst, const Vector3 & newGlobalPos );
...
private:
	...
	void avatarUpdateCommon( const Coord & pos, const YawPitchRoll & dir,
		bool onGround, const RelPosRef & posRef );
	bool avatarUpdateSwitchSpace(
		const CellAppInterface::avatarUpdateExplicitArgs & args );
	...
	bool writeClientMessageToBundle( Mercury::Bundle & bundle,
		...
		MemoryOStream & argstream, int callingMode );
	...
	void writeGhostControllersToStream( BinaryOStream & stream );

	void readRealControllersFromStream( BinaryIStream & data );
	void writeRealControllersToStream( BinaryOStream & stream );

	void startRealControllers();

	void runMethodHelper( BinaryIStream & data, int methodID, bool isExposed );

	bool sendDBDataToBase( bool isDestroy,
...
private:
	...
	static bool s_backupsDisabled_;
 ...
 *	(note that if the entity is moving through a gateway, you will be called
 *	twice - once for the near side of the gateway, and once for the far side,
 *	and on the far side you will not be in the range list of that space yet)
 *	(also note that even if it returns true, the move may still be disallowed
