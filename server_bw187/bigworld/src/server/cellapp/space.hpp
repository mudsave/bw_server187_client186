...
public:
	...
	void createGhost( const EntityID entityID,
					  BinaryIStream & data );
		// ( ObjectID id, TypeID type, Position3D & pos,
		//  string & ghostState, Mercury::Address & owner );

	void addEntity( Entity * pEntity );
	void removeEntity( Entity * pEntity );

	EntityPtr newEntity( ObjectID id, EntityTypeID entityTypeID );
	...
	void addSubscribers( GatewayDstController * pGatewayDst );
	void delSubscribers( GatewayDstController * pGatewayDst );

	typedef std::vector<GatewayDstController*> GatewayDstSubscribers;
	const GatewayDstSubscribers & subscribers() const	{ return subscribers_; }
...
private:
	...
	GatewayDstSubscribers	subscribers_;
