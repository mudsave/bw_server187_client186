...
public:
	...
	PY_AUTO_METHOD_DECLARE( RETOWN, moveToEntity, 
		ARG( int, ARG( float, ARG( float, OPTARG( int, 0, END ) ) ) ) )
	...
	PY_AUTO_METHOD_DECLARE( RETOWN, navigate, 
		...
		OPTARG( bool, true, OPTARG( float, 500.f, OPTARG( float, 0.5f, OPTARG( float, 0.01f, OPTARG(int, 0, END ) ) ) ) ) ) ) )
	...
	PyObject * navigateStep( const Vector3 & dstPosition, float velocity,
		float maxMoveDistance, float maxDistance = 500.f, bool faceMovement = true,
	...
	PY_AUTO_METHOD_DECLARE( RETOWN, findCover, 
		ARG( float, ARG( Vector3, 
		OPTARG( int, 0, OPTARG( float, 0.5f, END ) ) ) ) )
	PyObject * findCover( float radius, const Vector3 & enemyPt, 
		int coverType, float girth );
	...
	PyObject * getStopPoint( const Vector3 & dstPosition, 
			...
			float girth = 0.5f, float stopDistance = 1.5f, float nearPortalDistance = 1.8f );
