 ...
 */
INLINE EntityCache::EntityCache( ObjectID dummyID ) :
 ...
 */
INLINE float EntityCache::updatePriority( const Vector3 & origin, float offset )
{
	...
	const Priority INV_DISTANCE_RESOLUTION = 1.0/5.0;

	float diffX = this->pEntity()->position().x - origin.x;
	float diffZ = this->pEntity()->position().z - origin.z;
	const Priority distSQ = diffX * diffX + diffZ * diffZ + offset * offset;

	Priority delta = (sqrt(distSQ) * INV_DISTANCE_RESOLUTION + 1.0) /
					this->pEntity()->aoiPriority();
 ...
 */
INLINE
ObjectID EntityCache::dummyID() const
{
	MF_ASSERT( !this->pEntity() );
	return dummyID_;
}
 ...
 */
INLINE
void EntityCache::dummyID( ObjectID dummyID )
{
	MF_ASSERT( !this->pEntity() );
	dummyID_ = dummyID;
}
...
/**
 *	This method gets the primary viewport index.
 */
INLINE uint8 EntityCache::viewportIndex() const
{
	return viewportIndex_[0];
}

/**
 *	This method gets the array of viewport indices.
 */
INLINE uint8 * EntityCache::viewportIndices()
{
	return viewportIndex_;
}

/// Get viewportIndexData as a chunk
INLINE const uint32 EntityCache::viewportIndexData() const
{
	return (const uint32&)viewportIndex_[0];
}

/// Get viewportIndexData as a chunk
INLINE uint32 & EntityCache::viewportIndexData()
{
	return (uint32&)viewportIndex_[0];
}
