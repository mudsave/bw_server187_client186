...
{
	metresPerTick_ = velocity / CellApp::instance().updateHertz();
}
...
void MoveToPointController::update()
{
	if (this->move(destination_))
	{
		...
		START_PROFILE( ON_MOVE );
		this->standardCallback("onMove");
		STOP_PROFILE( ON_MOVE );

		if (this->isAttached()) this->cancel();
	}
}
 ...
 *	@param moveVertically	Whether or not the entity should move vertically.
 */
MoveToEntityController::MoveToEntityController( ObjectID destEntityID,
...
void MoveToEntityController::update()
{
	...
	// If we're getting close, point us in the right direction.

	/*
	if(remaining.length() - metresPerTick_ < range_)
	{
		printf("Pointing in right direction. %f %f %f\n",
				remaining.length(), metresPerTick_ , range_);
		offset_.x(0);
		offset_.y(0);
	}
	*/
