 ...
 *  the triangles should be visibile from both in front and behind.
 *
 *
 *  Example:
 *  @{
 ...
 *  are enforcable by the game's server.
...
ChunkPortal::ChunkPortal( ChunkBoundary::Portal * pPortal, PyTypePlus * pType ):
	...
	ChunkItem( 65536 ),	// 65536 => don't account for this item
	pPortal_( pPortal ),
	triFlags_( 0 ),
	activated_( false )
{
...
void ChunkPortal::toss( Chunk * pChunk )
{
	...
	{
		char buf[32];
		sprintf( buf, "portal_%08X", unsigned(pPortal_) );
		label = buf;
	}
	...
	SpaceID spaceID )
{
	...
	{
		char sillybuf[256];
		sprintf( sillybuf, "BigWorld.findChunkFromPoint(): "
