...
{	    
        ...
        for(int i=0;i<len;i++)
        {
		   ...
		   string s2 = HexCharToBinary(sz_P2[i]);//都转为0101这样的串
		   char val[5] ={0};
...
{	
	string strTemp = "";
	if(1 == len%2)return "";//要有偶数个字符,否则不转换
	for(int i=0; i<len; i++)
	{
		//输入的字符要在 0--9 A--F a--f之间
		if ( (src[i]<'0')
			||(src[i]>'9' && src[i]<'A')
			||(src[i]>'F' && src[i]<'a')
			||(src[i]>'f'))
...
{	
    ...
    string nextVevtor(m_initVector);//16位
	...
	for(int i = 0; i<iCount;i++)
	{
		string szTmp = strPlaintText.substr(i*8,8);//取出8位
		string sz16tmp = ConverByesToChars(szTmp.c_str(),8);//转为16位字节
		
		XOR16((char*)sz16tmp.c_str(),(char*)nextVevtor.c_str(),16,XORData);
        realEncryptData = ConverCharsToByes(XORData,16);//转为要加密的8字节
...
{
	//这里只取前8字节，输入长度由调用者控制。
    string temp = ConverByesToChars(initVector,8);//temp是16字节了
