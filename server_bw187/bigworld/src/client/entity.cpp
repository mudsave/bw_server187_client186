...
///	Constructor
Entity::Entity( EntityType & type, ObjectID id, Vector3 & pos,
	...
	targetFullBounds_( false )
{
	...
	if (it == s_entitiesOfType->end())
	{
		...
		MF_WATCH( (std::string("EntityCounts/") + type_.name()).c_str(), it->second.first );
		MF_WATCH( (std::string("EntityCounts/") + type_.name() + std::string(" ever")).c_str(), it->second.second );
	}
	else
	{
		it->second.first++;
...
Entity::~Entity()
{
	...
	s_entitiesCurrent--;
	ETypes::iterator it = s_entitiesOfType->find( type_.name() );
	MF_ASSERT(it != s_entitiesOfType->end());
	it->second.first--;
#endif
}
 ...
 */
class PrerequisitesOrder
{
public:
	PrerequisitesOrder( Entity * pEntity ) :
		loaded_( false ), pEntity_( pEntity ) { }
	~PrerequisitesOrder() { }

	static void s_load( void * arg ) { ((PrerequisitesOrder*)arg)->load(); }
...
bool Entity::checkPrerequisites()
{
	...
	{
		if (!prerequisitesOrder_->loaded_) return false;

		// keep the resources around if we will enter the world
		if (enterCount_ > 0)
			resourceRefs_ = prerequisitesOrder_->resourceRefs_;

		// it's done! now never call us again...
		delete prerequisitesOrder_;
		prerequisitesOrder_ = NULL;
		return true;
	}
	...
	prerequisitesOrder_ = new PrerequisitesOrder( this );
	prerequisitesOrder_->resourceIDs_  = missingPrereqs;
	prerequisitesOrder_->resourceRefs_ = presentPrereqs;

	ChunkLoader::LoadOrder lo;
	lo.func_ = PrerequisitesOrder::s_load;
	lo.arg_ = prerequisitesOrder_;
 ...
 */
void Entity::enterWorld( SpaceID spaceID, ObjectID vehicleID, bool transient )
{
	MF_ASSERT( !pSpace_ );
...
void Entity::leaveWorld( bool transient )
{
	if (!transient)
	{
		...
		Script::call( PyObject_GetAttrString( this, "leaveWorld" ), PyTuple_New(0),
...
void Entity::tick( double timeNow, double timeLast )
{
	if (tickAdvanced_)
	{
		// tick is already called by a passenger (controlled entity)
		// clear the flag.
		tickAdvanced_ = false;
		return;
	}
...
PY_BEGIN_ATTRIBUTES( Entity )
	 ...
	 *
	 *		0:	avatarStyle, suitable for player, other controlled animals, etc.
	 *		1:	hoverStyle, for any hovering vehicle physics.
	 *		2:	limpetStyle, has no effect (noStyle) unless Physics::chase() activated.
	 *			causes controlled Entity to follow exact movements of Entity being chased.
	 *		3:	weaponEmplacementStyle, useful for controlled turrets, guns, etc.
	 *		>3:	noStyle, performs update on current Filter without any physics processing.
	 *
	 ...
	 *	If true, targeting uses the full bounding box of this Entity
	 *	@type bool
	 */
...
PyObject * Entity::delModel( PyObjectPtr pEmbodimentPyObject )
{
	...
	*
	*	The "entitiesInTrap" argument is a count of the number of entities that are currently inside the "trap".
	*	Because Python is Python, you can use "Functor( trapCallback( self, entitiesInTrap, otherData, ... ) )" to
	*	pass in any additional data you may need, however the basic prototype must form the first parameters...
	*	See BigWorld.addPot for a cheaper method that only works for players.
	*
...
PyObject * Entity::py_addTrap( PyObject * args )
{
	...
	{
		PyErr_SetString( PyExc_TypeError, "py_addTrap: Argument parsing error." );
		return NULL;
	}
...
PyObject * Entity::py_delTrap( PyObject * args )
{
	...
	{
		PyErr_SetString( PyExc_TypeError, "py_delTrap: Argument parsing error." );
		return NULL;
	}
...
int Entity::pySet_filter( PyObject * value )
{
	...
	{
		double time = getGameTotalTime();
		SpaceID topSID;
		ObjectID topVID;
		Vector3 topPos = position_;
		float topAuxVol[3];
		for (uint i = 0; i<3; i++) topAuxVol[i] = auxVolatile_[i];
		bool gotInput = pFilter_->getLastInput( time, topSID, topVID, topPos, topAuxVol );
		...
		if (gotInput)
		{
			pFilter_->input( time, topSID, topVID, topPos, topAuxVol );
		}
	}
...
public:
	...
	virtual void matrix( Matrix & m ) const
	{
		...
		{
			m.setRotateY( pEntity_->auxVolatile()[0] );
...
{
	float tdiff = float( max( 0.0, getGameTotalTime() - visiblityTime_ ) );
...
bool Entity::setInvisible( bool invis )
{
	...
	else if (invisible_ != invis)
	{
		...
		double timeNow = getGameTotalTime();
...
//	DEBUG_MSG( "Entity::setProperty( '%s' )\n", propName.c_str() );
	...
	if (propName == "yaw")
	{
		...
		if (pFilter_)
		{
			...
			pFilter_->getLastInput( time, topSID, topVID, topPos, topAuxVol );
			topAuxVol[0] = yaw;
			pFilter_->input( time+0.00001, topSID, topVID, topPos, topAuxVol );
		}
		else
		{
			auxVolatile_[0] = yaw;
...
{
	MF_ASSERT( PyObject_IsSubclass( (PyObject *)pClass,
...
{
	MF_ASSERT( this->ob_type == type_.pClass() );
...
{
	MF_ASSERT( this->ob_type == type_.pPlayerClass() );
...
const Matrix & Entity::fallbackTransform() const
{
	...
	{
		fakeTransform.setRotateY( auxVolatile_[0] );
...
bool Entity::controlled( bool shouldBeControlled )
{
	...
	if (shouldBeControlled == this->selfControlled()) return true;
	if (shouldBeControlled)
	{
		...
		Script::call(
			this->isPoseVolatile( true ),
			Py_BuildValue( "(i)", int(true) ),
			"Entity::controlled",
			true );

		if (PyObject_HasAttrString( this, "onControlled" ))
		{
