	...
	debugDraw_( false )
{
	...
	MF_WATCH( "Client Settings/Entity Picker/selectionFoV",
		selectionFoV_,
		Watcher::WT_READ_WRITE,
		"The field-of-view of the selection frustum, for selection." );

	MF_WATCH( "Client Settings/Entity Picker/deselectionFoV",
		deselectionFoV_,
		Watcher::WT_READ_WRITE,
		"The field-of-view of the selection frustum, for de-selection." );
...
{
	MF_ASSERT( entity.pPrimaryEmbodiment() != NULL );
 ...
 *
 *	@param pDelta		If non-NULL, a pointer to float containing the time since
	...
	Vector3 & sourcePos ) const
{
	...
	else
	{
		...
		sourcePos = sourceMatrix.applyToOrigin();
...
private:
	...
	PY_RW_ACCESSOR_ATTRIBUTE_DECLARE( Entity *, exclude, exclude )
	...
	Entity * exclude() const	{ return EntityPicker::instance().exclude(); }
	void exclude( Entity * e )
	{
		EntityPicker::instance().exclude( e );
		Py_XDECREF( e );
	}
};
...
PyObject * PyTarget::py_caps( PyObject * args )
{
	...
	for (int i=0; i<ncaps; i++)
	{
		...
		{
			char	str[256];
			sprintf( str, "py_targetCaps: Argument %d is not an integer.", i+1 );
