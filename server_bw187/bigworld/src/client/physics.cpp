...
#if 0 // not used because waypointPhysicsHandler is no longer used
#include "waypoint/waypoint_flood.hpp"
#endif
...
public:
	MyDebug()
	{
		MF_WATCH( "debug/stand still", g_standStill );
		MF_WATCH( "debug/reverse collision check", g_reverseCollisionCheck );
		MF_WATCH( "debug/uphill slow down",
			g_slowUpHill,
...
void Physics::tick( double timeNow, double timeLast )
{
	...
	else if (style_ == 2 && chaseTarget_ != NULL && chaseTarget_->isInWorld())
	{
		SpaceID spaceID = chaseTarget_->pSpace()->id();
		ObjectID vehicleID = chaseTarget_->isOnVehicle() ?
		...
		pSlave_->filter().input( timeNow,
			spaceID, vehicleID, newPos, newAuxVolatile );
	}
	else if (style_ == 3)
	{
...
struct AvatarAnnalData
{
	AvatarAnnalData() {}
	AvatarAnnalData( const Vector3 & p, float d ) :
		newPos( p ), dist( d ) {}

	Vector3	newPos;
	float	dist;

	AvatarAnnalData operator+( AvatarAnnalData & aad ) const
	{
		return AvatarAnnalData( newPos + aad.newPos, dist + aad.dist );
	}

	AvatarAnnalData operator*( float c ) const
	{
		return AvatarAnnalData( newPos * c, dist * c );
	}
};

static AnnalSynchronised< AvatarAnnalData, TweenNumeric<AvatarAnnalData> >
	s_avatarAnnal;
...
void Physics::avatarStyleTick( double timeNow, double timeLast )
{
	...
	pSlave_->filter().getLastInput( unused,
		spaceID, vehicleID, oldPos, unused2 );
	this->transformLocalToCommon(
	...
	{
		velocityYaw = pSlave_->auxVolatile()[0];
		thisPitch	= pSlave_->auxVolatile()[1];
	}
	...
	bool usingKeyboard = InputDevices::joystick().isUsingKeyboard() || tempJoystickDisabled_;
	tempJoystickDisabled_ = false;
	...
	// If you accelerate faster than 1m/s/s, it takes you blendInTime to get there,
	...
	if (fabs(thisSpeed - lastSpeed) > relevantAccel*dTime)
	{
		blendingFromSpeed_ = lastBlendedSpeed_;
		blendingFromFinish_ = timeLast + 0.3;
	...
	if (blendingFromFinish_ > timeNow)
	{
		float requiredSpeed = thisSpeed + LINEAR_SCALE(
			...
			0.3f );	// TODO: Don't hardcode this.
		// Best would be to look up the actual action, and use
		//  its blend in time .. i.e. do action matcher twice
		//  for player... hmmm...
	...
	if (fall_)
	{
		droppingTime_ = this->applyGravity( vehicleID, oldPos, newPos, mc, dTime );
	}

	// Let's make sure the new position will put us the on top of the terrain,
		...
		pEmb->chunk()->isOutsideChunk())
	{
		...
		if (ChunkOverlappers::instance.exists( *pEmb->chunk() ))
		{
			...
			for (uint i = 0; i < overlappers.size(); i++)
			{
				...
				if (pOverlapper->bb().intersects( newPos, 0.1f ))
				{
					popToTerrain = false;
					posNotLoaded = true;
					break;
	...
	if (pEmb != NULL && pEmb->chunk() == NULL) posNotLoaded = true;
	...
	// Scale down our speed if we're going uphill
	...
	if (g_slowUpHill && newPos.y > oldPos.y)
	{
...
*/		
	...
	// If we're recording, we'll try recording it here
	if (AnnalVault::isRecording())
	{
		Vector3 disp( newPos - oldPos );
		if (fabs(disp[1]) < 0.2 * dTime) disp[1] = 0.f;
			// remove small falls/jumps incurred by corrections to dodgy
			// turning animations

		s_avatarAnnal.push( AvatarAnnalData( newPos, disp.length() ) );
	}
	// If we're playing back, then forget all that and use the playback data
	//  (let's try not doing anything about the seek just yet too...)
	else if (AnnalVault::isPlaying())
	{
		AvatarAnnalData	aad;
		s_avatarAnnal.pop( aad );
		if (aad.dist != 0.f || AnnalVault::atFrame() <= 1)
		{
			newPos = aad.newPos;
		}
	}
	...
	if (teleportSpace_)
	{
		...
		if (teleportVehicle_ != ObjectID(-1))
		{
			vehicleID = teleportVehicle_;

			teleportVehicle_ = ObjectID(-1);
		}
	...
	localAuxVolatile[1] = mc.modelPitch + newHeadPitch;
...
void Physics::weaponEmplacementStyleTick( double timeNow, double timeLast )
{
	...
	if (pEmb != NULL && pEmb->chunk() != NULL && pEmb->chunk()->isOutsideChunk())
	{
		...
		for (uint i = 0; i < overlappers.size(); i++)
		{
			ChunkOverlapper * pOverlapper = &*overlappers[i];
			if (pOverlapper->focussed()) continue;
			if (pOverlapper->bb().intersects( newPos ))
				popToTerrain = false;
		}
	}
	if (popToTerrain)
 ...
 */
void Physics::transformCommonToLocal( SpaceID spaceID, ObjectID vehicleID,
 ...
 */
void Physics::transformLocalToCommon( SpaceID spaceID, ObjectID vehicleID,
	Vector3 & pos, Vector3 & dir ) const
{
	if (vehicleID != 0)
	{
...
Vector3 Physics::movingForward( float maxDistance, Vector4ProviderPtr destinationPtr ) const
{
	...
	ObjectID vehicleID = pSlave_->isOnVehicle() ?
			pSlave_->pVehicle()->id() : 0;
	...
	while ((oldPos-newPos).lengthSquared() < maxDistance*maxDistance)
	{
		...
		float ndt = this->applyGravity( vehicleID, oldPos, newPos, mc, dTime );
		const_cast<Physics*>( this )->droppingTime_ = ndt;

		// if we moved less than half the desired distance, then stop
		if ((midPos-newPos).lengthSquared() < 0.1f*0.1f) break;
	}
	const_cast<Physics*>( this )->droppingTime_ = odt;
...
//	MF_ASSERT( delta.y == 0.f );
	...
	// off.
	const Vector3 widthVector( mc.modelWidth * flatDir.z, 0.f, -mc.modelWidth * flatDir.x );
	const Vector3 halfWidthVector = 0.5f * widthVector;
	const Vector3 heightVector( 0.f, mc.modelHeight - mc.scrambleHeight, 0.f );
	Vector3 bottomLeft = oldPos - halfWidthVector;
	bottomLeft.y += mc.scrambleHeight;

	// Make sure that we are at least this far away before we run into an object.

	float frontDistance = 0.0f;
	if (!reverseCheck)
		frontDistance = 0.5f * sqrtf( sqr(mc.modelDepth) + sqr(mc.modelWidth) );
...
#if 0
	// Debug start
	s_dtriadd( sweepTriangle, 0x00ffffff );
	WorldTriangle bufTri(
		sweepTriangle.v0() + frontBuffer,
		sweepTriangle.v1() + frontBuffer,
		sweepTriangle.v2() + frontBuffer );
	s_dtriadd( bufTri, 0x00ffffff );
	WorldTriangle destTri(
		sweepTriangle.v0() + delta + frontBuffer,
		sweepTriangle.v1() + delta + frontBuffer,
		sweepTriangle.v2() + delta + frontBuffer );
	s_dtriadd( destTri, 0x00ffffff );
	// Debug end
#endif

	float clear = collisionAdvance.advance() - frontDistance;

	Vector3 realDir = flatDir;
	realDir.y = (newPos.y - oldPos.y) / maxAdvance;
	...
	collided = (clear < maxAdvance - 0.0001f);
	if (shouldSlide && collided)
	{
		const float hitDepth = maxAdvance - clear;
		Vector3 leftOver = hitDepth * flatDir;
...
/**
 *	This class find the height of the topmost triangle encountered,
 *	limited to the range of a quadrilateral column.
 */
class TopTriangle : public CollisionCallback
{
public:
	TopTriangle( const Vector3 * corners ) :
		corners_( corners ),
		freeFall_( -1.f )
	{
	}

	const Vector3 * corners_;
	float freeFall_;
...
private:
		...
		const WorldTriangle & triangle, float dist )
	{
		...
		// find the topmost point of this triangle,
		//  as it appears within the given quadrilateral
		float ys[3] = { wt.v0().y, wt.v1().y, wt.v2().y };
		float fMax = corners_[1][1] - max( ys[0], max( ys[1], ys[2] ) );
		float fMin = corners_[1][1] - min( ys[0], min( ys[1], ys[2] ) );

		// and update freeFall_ to reflect that (but don't let it go -ve)
		if (fMax > 0.f)
			freeFall_ = (freeFall_ < 0.f) ? fMax : min( freeFall_, fMax );
		else if (fMin > 0.f)
			freeFall_ = 0.f;

		// TODO: actually clip the triangle to the quad passed in!
		// DEBUG: Draw this triangle green.
//		s_dtriadd( wt, 0x0000ff00 );
 ...
 */
float Physics::applyGravity( ObjectID & vehicleID, const Vector3 & oldPos, Vector3 & newPos,
	const MotionConstants & mc, float deltaTime ) const
{
	const ObjectID oldVehicleID = vehicleID;
	...
	if (pModel != NULL)
	{
		...
		if (pModel->chunk()->isOutsideChunk())
		{
			...
			for (uint i = 0; i < overlappers.size(); i++)
			{
				...
				if (pOverlapper->bb().intersects( testPos ))
					return droppingTime_;
			}
		}
	}
	...
	float dropDistance = (gravity_ / 2.0f) *
		(sqr(droppingTime_ + deltaTime) - sqr(droppingTime_ ));

	// we only need to check the max distance we can fall
	float maxDrop = dropDistance; // + FALL_HOP + FALL_HOP;
...
/*
	// figure out which way we're facing (for the purposes of this calc)
	Vector3 flatDir( testPos[0] - oldPos[0], 0.f, testPos[2] - oldPos[2] );
	flatDir.normalise();

	// TODO: Should probably pass in the model's direction rather than guessing
	// from the movement.
	if (flatDir.lengthSquared() < 0.9f)
	{
		flatDir.set(0.f, 0.f, 1.f);
	}
*/
...
//	collisionAdvance.shouldFindCentre( true );
	...
	WorldTriangle	dropOne( corners[1], corners[0], corners[3] );
	WorldTriangle	dropTwo( corners[2], corners[3], corners[0] );
	Vector3 dest( testPos[0], testPos[1] - maxDrop - 0.1f,testPos[2] );
	pSpace->collide( dropOne, dest, collisionAdvance );
	pSpace->collide( dropTwo, dest, collisionAdvance );

	// Are we falling?
	float newDroppingTime = droppingTime_;

	// reset vehicle
	ObjectID newVehicleID = 0;
	if (collisionAdvance.advance() == maxDrop)
	{
		// we didn't hit anything must be on the ground
		newDroppingTime += deltaTime;
	}
	else
	{
		if (oldVehicleID != 0)
	...
	if (vehicleID != 0 && 
		dropDistance < collisionAdvance.advance())
	{
	...
	if( testPos == newPos )
	{
		newDroppingTime = 0.0f;
	}

	newPos = testPos;

	return newDroppingTime;
}
 ...
 *						to teleport to (pitch, roll, yaw), otherwise the
...
PyObject * Physics::py_seek( PyObject * args )
{
	...
	else
	{
		...
		if (timeLimit >= 0.f )
		{
			seekDeadline_ = getGameTotalTime() + timeLimit;
		}
		else
		{
			seekDeadline_ = -1.f;
...
/**
 *	Special set method for python attribute 'fall'
 */
int Physics::pySet_fall( PyObject * value )
{
	bool oldFall = fall_;
	int ret = Script::setData( value, fall_, "fall" );
	if (oldFall != fall_ && !ret) droppingTime_ = 0.f;
	return ret;
}
...
PY_BEGIN_ATTRIBUTES( Physics )
	...
	PY_ATTRIBUTE( tempJoystickDisabled )
	 ...
	 */
	PY_ATTRIBUTE( fall )
	/*~ attribute Physics.droppingTime
	 *
	 *	This read only attribute shows how long, in seconds, the entity has
	 *	been falling for.  The time starts from	the last time the entity was
	 *	actually touching the ground.
	 *
	 *	@type	Read-Only Float
	 */
	PY_ATTRIBUTE( droppingTime )
...
void Physics::hoverStyleTick( double timeNow, double timeLast )
{
	...
	pSlave_->filter().input( timeNow,
		spaceID, vehicleID, localPos, localAuxVolatile );
