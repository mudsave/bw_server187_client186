...
bool AlertManager::init()
{
	...
	for ( int i = 0; i < MAXIMUM_ALERT_ID; i++ )
	{
		...
		{
			alertIcons_[i] = new SimpleGUIComponent( iconTextures[i%iconTextures.size()] );
			calculatePosition( i );
...
{
	if ( Moo::rc().primitiveCount() > primitives_AlertLevel )
	{
...
void AlertManager::calculatePosition( int n )
{
	if (alertIcons_[n])
	{
		...
		alertIcons_[n]->heightInClipCoords( true );

		alertIcons_[n]->width( clipSize / aspectRatio );
		alertIcons_[n]->widthInClipCoords( true );
	}
}
