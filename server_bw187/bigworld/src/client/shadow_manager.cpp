...
void ShadowManager::init( DataSectionPtr pSection )
{
	...
	qualitySettings_ = 
		Moo::makeCallbackGraphicsSetting(
			"SHADOWS_QUALITY", *this, 
			&ShadowManager::setQualityOption,
			-1, false, false);
			
	qualitySettings_->addOption("HIGH",  supported);
	qualitySettings_->addOption("MEDIUM", supported);
	qualitySettings_->addOption("LOW", supported);
	qualitySettings_->addOption("OFF", true);
	...
	while (shadowCount > 0)
	{
		...
		castersSettings_->addOption(countStr.str(), supported || shadowCount == 0);
	}
...
void ShadowManager::initCasters(int shadowCount)
{
	...
	for (uint32 i=0; i<this->curShadowCount_; i++)
	{
		...
		if (pCaster->init(pShadowCommon_, (i>0) && halfRes_, pBaseTarget))
		{
...
void ShadowManager::tick( float dTime )
{
	...
	if (active_ && pCasters_.size() > 0)
	{
		...
		while (i < pEntitiesOnHold_.size())
		{
			...
			if (entity != NULL)
			{
				PyModel* pModel = entity->pPrimaryModel();
				if (pModel != NULL)
				{
		...
		while (i < pEntities_.size())
		{
			...
			if (entity != NULL)
			{
				...
				if (pModel)
				{
					...
					Matrix m = pModel->worldTransform();
					m.postMultiply( vp );
		...
		if (ChunkManager::instance().cameraSpace() )
		{
			...
			for (uint32 i = 0; (i < pCasters_.size()) && (i < pEntities_.size()); i++)
			{
				...
				if (entity != NULL)
				{
					...
					PyModelPtr pModel = entity->pPrimaryModel();
...
{
	MF_ASSERT(uint(index) < pEntities_.size());
...
{
	MF_ASSERT(uint(index) < pEntitiesOnHold_.size());
	return static_cast<Entity*>(
 ...
 *	this function adds an entity to the shadow manager
 *	@param Entity 
 */
 ...
 *	this function removes an entity from the shadow manager
 *	@param Entity 
 */
