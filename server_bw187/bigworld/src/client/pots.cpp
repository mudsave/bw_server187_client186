...
Pots::~Pots()
{
	...
	MainLoopTasks::root().del( this, "Pots" );
}
 ...
 *
 *	@param	centre	a PyMatrixProvider.  This is the centre of the sphere.
...
{
	MF_ASSERT( source.hasObject() );
	MF_ASSERT( callback );
	MF_ASSERT( PyCallable_Check( callback.getObject() ) );

	uint32 id = s_instance.nextHandle_++;
	s_instance.pots_[id] = new Pot(source,dist,callback);
	return id;
}
...
{
	PotMap::iterator it = s_instance.pots_.find( handle );

	if ( it != s_instance.pots_.end() )
	{
		Pot* ppot = it->second;
...
/**
 *	This method adds a camera-only-trap to the active list.
 *
 *	@param	source	the location, as given by a matrix provider
...
uint32 Pots::addMat( MatrixProviderPtr source, SmartPointer<PyObject> callback, MatrixProviderPtr target )
{
	...
	uint32 id = s_instance.nextHandle_++;
	s_instance.mats_[id] = new Pot(source,0.f,callback,target);
	return id;
}
...
{
	PotMap::iterator it = s_instance.mats_.find( handle );

	if ( it != s_instance.mats_.end() )
	{
		Pot* pmat = it->second;
