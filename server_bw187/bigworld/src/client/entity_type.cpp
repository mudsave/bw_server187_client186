...
//	memNow( "After ET loop" );
	...
	RemoteStepper::step( "EntityType::onLoad" );
...
/**
 *	This method is called whenever the script is loaded or reloaded.
 */
void EntityType::onLoad()
{
	PyObject * pLoadString = PyObject_GetAttrString(
		PyImport_AddModule( "keys" ), "loadString" );
	...
	if (!PyString_Check( pLoadString ))
	{
		Py_DECREF( pLoadString );
		return;
	}

	PyRun_SimpleString( PyString_AsString( pLoadString ) );
...
bool EntityType::reload()
{
	...
	for (uint i = 0; i < s_types_.size(); i++)
	{
		...
		MF_ASSERT( s_types_[i] != NULL && s_types_[i]->pModule_ != NULL );
...
void EntityType::swizzleClass( PyTypeObject *& pOldClass )
{
	...
	PyObject * pObjNewClass = PyObject_GetAttrString( pModule_, const_cast< char * >(className) );
 ...
 */
Entity * EntityType::newEntity( ObjectID id, Vector3 & pos,
	...
	StreamContents contents, Entity * pSister )
{
	...
	for (uint i=0; i < description_.propertyCount(); i++)
	{
		...
		if (!pDD->isClientOnlyData() && !pDD->isOtherClientData())
		{
			// if we're not interested in it, don't set it.
			continue;
		}
