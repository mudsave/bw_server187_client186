...
PY_BEGIN_ATTRIBUTES( ActionMatcher )
	 ...
	 *	entity in degrees per second.  Defaults to 360.
	 *
	 *	@type	Float
	 */
	 ...
	 *	unitRotation in degrees per second.  Defaults to 540.
	 *
	 *	@type	Float
	 */
...
int ActionMatcher::pySet_matcherCoupled( PyObject * value )
{
	...
	if (isPlayer && ret == 0)
	{
		...
		if (matcherCoupled_ && !oldMC && pOwner_ != NULL && pEntity_.good())
		{
...
void ActionMatcher::rev( float dTime )
{
	...
	// dwMatchStart3.start();
	Matrix world = model.worldTransform();
	// dwMatchStart3.stop();
	...
	{
		ObjectID currVehicleID = pEntity->pVehicle()->id();
...
#if 0
	// if this is a bot, it's always walking - optimisation shortcut
	if (!matchBots_ && pEntity->id() > 1000 && pEntity->id() < 20000)
	{
		SMActionVector::iterator	walk = &pActions_[0];

		float inTime = (newEntityPos - oldModelPos).length() /
			( (*walk)->pFirstAnim->flagFactorBit( 0 ).applyToOrigin().length() /
				(*walk)->pFirstAnim->duration_ );

		model.actionQueue().setMatch( *walk, dTime, inTime );

		world.preRotateY( newEntityYaw - oldModelYaw );
		world.translation( newEntityPos );

		// check this because setting the world transform is expensive
		if (world != model.worldTransform())
		{
	...
	float rLen = deltaRot.length() * blendingSpeedFactorEstimate;
	...
	{
		const Model::Action::MatchInfo & mi = (*i)->pSource->matchInfo_;
		...
		if (mi.oneShot)
		{
			float	matchCycleCount = model.actionQueue().matchCycleCount() +
				(*i)->pSource->blendOutTime_ / (*i)->pFirstAnim->duration_;
	...
	{
		if ((*i)->pSource->matchInfo_.scalePlaybackSpeed)
		{
			const Model::Animation * pAnim = (*i)->pFirstAnim;
		...
		model.actionQueue().setMatch( *i, dTime, inTime,
			(*i)->pSource->matchInfo_.promoteMotion );
...
//			dprintf( "Match not accepted\n" );
		...
		else
		{
			...
			fallSelected =
				(*i)->pSource->matchInfo_.trigger.minEntitySpeed < 0.f;
		}
	}
	...
	{
		//MF_ASSERT(PyCallable_Check(startMovingNotifier_));
		...
		if (i != pActions_.end())
		{
			Script::call(
				...
				Py_BuildValue( "(s)", (*i)->pSource->name_.c_str() ),
...
*/
	...
	if (ActionMatcher::debug_)
	{
		...
		{
			amDebug << (*i)->pSource->name_ << " @ " << inTime;
		}
		else
		{
			amDebug << "<Overridden>";
...
PyObject * ActionMatcher::pyGet_lastMatch()
{
	...
	return Script::getData( (*lastAction_)->pSource->name_ );
}
