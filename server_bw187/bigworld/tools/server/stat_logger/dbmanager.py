...
class DbManager:
	...
	def createLogDb( self, name, prefTree, sampleTickInterval ):
		...
		if name == None:
			name = createdb.generateLogDbName( cursor )
	...
	def _connectToServer( self ):
		...
		if self._dbConn != None:
			try:
				...
				log.warning( "MySQL connection timed out. Performing reconnect." )
		try:
			...
			if e[0] == 1045:
				raise LogDbError("Incorrect password for database user \"%s\"" \
					% self.dbUser, LogDbError.ERR_BADPASSWORD)
			else:
	...
	def _createConversionDict( self ):
		...
		try:
			newDecimal = MySQLdb.FIELD_TYPE.NEWDECIMAL
		except AttributeError:
			# NEWDECIMAL not defined in MySQL-python 1.2.0 and below,
			# but using this constant should be fairly safe
			newDecimal = 246
		conv[ newDecimal ] = float
		return conv
