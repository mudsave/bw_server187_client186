...
)ENGINE=InnoDB ;""" % (constants.tblInfo) )
...
) ENGINE=InnoDB ;""" % (constants.tblStdWindows) )
...
) ENGINE=InnoDB ;""" % (constants.tblPrefProcesses) )
...
) ENGINE=InnoDB;""" % (constants.tblPrefStatistics, constants.tblPrefProcesses) )
...
) ENGINE=InnoDB ;""" % (constants.tblStdSessionTimes) )
...
) ENGINE=InnoDB ;""" % (constants.tblStdInfo) )
...
(
	...
	INDEX(time, id)
) ENGINE=InnoDB;""" % (constants.tblStdTickTimes) )

	c.execute("""\
CREATE TABLE %s
(
	touchTime INT UNSIGNED NOT NULL
) ENGINE=InnoDB;""" % (constants.tblStdTouchTime) )
...
)ENGINE=InnoDB;""" % (constants.tblSeenMachines) )
...
(
	...
	name VARCHAR(255)
)ENGINE=InnoDB;""" % (constants.tblSeenUsers) )
...
(
	...
	-- FOREIGN KEY (processPref) REFERENCES %s(id)
)ENGINE=InnoDB;""" % (constants.tblSeenProcesses, constants.tblSeenMachines, \
	...
	for windowPref in prefTree.windowPrefs:
		c.execute("""\
			...
			(
				...
				-- Columns go here
				%s
			)ENGINE=InnoDB;""" % (constants.tblStatMachines, windowPref.dbId,
...
def generateLogDbName( cursor ):
	...
	c.execute( "SELECT name FROM %s WHERE name REGEXP '%s[[:digit:]]+'" \
			% (constants.tblInfo, constants.logDbNameRoot) )
	results = c.fetchall()

	# Compile the regexp, we'll be using it a bit now...
	regexp = re.compile( '%s(\d+)' % (constants.logDbNameRoot) )
	...
	# server
	c.execute("SHOW DATABASES LIKE '%s%%'" % (constants.logDbNameRoot) )
	results = c.fetchall()
	...
	return "%s%d" % (constants.logDbNameRoot, appendNum)
