...
def main( args ):
	...
	manager = dbmanager.DbManager( options.dbHost,
		options.dbPort, options.dbUser, options.dbPass )

	# Write it again (so the format is always nice
	#print "Rewriting preferences to file"
	#prefxml.writePrefsToXMLFile( prefFilename, options, prefTree )
	...
	gatherer = gather.Gather( options, args, prefTree, manager )
	...
	if args.useWinpdb:
		try:
			import rpdb2
			log.info("Waiting for debugger to attach (2 mins)")
			rpdb2.start_embedded_debugger( "bw", True, True, 120 )
		except ImportError:
			pass
	...
	finishEvent = gatherer.finishUp()

	# Wait for the Gather thread to finish up writing log
	log.info( "Waiting for Gather thread to complete logging "\
			"(can take up to %.1f seconds)", options.sampleTickInterval)

	# If there's an error which freezes, time out after 5 seconds
	finishEvent.wait()
...
def printLogList( manager, options ):
	...
	#print "Printing list of log databases..."
	#print "+=%s=+=%s=+=%s=+=%s=+" % \
	#	("="*12, "="*22, "="*22, "=" * 6)
	#print "| %-12s | %-22s | %-22s | %-6s |" % \
	#	("Name", "Created", "Used", "Active")
	#print "+=%s=+=%s=+=%s=+=%s=+" % \
	#	("="*12, "="*22, "="*22, "=" * 6)
	#for name, created, used, active in results:
	#	print "| %-12s | %-22s | %-22s | %-6s |" % (name,
	#		utilities.formatTime(created),
	#		utilities.formatTime(used),
	#		boolToChar(active) )
	#print "+=%s=+=%s=+=%s=+=%s=+=%s=+" % \
	#	("="*12, "="*22, "="*22, "="*6, "=" * 6)
...
def readArgs():
	...
	parser.add_option( "--winpdb",
			default = False, action = "store_true", dest = "useWinpdb",
			help = "Allow a Winpdb client to attach upon starting up." \
