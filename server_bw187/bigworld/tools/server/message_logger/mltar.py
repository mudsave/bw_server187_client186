...
The --all option is a convenience option that is the same as passing --all-users
and --all-segments (i.e. please archive everything).
...
def archive( mlog, uid, fname, age = None, move = False, compression = "gzip" ):
	"""
	...
	# Macro for reading filename lists (active_files and dirty_files)
	def readList( name, missingOK ):
		try:
			return set( [line.rstrip() for line in
						 open( "%s/%s" % (mlog.root, name) ).readlines()] )
		except IOError, e:
			if missingOK:
				return set()
			else:
	...
	dirty = readList( "dirty_files", True )

	for i in xrange( 5 ):
		active = readList( "active_files", False )
		if active is None and os.path.exists( "%s/pid" % mlog.root ):
	...
	if active is None:
		if os.path.exists( "%s/pid" % mlog.root ):
			log.error( "Logs seem to be taking a long time to roll, bailing out" )
			return False
		else:
			active = set()
	...
	for fname in files:
		...
		# Ignore any file that is newer than the "active" files
		username = os.path.dirname( fname )
		if SEG_PATT.search( fname ) and \
			   cutoffs.has_key( username ) and \
			   timestamp( fname ) >= cutoffs[ username ]:
			log.verbose( "Skipping %s (active)", fname )
			continue
...
def extract( mlog, archive, outdir ):
	...
	for fname in filter( SEG_PATT.search, tar.getnames() ):
