...
class Process( Exposed ):
	...
	SERVER_PROCS = ["cellapp", "baseapp", "cellappmgr", "baseappmgr",
					"loginapp", "dbmgr"]

	WORLD_PROCS = ["cellappmgr", "baseappmgr", "loginapp", "dbmgr"]
	NON_WORLD_PROCS = ["cellapp", "baseapp", "bots"]
	...
	def __cmp__( self, other ):
		...
		# Macro which returns True if the process is a world process
		isWorld = lambda p: p.name in self.WORLD_PROCS
		...
		selfW = isWorld( self ); otherW = isWorld( other )

		if selfW and otherW:
			return cmp( index( self.WORLD_PROCS, self.name ),
						index( self.WORLD_PROCS, other.name ) )

		elif not selfW and not otherW:
			return cmp( index( self.NON_WORLD_PROCS, self.name ),
						index( self.NON_WORLD_PROCS, other.name ) ) \
	...
	def setWatcherValue( self, path, value ):
		...
		wdm.message = wdm.WATCHER_MSG_SET
		wdm.count = 0
		...
		while True:
			...
			wdm = messages.WatcherDataMessage().set( data )
...
class CellAppProcess( StoppableProcess ):

	def __init__( self, machine, mgm ):
		StoppableProcess.__init__( self, machine, mgm )


class BaseAppProcess( StoppableProcess ):
...
class MessageLoggerProcess( Process ):
	...
	def sendEvent( self, message, user, severity = "INFO" ):
		"""
...
class WatcherData( Exposed ):
	...
	def __init__( self, process, path, value=None ):
		Exposed.__init__( self )
	...
	def refresh( self ):
		...
		newvalue = None
		for path, value in messages.WatcherDataMessage.query(
...
class Machine( Exposed ):
	...
	def str( self, indent ):
		...
		if VERBOSE:
			fmt = "%s%s (%s) running %d process%s at %s of %4dMHz"
		else:
			fmt = "%s%-10s %-15s running %2d process%s at %s of %4dMHz"
		...
		s = fmt % ( "\t"*indent, self.name, self.ip,
					nprocs, suffix,
					",".join( ["%2d%%" % int( l*100 ) for l in self.loads] ),
					self.mhz )

		# CPU multiplier if appropriate
		if self.ncpus > 1:
			s += " x %d" % self.ncpus
		else:
			s += "    "
...
class User( Exposed ):
	...
	def __init__( self, uid, cluster, machine = None,
		...
		if not isinstance( uid, messages.UserMessage ):
			...
			while True:
				...
				if not replies:
					log.error( "%s didn't reply to UserMessage, blacklisting",
							   machine.name )
					blacklist.add( machine )

				elif replies[0].uid == mgm.UID_NOT_FOUND:
					log.verbose( "%s couldn't resolve user %s, "
								 "will not ask again",
								 machine.name, uid )
	...
	def _getLoads( self ):
		...
		def minmaxload( f, name ):
			return f( map( lambda p: p.machine.load(), self.getProcs( name ) ) )

		# Average of reported machine CPU loads
		def avgload( name ):
			loads = map( lambda p: p.machine.load(), self.getProcs( name ) )
		...
		if self.getProcs( "bots" ):
			self.load[ "bots" ] = LoadStats( minmaxload( min, "bots" ),
											avgload( "bots" ),
											minmaxload( max, "bots" ) )
	...
	def getLayoutStatus( self, layout, _async_ = None ):
		...
		for mname, pname, pid in layout:
			...
			p = m.getProc( pid )
			if p:
				status.append( (mname, pname, pid, "registered") )
			else:
		...
		for mname, pname, pid, state in status:
	...
	def lsMiscProcesses( self ):
		...
		miscps = [ p for p in self.getProcs() if \
				   Process.clean( p.name ) in Process.WORLD_PROCS ]
	...
	def getBestAddCandidate( self ):
		"""
		Returns the bots process with the lowest machine CPU load below the
		thresh, or None if none such exists.
		"""
		...
		if bps:
			...
			# If the lowest loaded bots machine is below the CPU thresh, then
			# we're OK to add bots to it.
			if bps[0].canAddBots():
				return bps[0]
	...
	def getBestDelCandidate( self ):
		...
		# Delete from the processes with the highest load first
		bps = filter( lambda p: p.nbots() > 0, self.getProcs( "bots" ) )
		bps.sort( key = lambda p: -p.load )
	...
	def lsBots( self ):
		...
		bps = self.getProcs( "bots" )
		active, inactive = util.partition( lambda bp: bp.nbots() > 0, bps )

		for bp in sorted( active, key = lambda p: p.machine.name ):
			log.info( "%d bots running at %2d%% on %s",
					  bp.nbots(), bp.load * 100, bp.machine.name )

		if active: log.info( "" )

		if inactive:
			log.info( "%d bots processes without any bots\n", len( inactive ) )
	...
	def addBots( self, numtoop ):
		...
		while numtoop > 0:
			...
			try:
				...
				# If no machines are even candidates (before load is taken into
				# account) we're not going to be able to add bots
				if not self.getProcs( "bots" ):
					log.error( "No bots processes, bailing out!" )
					return False

				targbp = self.getBestAddCandidate()
				targnum = min( numtoop, NBOTS_AT_ONCE )

				# All candidate machines are overloaded
				if targbp is None:

					log.info( "\tbot machines overloaded; " +
				...
				# Add bots
				targbp.addBots( targnum )
				numtoop -= targnum
				log.info( "\tadded %d to %s:%d (%d done in %.1fs)",
						  targnum, targbp.machine.name, targbp.port(),
						  numdesired - numtoop,
						  time.time() - starttime )
		...
		numdone = numdesired - numtoop
		log.info( "Added %d bots in %.1fs" %
				  (numdone, time.time() - starttime) )
	...
	def delBots( self, numtoop ):
		...
		while numtoop > 0:
			...
			# The machine and process we're deleting from
	...
	def setWatchersOnAllBots( self, *values ):
		...
		watcherMessage.message = watcherMessage.WATCHER_MSG_SET
...
class Cluster( Exposed ):
	...
	def refresh_( self, clearUsers = False ):
		...
		# Find all the machines in this cluster
		...
		# Run the MGM query
		replies = messages.MachineGuardMessage.batchQuery( [wmm, psm] )
