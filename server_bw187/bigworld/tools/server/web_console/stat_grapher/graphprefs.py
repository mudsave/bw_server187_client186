...
class PrefRequester:
	def __init__( self, backend ):
		...
		self.userDisplayPrefs = {}       # {logDb,uid -> displayPrefDict}
	...
	# -------------------------------------------------------------------------
	def retrieveDisplayPrefs( self, logDb, uid ):
		"""
		...
		userPrefs = self.retrieveUserDisplayPrefs( logDb, uid )
	...
	def retrieveUserDisplayPrefs( self, logDb, uid ):
		"""
		...
		try:
			...
			newPref = UserDisplayPref( logDb, uid )
			self.userDisplayPrefs[ (logDb, uid) ] = newPref
			return newPref

	def setProcessStatColour( self, logDb, uid, procType, prefId, newColour ):
		"""
	...
	def setMachineStatColour( self, logDb, uid, prefId, newColour ):
		"""
		Saves a colour change to the database.
	...
	def saveEnabledProcStatOrder( self, logDb, uid, procType, statList ):
		"""
	...
	def saveEnabledMachineStatOrder( self, logDb, uid, statList ):
		"""
		...
		"""
		utils.logArgs( apiLog.debug )
		userPrefs = self.getUserDisplayPrefs( logDb, uid )
		userDict = userPrefs.dict()
