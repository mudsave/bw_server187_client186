...
<div py:def="moduleContent()">
		...
		<tr style="vertical-align: top">
			...
			<td>
				<input type="text" size="16" name="message"
					   title="Pattern to match in log message, or blank for all messages"/>
	...
	<script type="text/javascript">
		// This is the basic time offset, without taking into account timezone
		// When this is passed through to the ServerTime class we will take
		// the timezone into account
		var serverTimeOffset = (${now} - (Date.now() / 1000.0));

		// Calculate the timezone difference now
		var timezoneOffset = ${serverTimezone} - ((new Date()).getTimezoneOffset() * 60);
	</script>
