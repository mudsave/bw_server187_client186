...
case "$1" in
  ...
  install)
	...
	if [ -x /usr/sbin/update-rc.d ]; then
		/usr/sbin/update-rc.d $SCRIPT_NAME defaults 99
		...
		ln -s /etc/init.d/$SCRIPT_NAME /etc/rc3.d/S99$SCRIPT_NAME
		ln -s /etc/init.d/$SCRIPT_NAME /etc/rc4.d/S99$SCRIPT_NAME
		ln -s /etc/init.d/$SCRIPT_NAME /etc/rc5.d/S99$SCRIPT_NAME
		ln -s /etc/init.d/$SCRIPT_NAME /etc/rc6.d/K99$SCRIPT_NAME
	fi
  ...
  remove)
	...
	if [ -x /usr/sbin/update-rc.d ]; then
		...
		rm -f /etc/rc[1-6].d/[SK]99$SCRIPT_NAME 
	fi
