...
bwsetup.addPath( "../pycommon/redist/mysql_python" )
import MySQLdb
...
ML_ARCHIVES = "%s/message_logs.tar.bz2" % LOG_DIR
...
def softMkDir( pwent, path, parents = False, asbw = False ):

	# If it exists, make sure we own it
	try:
		...
		if statinfo.st_uid != pwent.pw_uid:
			...
			log.warning( "%s already exists, not making", path )
...
def softSymlink( path, tgt ):
	...
	if os.path.islink( path ):
		...
		try:
			...
			log.error( "Couldn't symlink %s -> %s: %s", tgt, path, e )
			sys.exit( 1 )
...
def createChownDir( dir, username, groupname ):
	...
	if not os.path.isdir( dir ):
		print "\n+ Creating %s" % dir
		softMkDir( pwent, dir, parents=True )
	else:
		print
...
def install():
	...
	if os.system( "python -c 'import turbogears' 1> /dev/null 2>&1" ):
		print "Failed to import TurboGears module"
		print "Please install TurboGears and re-run this installation script."
		print "Refer to the server tools installation guide for details."
		sys.exit( 1 )
	...
	createChownDir( LOG_OUTPUT_DIR, username, groupname )
	createChownDir( piddir, username, groupname )
	createChownDir( logdir, username, groupname )
