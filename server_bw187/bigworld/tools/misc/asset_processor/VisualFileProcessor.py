	...
	def processVisualMaterial(self,material,visualEntry):
		...
		if sect != None:
			...
			if sect != None:
				...
				#third it is a direct material section in the visual
				fxs = extractSectionsByName(material,"fx")
				for fx in fxs: visualEntry.addDependency( fx.asString )
				if len(fxs) > 1: visualEntry.flagHasMoreThanOneFX(fxs)
