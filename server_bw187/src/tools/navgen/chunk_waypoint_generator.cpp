 ...
 *	Constructor
 */
ChunkWaypointGenerator::ChunkWaypointGenerator( Chunk * pChunk, bool overwrite, 
...
void ChunkWaypointGenerator::outputDirtyFlag(bool dirty /*= false*/)
{
    ...
    if (chunkBinSection)
    {
        ...
        if (navmeshDirtySection)
        {
            navmeshDirtySection->setBinary(new BinaryBlock(&dirty, sizeof(dirty)));
	...
	if (!chunk->online())
	{
		if (!chunk->loading())
		{
			ChunkManager::instance().loadChunkExplicitly(
				chunk->identifier(), chunk->mapping() );
