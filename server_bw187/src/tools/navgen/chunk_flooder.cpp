...
#include "waypoint/adjacent_chunk_set.hpp"
...
void ChunkFlooder::getSeedPoints( std::vector<Vector3> & pts, PhysicsHandler& ph )
{
	...
	if (pChunk_->isOutsideChunk())
	{
		float ny = Moo::TerrainBlock::getHeight( seedPt.x, seedPt.z );
		if (ny != Moo::TerrainBlock::NO_TERRAIN) seedPt.y = ny+1.f;
	}
	pts.push_back( seedPt );
    ...
    bool writeTGAs )
{
	...
	// instead of using an adjacent chunk list, now use portal list in
	// out pChunk.
	// pWF_->setAdjacentChunks(&acs);
...
bool ChunkFlooder::flashFlood( const Vector3 & seedPt )
{
	...
	if ( pCol->nHoldings() == 2 )
	{
		...
		Moo::TerrainBlockPtr pBlock = pTerrain->block();
		if ( !pBlock )
			return false;

		const std::vector<float> & heightMap = pBlock->heightMap();
		MF_ASSERT( !heightMap.empty() );

		// make sure there are either no holes or all holes
		if ( !pBlock->allHoles() && !pBlock->noHoles() )
			return false;

		// and make sure it is all the same height if not all holes
		if (!pBlock->allHoles())
		{
			wpHeight = heightMap[0];
			for ( uint i = 1; i < heightMap.size(); ++i )
			{
				if (heightMap[i] != wpHeight)
					return false;
			}
		}
	}
