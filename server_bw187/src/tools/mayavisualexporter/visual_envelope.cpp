	...
	bool useIdentifier )
{
	...
	if (spFile)
	{
		...
		if( object.isNull() == false )
			spFile->writeBinary( identifier_.substr( identifier_.find_last_of( "|" ) + 1 ) + ".mvertices", f.output() );
		else
			spFile->writeBinary( identifier_.substr( identifier_.find_last_of( "|" ) + 1 ) + ".vertices", f.output() );		
		...
		if (spRenderSet)
		{
			...
			for( int i = 0; i < (int)boneNodes_.size(); i++ )
			{
				...
				if (spNode)
				{
					spNode->setString( (boneNodes_[i].substr( boneNodes_[i].find_last_of( "|" ) + 1 ) + "_Blend_Bone") );
				}
			}
			...
			{
				if( object.isNull() == false )
					spGeometry->writeString( "vertices", identifier_.substr( identifier_.find_last_of( "|" ) + 1 ) + ".mvertices" );
				else
					spGeometry->writeString( "vertices", identifier_.substr( identifier_.find_last_of( "|" ) + 1 ) + ".vertices" );
				...
				for (uint32 i = 0; i < materials_.size(); i++)
				{
					...
					if (mat.inUse) mat.save( spGeometry, spExistingVisual, primGroup++, true, (int)boneNodes_.size(),
						hasBlended_ ? Material::SOFT_SKIN :
						Material::HARD_SKIN,
							boneNodes_.size() <= 17  );
				}
			}
