...
public:
	...
	bool load( const std::string & resourceID )
	{
		...
		if (pData_)
		{
			...
			{
				ERROR_MSG( "Supported index format %s\n", format.c_str() );
			}
		}
		else
		{
		...
		const PrimitiveGroups & primitiveGroups )
{
	...
	if (primitivesType == Primitive::PT_TRIANGLE_LIST)
	{
		...
		while (iter != primitiveGroups.end())
		{
			...
			// in the Maya file somehow that an area of a model never add
			// itself to the BSP.
...
/**
 *	This method generates a bsp from a visual.
 */
		...
		std::vector<std::string>& materialIDs )
{
	...
	int materialKind = pRoot->readInt( "materialKind" );
