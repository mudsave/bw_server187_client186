 ...
 */
bool VisualMesh::createPrimitiveGroups( PGVector& primGroups, IndexVector& indices )
{
	...
	bool useIdentifier )
{
	...
	if (!spFile)
		spFile = new BinSection( primitiveFile, new BinaryBlock( 0, 0 ) );
	...
	if (spFile)
	{
		...
		//spFile->updateBinary( idPrefix + "indices", f.output() );
...
bool VisualMesh::savePrimXml(	const std::string& xmlFile )
{
	...
	intIndices.resize( indices.size() );
	std::copy(indices.begin(), indices.end(), intIndices.begin());
	DataSectionPtr pIndices = spFile->newSection( "Indices" );
	pIndices->writeInts( "Indices", intIndices );
	...
	else if ( ExportSettings::instance().bumpMapped() )
	{
		...
		for (tbvvIt = vertices.begin(); tbvvIt != vertices.end(); ++tbvvIt)
		{
			...
			pVertex->writeInt( "Tangent", tbvvIt->tangent );
			pVertex->writeInt( "Binormal", tbvvIt->binormal );
		}
	}
	else
	{
	...
	{		
		...
		for (int i=0; i< shapeCount; i++)
		{
			...
			for (uint j = 0; j < blendShapes.numTargets( 0 ); j++)
			{
				...
				// Only a single base is supported at present, so zero is passed for base
				pMT->writeString( "MorphTargetName", blendShapes.target( 0, j ).name );
...
{
	bool ret = false;
	if( mesh.positions().size() > 0 && mesh.normals().size() > 0 && mesh.uvs().size() > 0 && mesh.faces().size() > 0 )
	{
		...
		for (int i = 0; i < (int)mesh.faces().size(); i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				if (hasUVs)
				{
					...
					bv.uv = reinterpret_cast<Point2&>( mesh.uvs()[ f.uvIndex[j] ] ); 
					bv.uv.v = 1 - bv.uv.v;
				}
				bv.meshIndex = 0;
	...
	else
	{
		if( mesh.positions().size() > 0 && mesh.normals().size() > 0 && mesh.faces().size() > 0 )
		{
			...
			::MessageBox( GetForegroundWindow(), msg, "Visual Exporter Error", MB_OK );
		}
	}
