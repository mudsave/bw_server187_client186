...
MLL_EXPORT MStatus initializePlugin( MObject obj )
{
	...
	MFnPlugin plugin( obj, "Micro Forte", "1.8.7", "Any");
	...
	{
		MessageBox( GetForegroundWindow(), "Cannot find resources.xml!", "Visual Exporter Error",
			MB_OK | MB_ICONERROR );
		return MS::kFailure;
	}

	ExportSettings::instance().setSettingFileName( exporterPath + "\\exporter.xml" );
