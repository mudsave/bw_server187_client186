...
void invalidExportDlg( const std::string& fileName )
{
	...
	MessageBox( NULL, errorString.c_str(), "Visual/Animation Exporter Error", MB_OK | MB_ICONWARNING );
}
	...
	if (pVisualSection)
	{
			...
			ExportSettings::instance().exportMode() != ExportSettings::STATIC && ExportSettings::instance().exportMode() != ExportSettings::MESH_PARTICLES )
		{
			...
			for( uint32 j = 0; j < skin.count(); ++j )
			{
				...
				{
						hierarchy.find( std::string( skin.boneDAG( i ).fullPathName().asChar() ) ).addNode( std::string( skin.boneDAG( i ).fullPathName().asChar() ) + "_Blend_Bone", matrix4<float>()/*skin.transform( i, false ).inverse()*/ );
				}
			}
		}
//		hierarchy.dump();
...
/*		{
	...
	::CopyFile( tempFileName.c_str(), (fileName + ".original").c_str(), false );
#endif
...
MStatus VisualFileTranslator::writer( const MFileObject& file, const MString& optionsString, FileAccessMode mode)
{
	...
	VisualChecker vc( ( output_filename + ".visual" ).asChar(), false );
	...
	// There is a 85 bone limit for each skinned object.  For each object that
	// has more than 85 bones, display a warning stating the objects name and
		...
		if (it->second > 85)
		{
			...
			bw_snprintf(
				...
				"There are %d bones in %s.  "
					"This exceeds the 85 bone limit!\n"
	...
	if (settings.exportAnim())
	{
		...
		if ((name2.length() == name.length()) || (name2.find_last_of("/\\") == std::string::npos))
		{
			...
			MessageBox( NULL, errorString.c_str(), "Animation Exporter Error", MB_OK | MB_ICONWARNING );
			visualMeshes.clear();
			hullMeshes.clear();
			bspMeshes.clear();
			visualPortals.clear();
...

		{
			...
			FILE* file2 = fopen( ( BWResource::removeExtension( name ) + ".animation").c_str(), "wb" );
			...
			if (file2)
			{
				...
				if (ExportSettings::instance().referenceNode())
				{
					...
					OPENFILENAME openFileName = {
						...
						GetForegroundWindow(),
						0,
						"Visual Files\0*.visual\0All Files\0*.*\0\0",
						NULL,
						0,
						0,
						fileName,
					...
					{
						this->loadReferenceNodes( fileName );
				...
				{
					visualMeshes[i]->exportMorphAnims( animation );

#if DEBUG_ANIMATION_FILE
					visualMeshes[i]->exportMorphAnimsXml( animDebugXmlFileName, animName );
#endif
				}
