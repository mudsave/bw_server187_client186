...
{
	static Moo::Visual::DrawOverride* override = NULL;
	if( override == NULL )
	{
		override = new MaterialDrawOverride( s_selectionFxPrefix, true );
	}
	return override;
}
// -----------------------------------------------------------------------------
...
void ChunkItemFrustumLocator::enterSelectionMode()
{
	...
	view.lookAt( Moo::rc().invView().applyToOrigin(),
		BigBang::instance().getWorldRay( ( minx + maxx ) / 2, ( miny + maxy ) / 2 ),
	...
	BigBang::instance().drawSelection( true );
}

void ChunkItemFrustumLocator::leaveSelectionMode()
{
	BigBang::instance().drawSelection( false );
...
void ChunkItemFrustumLocator::calculatePosition( const Vector3& worldRay, Tool& tool )
{
	...
	if (enabled_)
	{
		...
		{
			startPosition_ = BigBang::instance().currentCursorPosition();
		}
		...
		BigBang::instance().renderTerrain( 0 );
		...
		if( SUCCEEDED( texture->GetSurfaceLevel( 0, &rtSurface ) ) &&
			...
			SUCCEEDED( Moo::rc().device()->CreateTexture( desc.Width, desc.Height, 0, 0, desc.Format, D3DPOOL_SYSTEMMEM,
				&inmemTexture, NULL ) ) &&
