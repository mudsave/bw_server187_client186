...
#include "station_node_link_locator.hpp"
 ...
 *  @returns                True if the locator has found station node.
 */
/*virtual*/ bool EntityLinkProxy::canLinkAtPos(ToolLocatorPtr locator_) const
{
    StationNodeLinkLocator *locator =
        (StationNodeLinkLocator *)locator_.getObject();
	return locator->chunkItem() != NULL && EditorChunkCache::instance( *entity_->chunk() ).edIsWriteable();
}
...
/*virtual*/ void EntityLinkProxy::createLinkAtPos(ToolLocatorPtr locator_)
{
    ...
    const std::vector<ChunkItemPtr> &selection = 
        BigBang::instance().selectedItems();
    ...
    if (numLinked == 1)
        UndoRedo::instance().barrier("Link entity to graph", false);
    else
        UndoRedo::instance().barrier("Link entities to graph", false);
}
