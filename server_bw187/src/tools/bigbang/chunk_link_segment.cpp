...
namespace
{
    const float SELECTION_EXPANSION = 5.0f;
 ...
 *  @param vertexBuffer The vertex buffer to store vertices in.
 *  @param indexBuffer  The index buffer to store indices in.
    ...
    indexBuffer_(indexBuffer)
{
    ...
    void *vertexDataRaw = NULL;
    void *indexDataRaw  = NULL;
    ...
    hr =
        indexBuffer->Lock
        (
            indexBase*sizeof(uint16),
            numberIndices()*sizeof(uint16),
            &indexDataRaw,
            0
        );
    ASSERT(SUCCEEDED(hr));
    ...
    *inds++ = vertexBase + 3;
    ...
    vertexBase += 4;
    vert1.normal_.set(0, 0, 1);
    vert1.pos_.set(v1.x, v1.y, v2.z);
    vert1.uv_ .set(startU_, 0.0f);
    vert1.uv2_.set(startU_, 0.0f);
    addVertex(verts++, vert1);
    vert2.normal_.set(0, 0, 1);
    vert2.pos_.set(v2.x, v1.y, v2.z);
    vert2.uv_ .set(startU_, 1.0f);
    vert2.uv2_.set(startU_, 1.0f);
    addVertex(verts++, vert2);
    vert3.normal_.set(0, 0, 1);
    ...
    *inds++ = vertexBase + 2;
    ...
    addVertex(verts++, vert2);
    vert3.normal_.set(0, 1, 0);
    vert3.pos_.set(v1.x, v1.y, v2.z);
    vert3.uv_ .set(endU_, 0.0f);
    vert3.uv2_.set(endU_, 0.0f);
    addVertex(verts++, vert3);
    vert4.normal_.set(0, 1, 0);
    vert4.pos_.set(v2.x, v1.y, v2.z);
    vert4.uv_ .set(endU_, 1.0f);
    vert4.uv2_.set(endU_, 1.0f);
    addVertex(verts++, vert4);
    *inds++ = vertexBase;
...
)
{
    ...
    effectMaterial->begin();
    effectMaterial->beginPass(0);   
}
...
)
{
    ...
    ( 
        0,
        vertexBuffer.pComObject(),
        0, 
        sizeof(VertexType)
    );
    rc.setFVF(VertexType::fvf());
    Moo::rc().setIndices(indexBuffer);
    ...
    rc.addToPrimitiveGroupCount();
	rc.addToPrimitiveCount(number*numberTriangles());
}
