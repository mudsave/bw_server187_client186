...
static PageTerrainHeight* lastInstance;
...
BOOL PageTerrainHeight::OnInitDialog()
{
	...
	strengthEdit_.SetIntegerValue( (int)strengthSlider_.getValue() );
...
void PageTerrainHeight::OnSetFocus( CWnd* pOldWnd )
{
	//if ( BigBang2App::instance().panelsReady() )
	//{
	//	BigBang::instance().updateUIToolMode( PageTerrainHeight::contentID );
	//	OnActivateTool( 0, 0 );
	//}
}
...
LRESULT PageTerrainHeight::OnActivateTool(WPARAM wParam, LPARAM lParam)
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->onPageControlTabSelect("pgcTerrain", "TerrainBrushes");
...
afx_msg LRESULT PageTerrainHeight::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
	...
	falloffFlat_.SetCheck( Options::getOptionInt( "terrain/height/brushFalloff" ) == 0 );
	falloffLinear_.SetCheck( Options::getOptionInt( "terrain/height/brushFalloff" ) == 1 );
	falloffCurve_.SetCheck( Options::getOptionInt( "terrain/height/brushFalloff" ) == 2 );
    ...
    if (PanelManager::instance()->isCurrentTool( PageTerrainHeight::contentID ))
    {
		    ...
		    ToolManager::instance().tool() && ToolManager::instance().tool()->locator() )
	    {
		    ...
		    BigBang::instance().setCursor( cursor );
...
{
	Options::setOptionInt( "terrain/height/brushFalloff", 1 );
}

void PageTerrainHeight::OnBnClickedTerrainFalloffCurve()
{
	Options::setOptionInt( "terrain/height/brushFalloff", 2 );
}

void PageTerrainHeight::OnBnClickedTerrainFalloffFlat()
{
	Options::setOptionInt( "terrain/height/brushFalloff", 0 );
}
