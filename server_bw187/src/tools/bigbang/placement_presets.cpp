...
void PlacementPresets::readPresets()
{
		...
		i != presetsComboNames.end(); ++i )
	{
		...
		if ( (*i) != dialogCombo )
		{
			item = new GUI::Item( "CHILD",
				PLACECTRL_EDIT_STR,
				section_->readString( "editPresetsString", "(Edit Presets...)" ),
		...
		item = new GUI::Item( "CHILD",
			...
			section_->readString( "normalPlacementString", "(Normal Placement)" ),
		...
		c != comboBoxes_.end(); ++c )
	{
		...
		// add default non-random placement item
		int idx = (*c)->AddString( section_->readString( "editPresetsString", "(Edit Presets...)" ).c_str() );
		(*c)->SetItemData( idx, PLACECTRL_EDIT_INT );
		idx = (*c)->AddString( section_->readString( "normalPlacementString", "(Normal Placement)" ).c_str() );
...
bool PlacementPresets::showDialog( GUI::ItemPtr item )
{
	...
	dlg.DoModal();
	*/
	GUITABS::PanelHandle panel = GUITABS::Manager::instance()->getContent( PlacementCtrlsDlg::contentID );
	if ( !panel )
		GUITABS::Manager::instance()->insertPanel( PlacementCtrlsDlg::contentID, GUITABS::FLOATING );
	GUITABS::Manager::instance()->showPanel( panel, true );
