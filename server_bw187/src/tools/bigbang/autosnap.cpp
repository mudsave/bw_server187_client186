...
static bool matchPortal( Portal* p1, int startIndex1,
						...
						float* factor = NULL, Vector3* n1 = NULL,
						Vector3* n2  = NULL )
{
	if( factor )
		*factor = 0.f;
 ...
 */
bool portalsMatch(Portal* p1, Portal* p2, Chunk* c1, Chunk* c2)
{
	if( p1->points.size() < 3 )
		return false;
	...
	for( unsigned int startIndex2 = 0; startIndex2 < p2->points.size(); ++startIndex2 )
	{
		if( matchPortal( p1, 0, p2, startIndex2 ) )
			return true;
	}
...
Matrix getRotatedTransform( Portal* source, Portal* target, Chunk* c1, Chunk* c2 )
{
	unsigned int pointSize = source->points.size();

	Vector3 normal1 = c1->transform().applyVector( source->plane.normal() );
	Vector3 normal2 = c2->transform().applyVector( -target->plane.normal() );
	...
	Matrix transform = c1->transform();
	transform.postTranslateBy( -c1->transform().applyPoint( source->lcentre ) );
	transform.postMultiply( rotation );

	Vector3 p1o0, p1o1, p2o0, p2o1;

	float bestNormal = -1.f;
	...
	for( unsigned int startIndex2 = 0; startIndex2 < pointNum; ++startIndex2 )
	{
		...
		if (matchPortal( source, 0, target, startIndex2, &normal, &n1, &n2 ))
		{

			if (almostEqual( normal, bestNormal ))
			{
				n1 = transform.applyVector( n1 );
				n2 = c2->transform().applyVector( n2 );
				n1.normalise();
				n2.normalise();
			...
			else if (normal > bestNormal)
			{
				...
				normal2 = c2->transform().applyVector( n2 );
				normal1.normalise();
				normal2.normalise();
				bestNormal = normal;
			}
	...
	if (!almostEqual( fabs( normal1.dotProduct( normal2 ) ), 1.f ))
	{
		...
		Quaternion q = Quaternion( binormal.x * sinf( angle / 2.f ),
			binormal.y * sinf( angle / 2.f ),
			binormal.z * sinf( angle / 2.f ),
			cosf( angle / 2.f ) );
	...
	else
	{
		...
		{
			Quaternion q( binormal.x * sinf( D3DX_PI / 2.f ),
				binormal.y * sinf( D3DX_PI / 2.f ),
				binormal.z * sinf( D3DX_PI / 2.f ),
				cosf( D3DX_PI / 2.f ) );
	...
	Portals* snapPortals, Matrix* m )
{
	...
	while( getItemType != GET_ITEM_NUM )
	{
		...
		for (Portals::iterator si = snapPortals->begin(); si != snapPortals->end(); ++si)
		{
			...
			for (std::vector<Chunk*>::iterator i = snapToChunks.begin(); i != snapToChunks.end(); ++i)
			{
				...
				{
					// if they match
					if (portalsMatch(*si, *sti, snapChunk, snapToChunk))
					{
						Vector3 portalOffset = snapChunk->transform().applyPoint( (*si)->lcentre ) -
							snapToChunk->transform().applyPoint( (*sti)->lcentre );

						SnapHistory::iterator iter = std::find( s_snapHistory.begin(), s_snapHistory.end(),
							std::make_pair( *si, *sti ) );

						Matrix m = getRotatedTransform( *si, *sti, snapChunk, snapToChunk );
	...
	if (bestSnapToChunk)
	{
		...
		*m = getRotatedTransform( snapPortal, snapToPortal, snapChunk, bestSnapToChunk );

		m->postTranslateBy( currentOffset );
...
Matrix findRotateSnapTransform( const SnappedChunkSet& snapChunk, bool clockWise, Chunk* referenceChunk )
{
	...
	{for( unsigned int i = 0; i < portals.size(); ++i )
	{
			...
			clockWise ? ++startIndex2 : --startIndex2 )
		{
			Vector3 source;
			Vector3 target;
			if( !matchPortal( portal, 0, portal, startIndex2 ) )
				continue;
			...
			Quaternion q( axis.x * sinf( angle / 2.f ),
				axis.y * sinf( angle / 2.f ),
				axis.z * sinf( angle / 2.f ),
				cosf( angle / 2.f ) );
			q.normalise();
			rotation.setRotate( q );
