...
// page_objects.cpp : implementation file
//
...
{	
	...
	{
		PythonAdapter::instance()->selectFilterUpdate( &selectionFilter_ );
	}
...
// PageObjects message handlers

void PageObjects::OnSetFocus( CWnd* pOldWnd )
{
	//if ( BigBang2App::instance().panelsReady() )
	//{
	//	BigBang::instance().updateUIToolMode( PageObjects::contentID );
	//	OnActivateTool( 0, 0 );
	//}
}
...
void PageObjects::OnCbnSelchangeObjectsSelectionFilter()
{
	...
	if (PythonAdapter::instance())
		PythonAdapter::instance()->selectFilterChange( std::string( sel ));
}

void PageObjects::OnBnClickedObjectsCoordsWorld()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->coordFilterChange( "World" );
}

void PageObjects::OnBnClickedObjectsCoordsLocal()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->coordFilterChange( "Local" );
}

void PageObjects::OnBnClickedObjectsCoordsView()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->coordFilterChange( "View" );
}
	
void PageObjects::OnBnClickedObjectsLockFree()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->ActionScriptExecute("actXZSnap");
}

void PageObjects::OnBnClickedObjectsLockTerrain()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->ActionScriptExecute("actTerrainSnaps");
}

void PageObjects::OnBnClickedObjectsLockObstacle()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->ActionScriptExecute("actObstacleSnap");
}

void PageObjects::OnBnClickedObjectsDragOnSelect()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->ActionScriptExecute("actDragOnSelect");
}

void PageObjects::OnBnClickedObjectsLockToGrid()
{
	if (PythonAdapter::instance())
		PythonAdapter::instance()->ActionScriptExecute("actToggleSnaps");
}
