...
#include "pch.hpp"
#include "editor_chunk_terrain_projector.hpp"
	...
	if (Moo::rc().device())
	{
		...
		else
		{
			CRITICAL_MSG( "EditorChunkTerrainProjector::"
				"EditorChunkTerrainProjector - couldn't load effect material "
...
/**
 * This method is an alternative implementation to Moo::TerrainRenderer::draw.
...
void EditorChunkTerrainProjector::projectTexture(
	...
	EditorChunkTerrainPtrs& spEcts, bool showHoles )
{
	...
	while ( it != end )
	{
		...
		{	// make sure terrain block hasn't disappeared from chunk in the mean time
			blocks.push_back( std::make_pair( spEct->pChunk_->transform(), spEct ) );
		}
	}

	setInitialRenderStates(pTexture,wrap);
	renderBlocks(blocks);
}
 ...
 */
void EditorChunkTerrainProjector::setInitialRenderStates(const Moo::BaseTexturePtr& pTexture, bool wrap)
{
	...
	pEffect->SetInt( pEffect->GetParameterByName(NULL,"wrap"), wrap ? 1/*D3DTADDRESS_WRAP*/ : 3/*D3DTADDRESS_CLAMP*/ );
	if (pTexture && pTexture->pTexture())
...
void EditorChunkTerrainProjector::renderBlocks(BlockVector& blocks)
{
	...
	if (material_->begin())
	{
		...
		while (it != end)
		{
			...
			Moo::TerrainBlockPtr pBlock = spChunk->block_;		
			Matrix tt;
			Vector3 offset = offset_ - spChunk->pChunk_->transform().applyToOrigin();
			...
			for (uint32 i=0; i<material_->nPasses(); i++)
			{
				...
				{					
					...
					{
						EditorTerrainBlock* eBlock = static_cast<EditorTerrainBlock*>(&*pBlock);
						eBlock->drawIgnoringHoles(NULL);
					}	
					material_->endPass();
				}
			}			
	...
	Moo::rc().setTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);	
