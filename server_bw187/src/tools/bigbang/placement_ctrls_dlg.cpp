...
// PlacementCtrlsDlg.cpp : implementation file
//
#include "pch.hpp"
...
PlacementCtrlsDlg::SliderContainer::SliderContainer(
    ...
    controls::EditNumeric* maxEd, CButton* reset, int group )
	: slider_( slider )
	, minEd_( minEd )
	, maxEd_( maxEd )
	, reset_( reset )
...
{
	if ( MessageBox( "Do you want to delete the selected preset?",
		"Placement Controls", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2 ) != IDYES )
		return false;
...
BOOL PlacementCtrlsDlg::OnInitDialog()
{
	...
	// initialize slider-related controls, using the slider vector
	HICON buttonImg = (HICON)LoadImage( AfxGetInstanceHandle(),
			MAKEINTRESOURCE( IDI_PLACEMENT_LINK ),
			IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR );
	linkRandRot_.SetIcon( buttonImg );
	linkRandSca_.SetIcon( buttonImg );

	buttonImg = (HICON)LoadImage( AfxGetInstanceHandle(),
