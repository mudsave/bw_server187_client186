...
void EditorChunkModel::remove( EditorChunkModel* model )
{
		...
		editorChunkModels_.begin(); iter != editorChunkModels_.end(); ++iter )
	{
		...
		for( std::set<EditorChunkModel*>::iterator siter = modelSet.begin();
			siter != modelSet.end(); ++siter )
		{
			if( *siter == model )
			{
...
void EditorChunkModel::reload( const std::string& filename )
{
		...
		iter != modelSet.end(); ++iter )
	{
		...
		(*iter)->toss( NULL );
		DataSectionPtr section = new XMLSection( (*iter)->sectionName() );
		(*iter)->edSave( section );
		(*iter)->clean();
		...
		iter != modelSet.end(); ++iter, ++sec_iter, ++chunk_iter )
	{
		...
		if( *chunk_iter )
		{
			ownSect = EditorChunkCache::instance( **chunk_iter ).
				pChunkSection()->newSection( (*iter)->sectionName() );
		...
		if( (*iter)->chunk() )
		{
			if( (*iter)->chunk()->isOutsideChunk() )
				BigBang::instance().changedTerrainShadows( (*iter)->chunk() );
			else
				BigBang::instance().changedChunkLighting( (*iter)->chunk() );
			BigBang::instance().dirtyThumbnail( (*iter)->chunk() );
		}
	}
}
...
bool EditorChunkModel::edShouldDraw()
{
	if( !ChunkModel::edShouldDraw() )
		return false;
	if( isShellModel() && Chunk::hideIndoorChunks_ )
		return false;
...
void EditorChunkModel::draw()
{
	...
	if (pSuperModel_ != NULL)
	{
		...
		BigBang::instance().addPrimGroupCount( chunk(), primGroupCount_ );
		...
		if (drawBsp && verts_ != (Moo::VertexXYZL*) 1)
		{
			//set the transforms
			Matrix transform;
...
static const std::string DEFAULT_TINT_NAME = "default";
/**
...
static void addDyeTints( std::map<std::string, std::vector<std::string> >& sections, DataSectionPtr ds )
{
	...
	for (i = dyes.begin(); i != dyes.end(); ++i)
	{
		if( sections.find( (*i)->readString( "matter" ) ) == sections.end() )
		{
			...
			names.push_back( DEFAULT_TINT_NAME );
...
bool EditorChunkModel::load( DataSectionPtr pSection, Chunk * pChunk )
{
	...
	if (!ok)
	{
		...
		BigBang::instance().addError(pChunk, this, "Model not found: %s", mname.c_str());
	...
	else
	{
		...
		{
			std::string mname = pSection->readString( "resource" );
			BigBang::instance().addError(pChunk, this, "Model too big: %s", mname.c_str());
	//		CRITICAL_MSG( "Model %s too big!\n", mname.c_str() );
		}
		...
		// Build a list of materials in the model file ( if it has any )
		{
			...
			if( model )
			{
				...
				visualName += ".visual";
				Moo::VisualPtr nodefullVisual = Moo::VisualManager::instance()->get( visualName.c_str() );

				name = model->readString( "nodelessVisual" );
				visualName = BWResource::removeExtension( name );
				visualName += ".static.visual";
				Moo::VisualPtr nodelessVisual = Moo::VisualManager::instance()->get( visualName.c_str() );
				...
				std::vector<Moo::EffectMaterial*>	materials;
				...
				for( std::vector<Moo::EffectMaterial*>::iterator iter = materials.begin(); iter != materials.end();
					++iter)
				{
					if( existingMaterialOverrides.find( (*iter)->identifier() ) == existingMaterialOverrides.end() )
					{
						...
						Moo::EffectMaterial* mat = new Moo::EffectMaterial;
						mat->load( matSec );
...
{
	// Count the amount of primitive groups in all the visuals
...
/** Simple collision callback to find out if point a is visible from ponit b */
...
void EditorChunkModel::edRecalculateLighting( StaticLighting::StaticLightContainer& lights )
{
	if (!isModelNodeless())
		return;
	...
	else
	{
		...
		BinSectionPtr modelLightingSection = new BinSection( shortTag, new BinaryBlock( NULL, 0 ) );
		for (uint i = 0; i < visuals.size(); ++i)
			if (visuals[i])
...
bool EditorChunkModel::edSave( DataSectionPtr pSection )
{
	...
	if (pSuperModel_)
	{
		...
		{
			if( iter->second != DEFAULT_TINT_NAME )
			{
	...
	D3DXPARAMETER_CLASS mcMin, D3DXPARAMETER_CLASS mcMax, D3DXPARAMETER_TYPE mtMin, D3DXPARAMETER_TYPE mtMax ) const
{
	...
	for( std::vector<ChunkMaterialPtr>::const_iterator iter = materialOverride_.begin();
		iter != materialOverride_.end(); ++iter )
	{
		Moo::EffectMaterial* mat = (*iter)->material_;
		if( mat->identifier() == matName )
		{
			ComObjectWrap<ID3DXEffect> pEffect = MaterialUtility::effect( mat );
			if ( pEffect != NULL )
			{
				for ( uint32 i=0; i< mat->nEffects(); ++i )
				{
					ComObjectWrap<ID3DXEffect> pEffect = MaterialUtility::effect( mat, i );
					if( pEffect == NULL )
						continue;

					Moo::EffectMaterial::Properties& properties = mat->properties(i);
	...
	D3DXPARAMETER_CLASS mcMin, D3DXPARAMETER_CLASS mcMax, D3DXPARAMETER_TYPE mtMin, D3DXPARAMETER_TYPE mtMax ) const
{
	...
	pSuperModel_->topModel(0)->gatherMaterials( matName, uses );

	std::string propName = name.substr( name.find( '/' ) + 1, name.npos );

	{
		Moo::EffectMaterial* mat = *uses[0];
		{
			ComObjectWrap<ID3DXEffect> pEffect = MaterialUtility::effect( mat );
			if ( pEffect != NULL )
			{
...
Moo::EffectMaterial* EditorChunkModel::findMaterialByName( const std::string& name ) const
{
	std::string matName = name.substr( 0, name.find( '/' ) );
	std::string propName = name.substr( name.find( '/' ) + 1, name.npos );
...
{
	Moo::EffectMaterial* mat = findMaterialByName( name );
	mat->collisionFlags_ = collisionFlags_.find( collisionType )->second;
...
std::string EditorChunkModel::getMaterialKind( const std::string& name ) const
{
	...
	{
		if( iter->second == mat->materialKind_ )
			return iter->first;
	}
	for( StringHashMap<int>::const_iterator iter = s_materialKinds_.begin();
		iter != s_materialKinds_.end(); ++iter )
...
{
	Moo::EffectMaterial* mat = findMaterialByName( name );
	mat->materialKind_ = s_materialKinds_.find( collisionType )->second;
	changedMaterials_.insert( mat->identifier() );
	return true;
}
...
void EditorChunkModel::edit( Moo::EffectMaterial* material, ChunkItemEditor & editor )
{
...
#define NO_DEFAULT_MATERIAL_ATTRIBUTES
	...
	if( collisionFlags_.empty() )
	{
		...
		{
			if( collisionFlags_.find( (*it)->sectionName() ) == collisionFlags_.end() )
			{
				collisionFlags_[ (*it)->sectionName() ] = ( *it )->asInt();
				collisionFlagNames_.push_back( (*it)->sectionName() );
			}
		}
	}
...
bool EditorChunkModel::edEdit( ChunkItemEditor & editor )
{
			...
			resourceID().substr(8,6).c_str() ) != 0)
	{
		...
		editor.addProperty( new GenBoolProperty( "outside only",
			new AccessorDataProxy< EditorChunkModel, BoolProxy >(
				this, "outsideOnly", 
...
/*	else
	...
	editor.addProperty( new ListTextProperty( "animation",
	...
	editor.addProperty( new GenFloatProperty( "animation speed",
	...
	editor.addProperty( new StaticTextProperty( pSuperModel_->nModels() == 1 ? "modelName" : "modelNames", new
		...
		for (int i = 0; i < pSuperModel_->nModels(); i++)
		{
			...
			for (int j = 0; ; j++)
			{
				const std::pair< const char *, Model::Matter > *
					pmit = pTop->lookupLocalMatter( j );
				if (pmit == NULL) break;

				// and collect its tints it has
				const Model::Tints & mts = pmit->second.tints_;
				for (Model::Tints::const_iterator tit = mts.begin()+1;
					tit != mts.end();	// except default...
					tit++)
				{
					mds[ pmit->first ].tintNames.insert( tit->first );
				}
			}
		}
...
std::string EditorChunkModel::edDescription()
{
	if (isShellModel())
	{
		if (pChunk_)
		{
			return "Chunk " + pChunk_->identifier();
		}
		else
		{
...
std::vector<std::string> EditorChunkModel::edCommand( const std::string& path ) const
{
	...
	if( !models.empty() )
	{
		commands.push_back( "Edit in Model Editor..." );
