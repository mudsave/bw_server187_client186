 ...
 */
/*virtual*/ bool 
ElevationCodecBMP::canHandleFile
(
    ...
    try
    {
        ...
        if (infoHeader.bits == 8)
        {
            ...
            for (int i = 0; i < 256; ++i)
            {
                ...
                palette[i] = BMP_HEIGHT*(R_TO_L*r + G_TO_L*g + B_TO_L*b)/255.0f;
            }
        ...
        else if (infoHeader.bits == 24)
        {
            ...
            for (unsigned int y = 0; y < infoHeader.height; ++y)
            {
                ...
                {
                    float b = (float)(row[x*3    ]);
			        float g = (float)(row[x*3 + 1]);
			        float r = (float)(row[x*3 + 2]);
                    *p = BMP_HEIGHT*(R_TO_L*r + G_TO_L*g + B_TO_L*b)/255.0f;
                }
            }
        }
        ...
        else if (infoHeader.bits == 32)
        {
            ...
            for (unsigned int y = 0; y < infoHeader.height; ++y)
            {
                ...
                for (unsigned int x = 0; x < infoHeader.width; ++x, ++p)
                {
                    float b = (float)(row[x*4    ]);
			        float g = (float)(row[x*4 + 1]);
			        float r = (float)(row[x*4 + 2]);
                    *p = BMP_HEIGHT*(R_TO_L*r + G_TO_L*g + B_TO_L*b)/255.0f;
                }
            }
        }
 ...
 *  @returns            True.
 */
/*virtual*/ bool ElevationCodecBMP::canLoad() const
{
    return true;
}
 ...
 *  @returns            True.
 */
/*virtual*/ bool ElevationCodecBMP::canSave() const
{
    return true;
}
 ...
 */
/*static*/ ElevationCodecBMP &ElevationCodecBMP::instance()
{
    return s_instance;
}
