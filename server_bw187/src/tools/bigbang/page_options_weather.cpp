...
class ReturnNotifyEdit : public CEdit
{
...
IMPLEMENT_DYNAMIC(ReturnNotifyEdit, CEdit)

BEGIN_MESSAGE_MAP(ReturnNotifyEdit, CEdit)
END_MESSAGE_MAP()
...
{
	addWatch( "Client Settings/Clouds/wind y", "Cloud Velocity", "The speed at which the clouds move across the sky", -320.f, 0.f, 1.f, 1.0, true );

	addWatch( "Client Settings/Weather/CLEAR/propensity", "Clear Weighting", "The chance of clear weather weighted against the other types", 0.0f, 10.f, 0.1f, 0.1f, true );

	addWatch( "Client Settings/Weather/CLOUD/propensity", "Cloud Weighting", "The chance of cloudy weather weighted against the other types", 0.0f, 10.f, 0.1f, 0.1f, false );
	addWatch( "Client Settings/Weather/CLOUD/arg0", "Cloud Cover", "The amount of cloud cover", 0.0f, 1.f, 0.01f, 0.01f, false );
	addWatch( "Client Settings/Weather/CLOUD/arg1", "Cloud Cohesion", "How clumpy the clouds are", 0.0f, 1.f, 0.01f, 0.01f, true );

	addWatch( "Client Settings/Weather/RAIN/propensity", "Rain Weighting", "The chance of rain weighted against the other types", 0.0f, 10.f, 0.1f, 0.1f, false );
	addWatch( "Client Settings/Weather/RAIN/arg1", "Cloud Cohesion", "How clumpy the clouds are", 0.0f, 1.f, 0.01f, 0.01f, false );
	addWatch( "Client Settings/Weather/RAIN/arg0", "Darkness", "The final darkness of the whole scene", 0.0f, 1.f, 0.01f, 0.01f, false );
	addWatch( "Client Settings/Rain/area", "Effective Drop Size", "How big the rain drops are if it is raining", 0.5f, 10.f, 0.1f, 0.1f, true );

	addWatch( "Client Settings/Weather/STORM/propensity", "Storm Weighting", "The chance of a storm weighted against the other types", 0.0f, 10.f, 0.1f, 0.1f, false );
	addWatch( "Client Settings/Rain/area", "Effective Drop Size", "How big the rain drops are if it is raining", 0.5f, 10.f, 0.1f, 0.1f, true );

	addWatch( "Client Settings/Flora/Wind X", "Flora Wind Velocity X", "The velocity of the wind which affects the flora swaying along the X axis", 0.0f, 50.f, 0.01f, 0.01f, false );
	addWatch( "Client Settings/Flora/Wind Z", "Flora Wind Velocity Z", "The velocity of the wind which affects the flora swaying along the Z axis", 0.0f, 50.f, 0.01f, 0.01f, false );

	addWatch( "Client Settings/Weather/windVelX", "Particle Wind Velocity X", "The velocity of the wind which affects the particle systems along the X axis", -25.f, 25.f, 0.01f, 0.01f, false );
	addWatch( "Client Settings/Weather/windVelY", "Particle Wind Velocity Z", "The velocity of the wind which affects the particle systems along the Z axis", -25.f, 25.f, 0.01f, 0.01f, false );

	addWatch( "Client Settings/Weather/temperature", "Temperature", "The ambient temperature", -40.0f, 40.0f, 0.1f, 0.1f, false );

	descText_.Create( "This panel allows previewing of the space under "
		"various weather conditions.\n"
		"These are not saved as part of the space.",
	...
	float granularity, bool separaterFollow )
{
	...
	control.slider_->Create(
		...
		this, id );
	control.slider_->SetRange( int( min / granularity ), int( max / granularity ) );
	control.slider_->SetPos( int( control.cur_ / control.granularity_ ) );
...
float PageOptionsWeather::getValue( const CString& name )
{
	...
	if( !Watcher::rootWatcher().getAsString( NULL, name, result, type ) )
		...
		iter != WatchControls.end(); ++iter )
	{
		if( iter->edit_->GetDlgCtrlID() == nID )
		{
			...
			if( atof( text ) <= iter->maxValue_ && atof( text ) >= iter->minValue_ )
			{
				...
				iter->slider_->SetPos( int( iter->cur_ / iter->granularity_ ) );
					...
					jet != WatchControls.end(); ++jet )
				{
					...
					if( jet->name_ == iter->name_ )
					{
						...
						jet->slider_->SetPos( int( iter->cur_ / jet->granularity_ ) );
					}
				}
			}
			break;
...
afx_msg HBRUSH PageOptionsWeather::OnCtlColor( CDC* pDC, CWnd* pWnd, UINT nCtlColor )
{
	...
	for( std::vector<WatchControl>::iterator iter = WatchControls.begin();
		iter != WatchControls.end(); ++iter )
	{
		if( iter->edit_->m_hWnd == pWnd->m_hWnd )
		{
			CString text;
			iter->edit_->GetWindowText( text );
			if( atof( text ) <= iter->maxValue_ && atof( text ) >= iter->minValue_ )
...
afx_msg LRESULT PageOptionsWeather::OnUpdateControls(WPARAM wParam, LPARAM lParam)
{
		...
		iter != WatchControls.end(); ++iter )
	{
		...
		iter->cur_ = cur;
		iter->edit_->SetWindowText( text );
		iter->slider_->SetPos( int( cur / iter->granularity_ ) );
	}
...
void PageOptionsWeather::OnBnClickedWeatherResetall()
{
	...
	BigBang::instance().refreshWeather();
