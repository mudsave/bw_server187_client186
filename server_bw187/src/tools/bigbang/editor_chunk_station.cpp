...
#include "station_node_link_proxy.hpp"
...
void EditorChunkStationNode::toss( Chunk * pChunk )
{
	...
	if (pChunk_ != NULL)
	{
		if (!pOwnSect_ && !pChunk_->loading() && EditorChunkCache::instance( *pChunk_ ).pChunkSection() )
		{
			...
			Options::setOptionInt( "render/misc/drawPatrolGraphs", 1 );
		}
...
bool EditorChunkStationNode::edTransform( const Matrix & m, bool transient )
{
	...
	if( conn && pOldChunk != pNewChunk )
	{
		...
		conn->linkPoint( (int16)cx, (int16)cz, (int16)nx, (int16)nz );
	}
...
bool EditorChunkStationNode::edEdit( class ChunkItemEditor & editor )
{
	...
	editor.addProperty( new StaticTextProperty( "id", new
		ConstantDataProxy<StringProxy>( id().toString() ) ) );

	if (graph() != NULL)
		editor.addProperty( new StaticTextProperty( "graph", new
    ...
    editor.addProperty( new LinkProperty( "link", linkProxy, pMP ) );
...
/*virtual*/ bool EditorChunkStationNode::edIsEditable()
{
	...
	if( BWLock::BigBangdConnection* conn = BigBang::instance().connection() )
	{
		...
		for( std::vector<ChunkStationNode*>::size_type i = 0; i < nodes.size() - 1; ++i )
		{
			...
			if( !conn->isSameLock( (int16)cx, (int16)cz, (int16)nx, (int16)nz ) )
				return false;
		}
	}
	return true;
...
)
{
	...
	BigBang::instance().changedChunk( other->chunk() );
...
void EditorChunkStationNode::split(EditorChunkStationNode &other)
{
    ...
    UndoRedo::instance().barrier("Split link", false);
...
ModelPtr EditorChunkStationNode::reprModel() const
{
	static ModelPtr model =
		Model::get( "resources/models/station.model" );
	return model;
}
...
static PyObject* py_adjustStationLink( PyObject* args )
{
	...
	for (std::vector<ChunkItemPtr>::iterator it = items1.begin();
		...
		it++)
	{
		if ((*it)->edDescription() == "patrol node")
			stationsFound = true;
		else
	...
	{
		if ((*i)->edDescription() == "patrol node")
		{
	...
	if (!properties.empty())
		BigBang::instance().addCommentaryMsg( "Connected to " +
			node2->graph()->name().toString(), 0 );

	Py_Return;
}
PY_MODULE_FUNCTION( adjustStationLink, BigBang )
