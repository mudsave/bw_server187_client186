...
#include "pch.hpp"
#include "editor_chunk_marker.hpp"
...
private:
	MarkerModels()
	{
		...
		DataSectionPtr pMarkerSection = BWResource().openSection( "entities/marker_categories.xml" );
		if (pMarkerSection)
		{
...
bool EditorChunkMarker::loadEntityDescription()
{
	...
	{
		DataSectionPtr sect = BWResource::openSection("entities/marker_categories.xml");
		if (sect)
		{
			DataSectionPtr categoryLevel = sect->openSection( getCategory() + "/level" );
			if (categoryLevel)
			{
				DataSectionPtr markerXMLs = BWResource::openSection( "entities/marker_entities" );
...
bool EditorChunkMarker::edEdit( class ChunkItemEditor & editor )
{
	...
	DataSectionPtr edEntitySect = BWResource::openSection("entities/editor");
	if (edEntitySect)
	{
	...
	editor.addProperty( new ListTextProperty( "category",
...
/*
	Adjust the cluster, join or remove the cluster association between 
	what's currently selected (passed in the 1st arg, and what properties 
	are loaded), and the what the mouse is current over
*/
static PyObject* py_adjustMarkerCluster( PyObject* args )
{
	...
	if (items2.front()->edDescription() == "marker")
	{
	...
	else if (items2.front()->edDescription() == "marker cluster")
	{
...
PY_MODULE_FUNCTION( adjustMarkerCluster, BigBang )
