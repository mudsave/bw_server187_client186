...
namespace
{
    ...
    bool validMeshFilename(CString const &filename, CString const &fullname_)
    {        
	    const int extensionLength = 7;
	    CString extension = filename.Right(extensionLength);
	    if (extension != CString(".visual"))
            return false;        
        CString &fullname = const_cast<CString &>(fullname_);
        if (!MeshParticleRenderer::isSuitableVisual(fullname.GetBuffer()))
            return false;
        return true;
    }

    bool validVisualFilename(CString const &filename, CString const &)
    {
 	    const int extensionLength = 7;
	    CString extension = filename.Right(extensionLength);
	    return extension == CString(".visual");
    }

    bool validMaterialFilename(CString const &filename, CString const &)
    {
	    const int extensionLength = 4;
	    CString extension = filename.Right(extensionLength);
	    return extension == CString(".mfm");
    }

    bool validTextureFilename(CString const &filename, CString const &)
    {
	    const int extensionLength = 4;
	    CString extension = filename.Right(extensionLength);
	    ...
	    if 
        (
            (extension == CString(".tga")) 
		    || 
            (extension == CString(".bmp"))
		    || 
            (extension == CString(".dds"))
        )
	    {
		    return true;
	    }
    ...
    typedef bool (*TestFn)(CString const &s, CString const &t);
    ...
    )
    {
	    ...
	    if (!directory.empty())
	    {
            ...
            )
		    {
			    ...
			    filename.MakeLower();
                CString fullname = relativeDirectory.c_str() + filename;
			    if (test(filename, fullname))
			    {
				    theBox.AddString(filename);
			    }
		    }
	    }
...
BEGIN_MESSAGE_MAP(PsRendererProperties, CFormView)
	...
	ON_BN_CLICKED(IDC_PS_RENDERER_VIEWDEPENDENT, OnGenericBtn)
...
void PsRendererProperties::SetParameters(SetOperation task)
{
	...
	{
		// read in
		viewDependent_.SetCheck(renderer()->viewDependent() ? BST_CHECKED : BST_UNCHECKED);
        ...
        )
		{
			...
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
			opts->writeString( "spriteTexture", longFilename.GetBuffer() );
		...
		else if (renderer()->nameID() == MeshParticleRenderer::nameID_)
		{
			...
			// Remember for next time:
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
			opts->writeString( "meshVisual", longFilename.GetBuffer() );
		...
		else if (renderer()->nameID() == VisualParticleRenderer::nameID_)
		{
			...
			// Remember for next time:
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
			opts->writeString( "visualVisual", longFilename.GetBuffer() );
		...
		else if(renderer()->nameID() == AmpParticleRenderer::nameID_)
		{
			...
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
			opts->writeString( "ampTexture", longFilename.GetBuffer() );
		...
		else if(renderer()->nameID() == TrailParticleRenderer::nameID_)
		{
			...
			// remember the selection
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
			opts->writeString( "trailTexture", longFilename.GetBuffer() );
		...
		else if(renderer()->nameID() == BlurParticleRenderer::nameID_)
		{
			...
			// remember the selection
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
			opts->writeString( "blurTexture", longFilename.GetBuffer() );
	...
	else
	{
		...
		// write out
		renderer()->viewDependent(viewDependent_.GetCheck() == BST_CHECKED);
        ...
        )
		{
			...
			if (selected != -1)
			{
				...
				// remember for next time
				DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
				opts->writeString( "spriteTexture", (dirName + texName).GetBuffer() );
			}
		...
		else if (renderer()->nameID() == MeshParticleRenderer::nameID_)
		{
			...
			if (selected != -1)
			{
				...
				// remember for next time
				DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
				opts->writeString( "meshVisual", (dirName + visName).GetBuffer() );
			}
		...
		else if(renderer()->nameID() == AmpParticleRenderer::nameID_)
		{
			...
			if (selected != -1)
			{
				...
				// remember the selection
				DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
				opts->writeString( "ampTexture", (dirName + texName).GetBuffer() );
			}
		...
		else if(renderer()->nameID() == TrailParticleRenderer::nameID_)
		{
			...
			if (selected != -1)
			{
				...
				// remember the selection
				DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
				opts->writeString( "trailTexture", (dirName + texName).GetBuffer() );
			}
		...
		else if(renderer()->nameID() == BlurParticleRenderer::nameID_)
		{
			...
			if (selected != -1)
			{
				...
				// remember the selection
				DataSectionPtr opts = Options::pRoot()->openSection( "defaults/renderer", true );
				opts->writeString( "blurTexture", (dirName + texName).GetBuffer() );
			}
...
void PsRendererProperties::OnSpriteBtn()
{
    ...
    )
	{
		...
		string defaultTexture = 
            Options::pRoot()->readString
            ( 
                "defaults/renderer/spriteTexture", 
                s_notFoundTexture 
            );
...
void PsRendererProperties::OnAmpBtn()
{
	...
	if (renderer()->nameID() != AmpParticleRenderer::nameID_)
	{
		...
		{
			string defaultTexture = Options::pRoot()->readString( "defaults/renderer/ampTexture", s_notFoundTexture );
...
void PsRendererProperties::OnTrailBtn()
{
	...
	if (renderer()->nameID() != TrailParticleRenderer::nameID_)
	{
		...
		{
			// get default parameters from the psa
			string defaultTexture = Options::pRoot()->readString( "defaults/renderer/trailTexture", s_notFoundTexture );
...
void PsRendererProperties::OnBlurBtn()
{
	...
	if (renderer()->nameID() != BlurParticleRenderer::nameID_)
	{
		...
		{
			// get default parameters from the psa
			string defaultTexture = Options::pRoot()->readString( "defaults/renderer/blurTexture", s_notFoundTexture );
...
void PsRendererProperties::OnSpriteTexturenameDirectoryBtn()
{
	...
	dlg.promptText_ = _T("Choose directory..");
