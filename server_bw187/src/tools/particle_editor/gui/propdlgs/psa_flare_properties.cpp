...
static bool validFlareFilename(const CString & filename)
{
	const int extensionLength = 4;
	CString extension = filename.Right(extensionLength);
	const CString correctExtension(".xml");
...
void PsaFlareProperties::OnInitialUpdate()
{
	...
	// for some reason, it seems to be good to set it back to the PSA
	//CopyDataToPSA();
...
void PsaFlareProperties::SetParameters(SetOperation task)
{
	...
	if (task == SET_CONTROL)
	{
		...
		if (longFilename.IsEmpty())
		{
			...
			longFilename = Options::pRoot()->readString( "resourceGlue/environment/sunFlareXML" ).c_str();

			// see if already have saved one
			longFilename = Options::pRoot()->readString( "defaults/flareXML", longFilename.GetBuffer() ).c_str();
		}
		...
		opts->writeString( "flareXML", longFilename.GetBuffer() );
	...
	else
	{
		...
		if (selected != -1)
		{
			...
			// remember for next time
			DataSectionPtr opts = Options::pRoot()->openSection( "defaults", true );
			opts->writeString( "flareXML", (dirName + "/" + texName).GetBuffer() );
		}
	}
}
