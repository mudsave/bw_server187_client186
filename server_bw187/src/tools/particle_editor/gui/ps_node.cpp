...
void PSNode::addChildren()
{
    ...
    tree->AddNode
    (
        new ActionNode(NULL, "System Properties", ActionNode::AT_SYS_PROP), 
        this
    );
    // Add the renderer properties nodes:
    tree->AddNode
    (
        new ActionNode(NULL, "Renderer Properties", ActionNode::AT_REND_PROP), 
        this
    ); 
    ...
    for (size_t j = 0; j < actions.size(); ++j)
    {
        ...
        tree->AddNode(new ActionNode(psa, name), this);              
    }
}
