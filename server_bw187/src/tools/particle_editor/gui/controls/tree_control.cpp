...
void TreeNode::SetLabel(string const &label)
{
    label_ = label;
    if (htreeitem_ != NULL)
        tree_->SetItemText(htreeitem_, label_.c_str());
}
...
)
{
    ...
    if (load)
    {
        ...
        if (selected)
            result = this;
        size_t nodeidx = 0;
        DataSectionIterator it = data->begin();
        for (;it != data->end() && nodeidx < children_.size(); ++it)
        {
...
UINT TreeControl::GetDragDropID()
{
    if (TC_CFORMAT == 0)
    {
        TC_CFORMAT = ::RegisterClipboardFormat("TreeControlClipboard");
    }
    return TC_CFORMAT;
}
    ...
    if (node != NULL)
    {
        if (node->CanEditLabel())
        {
            if (dispInfo->item.pszText != NULL)
                node->OnEditLabel(dispInfo->item.pszText);
