...
MainFrame::MainFrame() : 
    ...
    bgColor_(0.63f, 0.63f, 0.81f, 0.f),
...
int MainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    DataSectionPtr opts = Options::pRoot()->openSection( "defaults" );
    if (opts)
    {
        particleDirectory_ = 
            opts->readString
            ( 
                "startDirectory", 
                particleDirectory_.GetBuffer() 
            ).c_str();
        Vector4 colour = 
            opts->readVector4("backgroundColour", Vector4::zero());
        bgColor_.r = colour.x;
        bgColor_.g = colour.y;
        bgColor_.b = colour.z;
        bgColor_.a = colour.w;
        drawScene = opts->readBool("drawScene", true);
    }
    else
    {
        // create the defaults
        ParticlesDirectory(particleDirectory_);
        BgColor(bgColor_);
    }
...
bool MainFrame::InitialiseMF()
{
    ...
    argv[argc] = strtok(ParticleEditorApp::instance().m_lpCmdLine, seps);

    while (argv[ argc ] != NULL)
    {
        if (++argc >= MAX_ARGS)
        {
            ERROR_MSG("Too many command line arguments!!\n");
            return FALSE;
        }
    ...
    INFO_MSG( "argc, argv args are:\n");
    for (int i = 0; i < argc; i++)
        INFO_MSG( "arg[%d] = %s\n", i, argv[i] );

    if (!Options::init(argc, argv, false)) // We don't want a userName in PE
    {
        ERROR_MSG("Unable to initialize options\n");
        return FALSE;
    }

    if (!BWResource::init( argc, argv ))
    {
        ERROR_MSG("Unable to initialize resources\n");
        return FALSE;
    }

    if (!BWResource::isValid())
    {
        ERROR_MSG( "No resource paths were found.  Please check your setup and try again.\n" );
        return FALSE;
    }
...
int MainFrame::PromptSave(UINT type, bool clearUndoStack /*= false*/)
{
    ...
    {        
        CString promptText;
        CString promptTitle;
        promptText.LoadString(IDS_PROMPTSAVE);
        promptTitle.LoadString(IDS_PROMPTSAVETITLE);
        result = 
            ::MessageBox
            ( 
...
void MainFrame::ParticlesDirectory(CString directory)
{
    ...
    DataSectionPtr opts = Options::pRoot()->openSection("defaults", true);
    opts->writeString("startDirectory", particleDirectory_.GetBuffer());
}

void MainFrame::BgColor(Moo::Colour c)
{
    bgColor_ = c;
    Vector4 colour;
    colour.x = bgColor_.r;
    ...
    colour.w = bgColor_.a;
    DataSectionPtr opts = Options::pRoot()->openSection("defaults", true);
    opts->writeVector4("backgroundColour", colour);
}
...
#include "../bigbang/editor_group.hpp"
...
bool chunkWritable( Chunk * pChunk )	{	return true;	}

#include "chunk/chunk_vlo.hpp"
ChunkVLO* VeryLargeObject::createReference( std::string& uid, Chunk*
pChunk ) { return NULL; }
