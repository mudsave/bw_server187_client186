...
void init()
{
	...
	Moo::rc().instance().createDevice( hWnd );
							...
							std::vector<std::string>& materialIDs )
{
	...
	if( pt == D3DPT_TRIANGLELIST )
	{
		...
		{
			const Moo::Primitive::PrimGroup& pg = prims->primitiveGroup( i );
	...
	if ( pt == D3DPT_TRIANGLELIST )
	{
		...
		{			
			...
			const Moo::Primitive::PrimGroup& pg = prims->primitiveGroup(
...
static std::string updateBSPVersion( const std::string& visualName )
{
	...
	wasNewVersion = loader.loadBSPTree( pFoundBSP, bspMaterialIDs );
