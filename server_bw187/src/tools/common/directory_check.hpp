...
public:
	DirectoryCheck( const std::string& appName )
	{
		...
		{// we are not running in the proper directory
			...
			{
				GetCurrentDirectory( MAX_PATH_SIZE, curDir );
				TCHAR message[ MAX_PATH_SIZE * 2 ];
				_stprintf( message, TEXT( "You are running %s from:\n\t\"%s\"\nThis is not a correct folder for %s. Please run the exectable directly." ),
					appName.c_str(), curDir, appName.c_str() );
				MessageBox( NULL, message, appName.c_str(), MB_OK | MB_ICONASTERISK );
			}
		}
	}
};