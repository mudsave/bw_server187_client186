...
// helper function to ensure that paths are understanded by windows even if
...
std::string SpaceManager::browseForSpaces( HWND parent ) const
{
    ...
    browseInfo.lpszTitle = "\nSelect the space folder to open."
		"\nIt must contain a 'space.settings' file.";
	...
	case BFFM_SELCHANGED:
		...
		if( !path.empty() )
		{
			...
			SendMessage( hwnd, BFFM_ENABLEOK, 0,
				GetFileAttributes( ( path + "space.settings" ).c_str() )
