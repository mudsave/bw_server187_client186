...
public:
	MaterialEditor( Moo::EffectMaterial* m, PyTypePlus * pType = &s_type_ );
	~MaterialEditor();
	
private:
	void edit( Moo::EffectMaterial* m );
};
...
{
public:
	MaterialKindProxy( Moo::EffectMaterial* m ):
      material_(m)
	{
	}

	uint32 EDCALL get() const { return material_->materialKind_; }

	void EDCALL set( uint32 f, bool transient )
	{
		material_->materialKind_ = f;
	}
...
{
public:
	CollisionFlagsProxy( Moo::EffectMaterial* m ):
      material_(m)
	{
	}

	uint32 EDCALL get() const { return material_->collisionFlags_; }

	void EDCALL set( uint32 f, bool transient )
	{
        if ( material_->collisionFlags_ != f )
        {
    		material_->collisionFlags_ = f;
