    ...
    , maxZoomOut_(std::numeric_limits<float>::max())
{
	...
	mouseLookCamera_ = new MouseLookCamera();
	orbitCamera_ = new OrbitCamera();
...
ToolsCamera::~ToolsCamera()
{
	delete mouseLookCamera_;
	delete orbitCamera_;
}

void ToolsCamera::update( float dTime /* = 0.f */, bool activeInputHandler /* = true */)
{
	BaseCamera* bc = (cameraMode_ != CameraMode_Orbit) ?
		(BaseCamera*)mouseLookCamera_ :
		(BaseCamera*)orbitCamera_;
	...
	case CameraMode_X:
		{
			...
			bc->view().setRotateY( -0.5f * MATH_PI );
			bc->view().postTranslateBy( camPos );
			bc->view().invert();
	...
	case CameraMode_Y:
		{
			...
			bc->view().multiply(viewMat, rotateMat);
			bc->view().postTranslateBy( camPos );
			bc->view().invert();
...
bool ToolsCamera::handleMouseEvent( const MouseEvent & me )
{
	...
	if (cameraMode_ != CameraMode_Orbit)
	{
		...
		bc = (BaseCamera*)mouseLookCamera_;
	}
	else
	{
		handled = orbitCamera_->handleMouseEvent(me);
		bc = (BaseCamera*)orbitCamera_;
	}
...
void ToolsCamera::save()
{
	...
	Options::pRoot()->writeMatrix34( "startup/lastView", view() );
...
void ToolsCamera::zoomToExtents( bool animate, BoundingBox& box /* = BoundingBox::s_insideOut_ */ )
{
	...
	BaseCamera* bc = (cameraMode_ != CameraMode_Orbit) ?
		(BaseCamera*)mouseLookCamera_ : (BaseCamera*)orbitCamera_;
...
Vector3& ToolsCamera::origin() const
{
	return orbitCamera_->origin();
}

Matrix& ToolsCamera::view() const
{
	BaseCamera* mbc;
	
	if (cameraMode_ != CameraMode_Orbit)
		mbc = (BaseCamera*)mouseLookCamera_;
	else
		mbc = (BaseCamera*)orbitCamera_;
...
/**
 *	This function gets the ModelEditor camera
 */
