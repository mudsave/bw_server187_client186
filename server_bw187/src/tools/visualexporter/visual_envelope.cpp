	...
	if (pPhyMod)
	{
		...
		if (phyExport)
		{
			...
			if( contextExport )
			{
				...
				{
					MessageBox( GetForegroundWindow(),
						"VisualEnvelope::init - unable to collect initial transform in physiqued object\n",
						"Visual Exporter", MB_OK | MB_ICONEXCLAMATION );
					return false;
				}
		...
		if (hasBlended_)
			...
			ret = checkDXMaterials("_hardskinned");
	}
	else if (pSkinMod)
	{
	...
	if (pFile)
	{
		...
		else
		{
			...
			else
			{
				...
				Moo::VertexHeader vh;
				vh.nVertices_ = vertices.size();
				bw_snprintf( vh.vertexFormat_, sizeof(vh.vertexFormat_), "xyznuvitb" );
				f << vh;
				f << vertices;
			}
		}
		...
		if (pRenderSet)
		{
			...
			for (int i = 0; i < boneNodes_.size(); i++)
			{
				...
				if (pNode)
				{
					...
					{
						pNode->setString( trimWhitespaces( boneNodes_[i]->GetName() ) );
					}
				}
			}
