...
static const float distTolerance = 0.01f;
...
void HullMesh::extractPlanesAndPortals( VertexVector& vv, std::vector<TriIndex>& faces )
{
	...
	while ( it != end )
	{
		Triangle& tri = *it++;
		PlaneEq peq(
			Vector3(vv[tri.index[0]].pos),
			Vector3(vv[tri.index[1]].pos),
			Vector3(vv[tri.index[2]].pos) );
		...
		if (isPortal( tri.materialIndex ))
		{
			...
			vpp->addSwizzledPoint(Vector3(vv[tri.index[2]].pos));
			vpp->addSwizzledPoint(Vector3(vv[tri.index[1]].pos));
			vpp->addSwizzledPoint(Vector3(vv[tri.index[0]].pos));
		}
	}
	...
	for ( it = verts.begin(); it != verts.end(); )
	{
		...
		for ( face_it = faces.begin(); face_it != faces.end(); face_it++ )
		{
			...
			if ( PlaneEq(	Vector3( vertices[face_it->i1].pos ),
					...
					.isInFrontOf( Vector3( vertices[*it].pos ) ) )
			{
				inside = false;
				break;
			}
								...
								std::vector<uint>& verts, std::vector<TriIndex>& faces ) const
{
	...
	for ( size_t i = 0; i < faces.size(); i++ )
	{
		if ( PlaneEq(	Vector3( vertices[faces[i].i1].pos ),
				...
				.isInFrontOf( Vector3( vertices[vertIndex].pos ) ) )
		{
	...
	while ( sortedHorizonVerts.size() != horizonVerts.size() )
	{
		...
		{
			// Loop throught the vertices of the jth front face
			for ( int k = 0; k < 3; k++ )
			{
