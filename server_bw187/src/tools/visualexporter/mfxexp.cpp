...
int MFXExport::DoExport(const TCHAR *nameFromMax,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options)
{
	...
	if (!validResource( nameFromMax, resNameCamelCase ))
	{
		invalidExportDlg( nameFromMax, ip_ );
		errors_ = true;
		return 1;
	}
	...
	if (settings_.exportMode() != ExportSettings::MESH_PARTICLES)
	{
		...
		if (ds && (bp = ds->asBinary()))
		{
			...
			{
				MessageBox( ip_->GetMAXHWnd(), "Unable to open primitives file", "MFExporter Error", MB_OK | MB_ICONERROR );
				return 1;
			}
...
/*		std::string nodeName = node->GetName();
	...
	if (trailingLeadingWhitespaces(thisNode->getIdentifier()))
	{
		char s[1024];
