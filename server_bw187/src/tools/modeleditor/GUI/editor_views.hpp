...
public:
	...
	virtual void updateGUI()
	{
		...
		if (newValue != oldValue_)
		{
			...
			std::string strValue = "";
			for ( StringMap< int >::iterator it = choices_.begin();
				it != choices_.end();
				it++ )
			{
...
public:
	...
	virtual void onChange( bool transient )
	{
		...
		property_.pMatrix()->setMatrix( matrix );
...
public:
	...
	virtual void onChange( bool transient )
	{
		...
		property_.pMatrix()->setMatrix( newMatrix );
		property_.pMatrix()->commitState();
	}
...
private:
	...
	{
		sprintf( buf, "#%02X%02X%02X%02X", (int) c.r, (int) c.g, (int) c.b, (int) c.a );
	}
