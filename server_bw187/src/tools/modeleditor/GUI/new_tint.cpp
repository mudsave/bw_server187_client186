...
void CNewTint::redrawList( CComboBox& list, const std::string& name )
{
	...
	list.InsertString( data.size(), "(Other...)" );
...
{	
	...
	if ( fileDlg.DoModal() == IDOK )
	{
		...
		{
			MessageBox( "You can only select an effect file located in one of the resource folders.",
...
void CNewTint::OnBnClickedNewTintMfmSel()
{
	...
	if ( fileDlg.DoModal() == IDOK )
	{
		...
		{
			MessageBox( "You can only select an MFM file located in one of the resource folders.",
				"Unable to resolve MFM file path",
				MB_OK | MB_ICONWARNING );
		}
	}
}
...
void CNewTint::OnOK()
{
	...
	{
		MessageBox( "Unable to create the tint since no name was given.\nAt least one alphanumeric character is needed.",
	...
	{
		MessageBox( "Unable to create a tint with that name since one already exists, please select another name.",
	...
	{
		MessageBox( "The tint name \"Default\" is reserved for the default tint, please select another name.",
			"Cannot use \"Default\" as tint name", MB_OK | MB_ICONERROR );
