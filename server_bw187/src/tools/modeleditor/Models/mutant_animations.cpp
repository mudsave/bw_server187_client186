...
StringPair Mutant::createAnim( const StringPair& animID, const std::string& animPath )
{
	...
	{
		index++;
		sprintf(buf, "%s %d", animID.first.c_str(), index);
		new_anim.first = std::string(buf);
	}

	UndoRedo::instance().add( new UndoRedoOp( 0, models_[animID.second] ));
	UndoRedo::instance().barrier( "Adding a new animation", false );
...
void Mutant::changeAnim( const StringPair& animID, const std::string& animPath )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data, false, animID ));
	UndoRedo::instance().barrier( "Changing an animation source", false );
	...
	dirty( animations_[animID].model );
...
void Mutant::removeAnim( const StringPair& animID )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].model ));
	UndoRedo::instance().barrier( "Deleting of animation", false );
...
bool Mutant::animName( const StringPair& animID, const std::string& animName )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[new_anim].data ));
	UndoRedo::instance().barrier( "Changing name of animation", false );
	...
	dirty( animations_[new_anim].model );
...
void Mutant::firstFrame( const StringPair& animID, int frame )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data ));
	UndoRedo::instance().barrier( "Changing first frame of animation", false );
...
void Mutant::lastFrame( const StringPair& animID, int frame )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data ));
	UndoRedo::instance().barrier( "Changing last frame of animation", false );
...
void Mutant::localFrameRate( const StringPair& animID, float rate, bool final )
{
	...
	if (final)
	{
		...
		UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].frameRates ));
		UndoRedo::instance().barrier( "Changing frame rate of animation", false );
		lastFrameRate_ = rate;
	}
...
void Mutant::saveFrameRate( const StringPair& animID )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data ));
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].frameRates ));
	UndoRedo::instance().barrier( "Saving frame rate of animation", false );
...
void Mutant::frameNum( const StringPair& animID, int num )
{
	...
	if ((animID.first != "") && (animID.second != "") && (animations_.find(animID) != animations_.end()))
	{
		...
		if (animation)
		{
			animation->time = (num-1.f) / frameRate(animID);
		}
	}
}
...
int Mutant::frameNum( const StringPair& animID )
{
	if ((animID.first != "") && (animID.second != "") && (animations_.find(animID) != animations_.end()))
	{
		...
		if (animation)
		{
			return (int)((frameRate(animID) * animation->time) + 1);
		}
	}
	return 1;
}
...
int Mutant::numFrames( const StringPair& animID )
{
	if ((animID.first != "") && (animID.second != "") && (animations_.find(animID) != animations_.end()))
	{
		...
		{
			return (int)(frameRate(animID) * animation->pSource(*superModel_)->duration_);
		}
	}
	return 100;
}
...
void Mutant::animFile( const StringPair& animID, const std::string& animFile )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data ));
	UndoRedo::instance().barrier( "Changing the animation file of an animation", false );
...
void Mutant::animBoneWeight( const StringPair& animID, const std::string& boneName, float val )
{
	...
	std::string boneNameSanitised = SanitiseHelper::substringReplace
		(
			boneName
		);
	...
	else if (val == 1.f) return;

	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data ));
	UndoRedo::instance().barrier( "Changing the node weight of animation", false );
...
void Mutant::removeAnimNode( const StringPair& animID, const std::string& boneName )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, animations_[animID].data ));
	UndoRedo::instance().barrier( "Removing a blend value of an animation", false );

	//Remove the data section
	DataSectionPtr alpha = animations_[animID].data->openSection( "alpha" );
	if (alpha)
		alpha->delChild( boneName );
