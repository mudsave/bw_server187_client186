...
MaterialInfo::MaterialInfo(
	...
	std::set< Moo::EffectMaterial* > cEffect,
...
void Mutant::setDye( const std::string& matterName, const std::string& tintName, Moo::EffectMaterial** material  )
{
...
void Mutant::getMaterial( const std::string& materialName, std::set< Moo::EffectMaterial*>& material )
{
...
void Mutant::cleanMaterials()
{
	...
	for (; it != end; ++it)
	{
		...
		Moo::EffectMaterial* effect = *(it->second.effect.begin());
		...
		XMLSectionPtr materialData = new XMLSection("old state");
		...
		for ( uint32 num=0; num<effect->nEffects(); num++ )
		{
			ComObjectWrap<ID3DXEffect> pEffect = MaterialUtility::effect( effect, num );
			if ( pEffect == NULL )
				continue;

			Moo::EffectMaterial::Properties& properties = effect->properties( num );
...
void Mutant::cleanTints()
{
	...
	for(; dyeIt != dyeEnd; ++dyeIt)
	{
		...
		for(; tintIt != tintEnd; ++tintIt)
		{
			...
			Moo::EffectMaterial* effect = tintIt->second.effect;
...
bool Mutant::setTintProperty( const std::string& matterName, const std::string& tintName, const std::string& descName,  const std::string& uiName,  const std::string& propType, const std::string& val )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, materialData ));
	UndoRedo::instance().barrier( "Change to " + uiName, true );
...
bool Mutant::materialName( const std::string& materialName, const std::string& new_name )
{
	...
	if (pNameData != NULL)
	{
		...
		UndoRedo::instance().add( new UndoRedoOp( 0, pNameData ));
		UndoRedo::instance().barrier( "Changing a material name", true );
...
bool Mutant::matterName( const std::string& matterName, const std::string& new_name )
{
	if (matterName == new_name) return true;
	
	if (dyes_.find( new_name ) != dyes_.end()) return false;
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, dyes_[matterName] ));
	UndoRedo::instance().barrier( "Changing a dye name", true );
...
bool Mutant::tintName( const std::string& matterName, const std::string& tintName, const std::string& new_name )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, data ));
	UndoRedo::instance().barrier( "Changing a tint name", true );
...
std::string Mutant::newTint( const std::string& materialName, const std::string& matterName, const std::string& oldTintName, const std::string& newTintName, const std::string& fxFile, const std::string& mfmFile )
{
	...
	if (matterName == "")
	{
		UndoRedo::instance().add( new UndoRedoOp( 0, currModel_ ));
		UndoRedo::instance().barrier( "Adding a tint", true );
	...
	{
		data->copy( BWResource::openSection( mfmFile, false ) );
	}
...
void Mutant::deleteTint( const std::string& matterName, const std::string& tintName )
{
	...
	UndoRedo::instance().add( new UndoRedoOp( 0, currModel_ ));
	UndoRedo::instance().barrier( "Deleting a tint", true );
	...
	else
	{
		if (softskinned)
		{
			...
			::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
				"You are applying an unskinned shader to a softskinned object.\n"
				"This is not recommended since the rendering will be done in software rather than hardware.\n"
				"If possible use a softskinned shader instead.",
				"Applying an unskinned shader to a softskinned material", MB_OK );
		}
		else if (hardskinned)
		{
			WARNING_MSG("Applying an unskinned shader to a hardskinned object.\n");
			::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
				"You are applying an unskinned shader to a hardskinned object.\n"
				"This is not recommended since the rendering will be done in software rather than hardware.\n"
				"If possible use a hardskinned shader instead.",
				"Applying an unskinned shader to a hardskinned material", MB_OK );
		}
	}
	return true;
}
...
bool Mutant::materialShader( const std::string& materialName, const std::string& matterName, const std::string& tintName, const std::string& fxFile, bool undoable /* = true */ )
{
	if ((matterName == "") || (tintName == "Default"))
	{
		...
		if (tintName == "")
		{
			std::set< Moo::EffectMaterial* >::iterator matIt = materials_[materialName].effect.begin();
			std::set< Moo::EffectMaterial* >::iterator matEnd = materials_[materialName].effect.end();

			while (matIt != matEnd)
			{
				Moo::EffectMaterial* effect = *matIt++;
				effect->load( data );
			}
		}
	...
	if (undoable)
	{
		UndoRedo::instance().barrier( "Changing the material effect", true );
	}
...
bool Mutant::materialMFM( const std::string& materialName, const std::string& matterName, const std::string& tintName, const std::string& mfmFile, std::string* fxFile )
{
	...
	UndoRedo::instance().barrier( "Changing the material MFM", true );
...
void Mutant::tintFlag( const std::string& matterName, const std::string& tintName, const std::string& flagName, uint32 val )
{
	...
	UndoRedo::instance().barrier( "Changing a tint flag", true );
...
void Mutant::materialFlag( const std::string& materialName, const std::string& flagName, uint32 val )
{
	...
	for (unsigned i=0; i<materials_[materialName].data.size(); i++ )
	{
		...
		UndoRedo::instance().add( new UndoRedoOp( 0, data, true ));

		instantiateMFM( data );
		data->writeInt( flagName, val );
	}

	UndoRedo::instance().barrier( "Changing a material flag", true );
...
void Mutant::modelMaterial ( int id )
{
	...
	UndoRedo::instance().barrier( "Changing model material kind", false );
...
void Mutant::materialTextureFeedName( const std::string& materialName, const std::string& matterName, const std::string& tintName, const std::string& propName, const std::string& feedName )
{
	...
	UndoRedo::instance().barrier( "Changing the texture feed name", false );
...
void Mutant::toggleExposed( const std::string& matterName, const std::string& tintName, const std::string& propName )
{
	...
	while (propsIt != propsEnd)
	{
		...
		if (propName == prop->readString( "name", "" ))
		{
			...
			UndoRedo::instance().barrier( "Disabling the exposed to Python state", false );
	...
	UndoRedo::instance().barrier( "Enabling the exposed to Python state", false );
...
Vector4 Mutant::getExposedVector4( const std::string& matterName, const std::string& tintName, const std::string& descName,  const std::string& propType, const std::string& val )
{
	...
	{
		Moo::EffectMaterial* effect = tints_[matterName][tintName].effect;
		for ( uint32 num=0; num<effect->nEffects(); num++ )
		{
			ComObjectWrap<ID3DXEffect> pEffect = MaterialUtility::effect( effect, num );
			if ( pEffect == NULL )
...
Moo::EffectMaterial* Mutant::getEffectForTint(
