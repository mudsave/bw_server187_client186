...
void PageMaterials::drawMaterialsList()
{
	...
	//Need to scope the DelayRedraw since it interfers with EnsureVisible
	{
		...
		for (unsigned m=0; m<treeRoot->size(); m++)
		{
			...
			{
				sprintf( buf, "%s (%s)", materialDisplayName.c_str(), matterData->second.c_str() );
			}
			else
			{
				sprintf( buf, "%s", materialDisplayName.c_str() );
			}
			
			HTREEITEM material = pImpl_->materialTree.InsertItem( buf );
...
void PageMaterials::OnTvnSelchangedMaterialsTree(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	pImpl_->editor = new GeneralEditor();
	...
	std::set< Moo::EffectMaterial* > effects;
	...
	{
		pImpl_->fxList.SelectString(-1, "(Don't Render)");
	}
	...
	std::set< Moo::EffectMaterial* >::iterator materialIt = effects.begin();
	std::set< Moo::EffectMaterial* >::iterator materialEnd = effects.end();
	...
	pProp = new ChoiceProperty( "Collision Flags", cfp, collisionFlags );
	pProp->UIDesc( "The Collision flag used for the material" );
	...
	pProp = new ChoiceProperty( "Material Kind", mkp, materialKinds );
	pProp->UIDesc( "The primary material kind of the material" );
	pImpl_->editor->addProperty( pProp );
    ...
    {
        std::vector<Moo::EffectMaterial*> mats;
        int count = pImpl_->previewObject->collateOriginalMaterials( mats );
        if ( count > 0 )
        {
            Moo::EffectMaterial* mat = mats[0];
	...
	while (materialIt != materialEnd)
	{
		...
		for ( uint32 i=0; i<(*materialIt)->nEffects(); i++ )
		{
			ComObjectWrap<ID3DXEffect> pEffect = MaterialUtility::effect((*materialIt),i);
			if ( pEffect == NULL )
				continue;

			Moo::EffectMaterial::Properties& properties = (*materialIt)->properties(i);
					...
					if ( SUCCEEDED(hr) )
					{
							...
							desc.Type == D3DXPT_TEXTURECUBE ) )
						{
							...
							else
							{
								...
								if (UIWidget != "CubeMap")
								{
									pProp->fileFilter( "Texture files(*.bmp;*.tga;*.jpg;*.dds;*.texanim)|*.bmp;*.tga;*.jpg;*.dds;*.texanim|"
...
Moo::EffectMaterial* PageMaterials::currMaterial()
{
...
void PageMaterials::tintNew()
{
	...
	{
		pImpl_->matterName = MeApp::instance().mutant()->newTint( pImpl_->materialName, pImpl_->matterName, pImpl_->tintName,
...
void PageMaterials::mfmLoad()
{
	...
	if ( fileDlg.DoModal() == IDOK )
	{
		...
		if (BWResource::validPath( fileName ))
		{
			...
			else
			{
				::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
					"The file selected cannot be used for a material.\nPlease select either an effect (*.fx) or an MFM (*.mfm) file.",
		...
		{
			::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
				"You can only load material files from one of the resource folders.",
				"Unable to resolve material file path",
				MB_OK | MB_ICONWARNING );
		}
	}
...
void PageMaterials::mfmSave()
{
	...
	if ( fileDlg.DoModal() == IDOK )
	{
		...
		if (BWResource::validPath( mfmName ))
		{
			...
			{
				::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
					"An error occured while trying to save the MFM.",
					"Failed to Save MFM",
					MB_OK | MB_ICONWARNING );
			}
		}
		...
		{
			::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
				"You can only save an MFM into one of the resource folders.",
				"Unable to resolve MFM path",
				MB_OK | MB_ICONWARNING );
		}
	}
}
...
void PageMaterials::OnEnChangeMaterialsMaterial()
{
	...
	char buf[256];
	if (pImpl_->matterName != "")
		sprintf( buf, "%s (%s)", new_name, pImpl_->matterName.c_str() );
	else
		sprintf( buf, "%s", new_name );

	pImpl_->materialTree.SetItemText( pImpl_->selParent, buf );
}
...
void PageMaterials::OnEnKillfocusMaterialsMaterial()
{
	...
	else
	{
		char buf[256];
		if (pImpl_->matterName != "")
			sprintf( buf, "%s (%s)", pImpl_->materialDisplayName.c_str(), pImpl_->matterName.c_str() );
		else
			sprintf( buf, "%s", pImpl_->materialDisplayName.c_str() );
	
		pImpl_->materialTree.SetItemText( pImpl_->selParent, buf );
		pImpl_->material.SetWindowText( pImpl_->materialDisplayName.c_str() );
		
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			"Unable to rename the material since no name was given.\nAt least one alphanumeric character is needed.",
			"Invalid material name given", MB_OK | MB_ICONERROR );
	...
	else
	{
		...
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			"Unable to rename the material since there is already a material with the same name.",
			"Material name already used", MB_OK | MB_ICONERROR );
	}
}
...
void PageMaterials::OnEnChangeMaterialsMatter()
{
	...
	char buf[256];
	sprintf( buf, "%s (%s)", pImpl_->materialDisplayName.c_str(), new_name.c_str() );
	pImpl_->materialTree.SetItemText( pImpl_->selParent, buf );
...
void PageMaterials::OnEnKillfocusMaterialsMatter()
{
	...
	if (new_name == "")
	{
		...
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			"Unable to rename the dye since no name was given.\nAt least one alphanumeric character is needed.",
			"Invalid dye name given", MB_OK | MB_ICONERROR );
	}
	...
	else
	{
		...
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			"Unable to rename the dye since there is already a dye with the same name.",
			"Dye name already used", MB_OK | MB_ICONERROR );
	}
}
...
void PageMaterials::OnEnKillfocusMaterialsTint()
{
	...
	else
	{
		...
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			"Unable to rename the tint since no name was given.\nAt least one alphanumeric character is needed.",
			"Invalid tint name given", MB_OK | MB_ICONERROR );
	...
	else
	{
		...
		::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
			"Unable to rename the tint since there is already a tint with the same name.",
			"Tint name already used", MB_OK | MB_ICONERROR );
	}
}
...
void PageMaterials::redrawList( CComboBox& list, const std::string& name, bool sel )
{
	...
	list.InsertString( data.size(), "(Don't Render)" );
	list.InsertString( data.size() + 1, "(Other...)" );
...
void PageMaterials::fillTechniqueList()
{
	...
	/* 
	static Moo::EffectMaterial* lastMaterial = NULL;
...
void PageMaterials::OnBnClickedMaterialsFxSel()
{
	...
	if ( fileDlg.DoModal() == IDOK )
	{
		...
		{
			::MessageBox( AfxGetApp()->m_pMainWnd->GetSafeHwnd(),
				"You can only select an effect file located in one of the resource folders.",
				"Unable to resolve effect file path",
				MB_OK | MB_ICONWARNING );
		}
	}
}
