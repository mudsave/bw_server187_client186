...
void PageAnimations::updateCompression()
{
    ...
    {
        Moo::AnimationChannelPtr channel = anim->channelBinder( i ).channel();
		Moo::NodePtr node = Model::findNodeInCatalogue( channel->identifier().c_str() );
	...
	if (opk > 0)
	{
		sprintf( buf, "%.0f %%", 100.f*(1.f - ((float)tpk / (float)opk)) );
	}
	else
	{
		buf[0] = 0;
	...
	if (ork > 0)
	{
		sprintf( buf, "%.0f %%", 100.f*(1.f - ((float)trk / (float)ork)) );
    }
	else
	{
		buf[0] = 0;
	...
	if (osk > 0)
	{
		sprintf( buf, "%.0f %%", 100.f*(1.f - ((float)tsk / (float)osk)) );
    }
	else
	{
		buf[0] = 0;
	...
	sprintf( buf, 
		...
		100.f * (1.f - ((float)compressedSize / (float)origSize )));
	pImpl_->compTotal.SetWindowText( buf );
}
...
{
	return !!MeApp::instance().mutant()->getMooAnim( selID() );
}
