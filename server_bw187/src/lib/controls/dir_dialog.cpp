										  ...
										  IN LPCITEMIDLIST pidlItem )
{
	...
	CString strResult;
	strResult = name.pOleStr;

    /*
	//Convert the STRRET to a CString
	switch (name.uType)
    {
    case STRRET_WSTR:
        strResult = name.pOleStr;
	...
	*/
	...
	//Run thorugh the possible paths
	for (unsigned i=0; i<DirDialog::paths().size(); i++)
	{
		CString test = DirDialog::paths()[i];

		//Firstly check whether the directory is a child of a path folder
		if ( strResult.Find( test ) != -1 )
		{
			return ( S_OK );
		}

		//Now check if it is a parent
		while (strcmp(test,"") != 0)
		{
			test.TrimRight('\\');
			test.Delete( test.ReverseFind('\\') + 1, test.GetLength() );
			if ( strcmp(test,strResult) == 0 )
			{
				return ( S_OK );
			}
		}
...
// Callback function called by SHBrowseForFolder's browse control after initialization and when selection changes
...
DirDialog::DirDialog()
{
	...
	if (s_basePath_.Compare("") == 0) // If the base path has not yet been set, set it...
	{
		...
		defaultPath.Delete( defaultPath.ReverseFind('/'), defaultPath.GetLength() );
