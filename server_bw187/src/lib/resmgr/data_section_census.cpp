...
memoryCounterDeclare( dSectCache );
...
DataSectionPtr DSCStuff::add( const std::string & id, DataSectionPtr pSect )
{
	...
	if (found != strMap.end())
	{
		...
		memoryClaim( found2->second );
		memoryClaim( ptrMap, found2 );
		ptrMap.erase( found2 );

		memoryClaim( found->first );
		memoryClaim( strMap, found );
		strMap.erase( found );
#endif
	}
	...
	memoryClaim( sit->first );
	memoryClaim( strMap, sit );
	DSPointerMap::iterator pit = ptrMap.insert(
		std::make_pair( pSect.getObject(), id ) ).first;
	memoryClaim( pit->second );
	memoryClaim( ptrMap, pit );
#endif
...
void DSCStuff::del( DataSection * pSect )
{
	...
	if (found != ptrMap.end())
	{
		...
		memoryClaim( found2->first );
		memoryClaim( strMap, found2 );
		strMap.erase( found2 );

		memoryClaim( found->second );
		memoryClaim( ptrMap, found );
		ptrMap.erase( found );
#endif
	}
}
