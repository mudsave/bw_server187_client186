								...
								bool makeNewSection )
{
	...
	if( 0 <= pos && pos < (int)tagPath.size() )
	{
		...
		return pChild->openSection(tagPath.substr(pos + 1), makeNewSection);
	}
	...
	if(!pChild && makeNewSection)
		pChild = this->newSection(tagPath);
		...
		std::vector<DataSectionPtr> &dest )
{
	...
	if( 0 <= pos && pos < (int)tagPath.size() )
	{
		...
		if(pChild)
		{
			pChild->openSections( tagPath.substr( pos + 1 ), dest );
		}
	}
	else
	{
...
void DataSection::writeInts( const std::string &tagPath,
		std::vector<int> &src, int /*flags*/, char* format)
{
...
void DataSection::writeBools( const std::string &tagPath,
		std::vector<bool> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeLongs( const std::string &tagPath,
		std::vector<long> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeFloats( const std::string &tagPath,
		std::vector<float> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeDoubles( const std::string &tagPath,
		std::vector<double> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeVector2s( const std::string &tagPath,
		std::vector<Vector2> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeVector3s( const std::string &tagPath,
		std::vector<Vector3> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeVector4s( const std::string &tagPath,
		std::vector<Vector4> &src, int /*flags*/, char* format)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
void DataSection::writeMatrix34s( const std::string &tagPath,
		std::avector<Matrix> &src, int /*flags*/, char* vectorFormat)
{
	DataSectionPtr pSection, pNewSection;
	std::string tag;
...
bool DataSection::setBool( bool /*value*/, char * /*format*/ )
{
	return false;
}

bool DataSection::setInt( int /*value*/, char * /*format*/ )
{
	return false;
}

bool DataSection::setLong( long /*value*/, char * /*format*/ )
{
	return false;
}

bool DataSection::setFloat( float /*value*/, char * /*format*/ )
{
	return false;
}

bool DataSection::setDouble( double /*value*/, char * /*format*/ )
{
	return false;
}
...
bool DataSection::setVector2( const Vector2 & /*value*/, char * /*format*/ )
{
	return false;
}

bool DataSection::setVector3( const Vector3 & /*value*/, char * /*format*/ )
{
	return false;
}

bool DataSection::setVector4( const Vector4 & /*value*/, char * /*format*/ )
{
	return false;
}
...
DataSectionPtr DataSection::splitTagPath(const std::string& tagPath,
		std::string& tag, bool makeNewSection)
{
	DataSectionPtr pSection;
	int pos;
	...
	pSection = this->openSection(tagPath.substr(0, pos), makeNewSection);
	...
	const std::string& tag, BinaryPtr pData, bool allowModifyData /*=true*/ )
{
	if (pData->len() > 0)
	{
		...
		{
			//g_longTime[0] = 0;
			//DiaryEntry & de = Diary::instance().add( "bin" );
			if ( !allowModifyData )
			{
...
void DataSection::setWatcherValues( std::string path )
{
	...
	bool succeeded = Watcher::rootWatcher().setFromString( NULL, path.c_str(),
		this->asString().c_str() );

	if (!succeeded && !this->asString().empty())
	{
		WARNING_MSG( "DataSection::setWatcherValues: Failed to set %s to %s\n",
				path.c_str(), this->asString().c_str() );
	}
#endif
}
