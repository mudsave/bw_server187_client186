 ...
 *	@param dataSection	Reference to the cache object to use
 *
 *	@return 			None
 */
DirSection::DirSection(const std::string& fullPath, IFileSystem* pFileSystem) :
	fullPath_(fullPath),
	tag_("root"),
...
DataSectionPtr DirSection::newSection( const std::string & tag )
{
	...
	if(pos >= 0 && pos < (int)tag.size())
	{
		XMLSection * pSection = new XMLSection( tag );
		pSection->setParent( this );
		DataSectionCache::instance()->add( fullPath, pSection );
	}
	else
	{
		if(pFileSystem_->makeDirectory( fullPath ))
		{
			DataSectionCache::instance()->add(fullPath, new DirSection(fullPath, pFileSystem_));
			children_.push_back(tag);
		}
	}

	return this->findChild(tag);
}
 ...
 */
DataSectionPtr DirSection::findChild( const std::string &tag ) const
{
	std::string fullName;
	DataSectionPtr pSection;
	...
	switch(pFileSystem_->getFileType(fsFullName))
	{
		...
		case IFileSystem::FT_FILE:
		{
			...
			if (pBinary)
			{
				...
				if (pSection)
					pSection->setParent( const_cast<DirSection*>( this ) );
			}
			break;
		}
