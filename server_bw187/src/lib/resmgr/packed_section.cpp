...
public:
	...
	{
		list_.push_back( new BinaryBlock( ptr, size ) );
		size_ += size;
	}
...
{
	static DataSectionPtr pXMLSection = new XMLSection( "" );
...
namespace
{
MemoryExchange<PackedSection> s_exchange( 256, 256 );

THREADLOCAL(PackedSection *) s_pSectionBlock;
THREADLOCAL(int) s_sectionBlockUsed;

THREADLOCAL(PackedSection *) s_pFreeList;
 ...
 */
void * PackedSection::operator new( size_t count )
{
#ifdef LDLINUX_TLS_IS_BROKEN
	PackedSection * pFreeList = s_pFreeList;
	PackedSection * pSectionBlock = s_pSectionBlock;
 ...
 */
void PackedSection::operator delete( void * addr )
{
#ifdef LDLINUX_TLS_IS_BROKEN
	PackedSection * pFreeList = s_pFreeList;
	int freeListSize = s_freeListSize;
...
{
	BinaryPtr pEncrypted = new BinaryBlock( NULL, pData->len() + 1 );
	...
	pFile_( pFile )
{
	if (ownDataType_ == TYPE_DATA_SECTION)
	{
		...
		int numChildren = const_cast<PackedSection *>(this)->countChildren();
...
DataSectionPtr PackedSection::openChild( int index )
{
	...
	if (this->isMatrix() &&
		0 <= index && index < 4)
	{
		static char * pRow[] = { "row0", "row1", "row2", "row3" };
 ...
 *	@return 		NULL
 */
DataSectionPtr PackedSection::findChild( const std::string & tag ) const
{
	int numChildren = const_cast<PackedSection *>( this )->countChildren();
...
{
	if (ownDataType_ == TYPE_BLOB)
		return new BinaryBlock( pOwnData_, ownDataLen_, pFile_->pFileData() );
...
bool convertToPackedFile( DataSectionPtr pDS, OutputWriter & output )
{
	...
	std::list< std::string > path;
	MF_VERIFY( writeAsPackedSection( stringTable, pDS, output, path ) ==
			TYPE_DATA_SECTION );
...
DataSectionPtr PackedSection::openDataSection( const std::string & path )
{
	...
	BinaryPtr pData = new BinaryBlock( NULL, len );
