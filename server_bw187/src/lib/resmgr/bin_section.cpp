...
DataSectionPtr BinSection::newSection( const std::string & tag )
{
	...
	DataSectionPtr pChild = new BinSection( tag, new BinaryBlock( NULL, 0 ) );
 ...
 *	@return 		NULL
 */
DataSectionPtr BinSection::findChild( const std::string & tag ) const
{
	if (!introspected_) const_cast<BinSection*>(this)->introspect();
...
void BinSection::delChildren()
{
	...
	binaryData_ = new BinaryBlock( NULL, 0 );
}
...
void BinSection::introspect()
{
	...
	while (offset <= len - (int)(sizeof(int) + sizeof(int)*2))
	{
		...
		children_.push_back( DataSection::createAppropriateSection(
			entryStr,
			new BinaryBlock(
				data+entryDataOffset, entryDataLen, binaryData_ ),
...
BinaryPtr BinSection::recollect()
{
	...
	BinaryPtr pBinary = new BinaryBlock( NULL, bsz );
