...
// Different caseless compare functions across platforms.
#ifdef _WIN32
#define caselessCompare	_stricmp
#else
#define caselessCompare	strcasecmp
#endif
...
#include "resmgr/memory_exchange.hpp"


namespace
{
MemoryExchange<XMLSection> s_exchange( 256, 1024 );

THREADLOCAL(XMLSection *) s_pSectionBlock;
THREADLOCAL(int) s_sectionBlockUsed;

THREADLOCAL(XMLSection *) s_pFreeList;
THREADLOCAL(int) s_freeListSize;
}


/**
 *	Operator new. Takes head of free list or allocates a new block.
 */
void * XMLSection::operator new( size_t count )
{
#ifdef LDLINUX_TLS_IS_BROKEN
	XMLSection * pFreeList = s_pFreeList;
	XMLSection * pSectionBlock = s_pSectionBlock;
	int freeListSize = s_freeListSize;
	int sectionBlockUsed = s_sectionBlockUsed;

	void * pResult =  s_exchange.getMemory( pFreeList, freeListSize,
			pSectionBlock, sectionBlockUsed );

	s_pFreeList = pFreeList;
	s_pSectionBlock = pSectionBlock;
	s_freeListSize = freeListSize;
	s_sectionBlockUsed = sectionBlockUsed;

	return pResult;
#else
	return s_exchange.getMemory( s_pFreeList, s_freeListSize,
			s_pSectionBlock, s_sectionBlockUsed );
#endif
}


/**
 *	Operator delete. Adds to free list
 */
void XMLSection::operator delete( void * addr )
{
#ifdef LDLINUX_TLS_IS_BROKEN
	XMLSection * pFreeList = s_pFreeList;
	int freeListSize = s_freeListSize;

	s_exchange.addToFreeList( addr, pFreeList, freeListSize );

	s_pFreeList = pFreeList;
	s_freeListSize = freeListSize;
#else
	s_exchange.addToFreeList( addr, s_pFreeList, s_freeListSize );
#endif
}
...
DataSectionPtr XMLSection::openChild( int index )
{
	...
	{
		ERROR_MSG( "XMLSection::openChild: Invalid index %d. Children = %d\n",
 ...
 * with the input tag.
 */
DataSectionPtr XMLSection::findChild( const std::string & tag ) const
{
...
bool XMLSection::asBool( bool defaultVal )
{
	...
	if ( !caselessCompare( readValue.c_str(),"true" ) )
	{
		return true;
	}
	if ( !caselessCompare( readValue.c_str(),"false" ) )
	{
		return false;
	}
...
BinaryPtr XMLSection::asBinary()
{
	...
	BinaryPtr pBinary = new BinaryBlock(stream, stream.tellp());
 ...
 */
bool XMLSection::setBool( bool value, char *format )
{
	char formattedString[512];

	// TODO:PM Does it make sense for these to have a format string?
	sprintf( formattedString, format, value ? "true" : "false" );
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setInt( int value, char *format )
{
	char formattedString[512];
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setLong( long value, char *format )
{
	char formattedString[512];
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setFloat( float value, char *format )
{
	char formattedString[512];
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setDouble( double value, char *format )
{
	char formattedString[512];
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setVector2( const Vector2 &value, char *format )
{
	char formattedString[512];

	sprintf( formattedString, format, value.x, value.y );
	return this->setString( formattedString );
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setVector3( const Vector3 &value, char *format )
{
	char formattedString[512];

	sprintf( formattedString, format, value.x, value.y, value.z );
	return this->setString( formattedString );
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setVector4( const Vector4 &value, char *format )
{
	char formattedString[512];

	sprintf( formattedString, format, value[0], value[1], value[2], value[3] );
	return this->setString( formattedString );
}
 ...
 *	@return	True if it was successful, False otherwise.
 */
bool XMLSection::setMatrix34( const Matrix &value, char *vectorFormat )
{
	bool good = true;
...
{
	int bufnow = astream.tellg();
	astream.seekg( 0, ios_base::end );
	int buflen = int(astream.tellg()) - bufnow;
	astream.seekg( bufnow, ios_base::beg );

	BinaryPtr pBlock = new BinaryBlock( astream, buflen );
	...
	const std::string & rootTag, BinaryPtr pBlock )
{
	...
	if (s_parseStack == NULL) s_parseStack = new XMLParseStack();
	s_parseStack->clear();
	if (s_parseKids == NULL) s_parseKids = new XMLParseStack();
	s_parseKids->clear();
...
bool XMLSection::writeToStream( std::ostream & stream, int indentation ) const
{
	...
	stream << '<' << ( indentation == 0 ? "root" : this->sectionName() ) << '>';
	...
	stream << "</" << ( indentation == 0 ? "root" : this->sectionName() ) << '>' << endl;
...
#ifdef _WIN32
		wsprintfW( buf, L"%.256S", val.c_str() );
#else
