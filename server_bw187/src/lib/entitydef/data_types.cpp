...
public:
	...
	PyObject * index( PyObjectPtr pObject );
	PY_AUTO_METHOD_DECLARE( RETOWN, index, ARG( PyObjectPtr, END ) )
...
protected:
	...
	{
		MF_ASSERT( PyArrayDataInstance::Check( pObject ) );
...
void PyArrayDataInstance::setInitialSequence( PyObject * pSeq )
{
	...
	MF_ASSERT( pDataType_->getSize() == 0 || pDataType_->getSize() == sz );
	MF_ASSERT( uint(sz) == values_.size() );
...
int PyArrayDataInstance::pySeq_ass_item( int index, PyObject * value )
{
	...
	else
	{
		...
		// We need to detach and re-attach the values after the deletion
		// because the ownerRefs will be shifted..
		for (size_t i = index; i < values_.size(); ++i)
		{
			PyObjectPtr value = values_[i];
			dataType.detach( value.get() );
			dataType.attach( value.get(), this, /*ownerRef=*/ i );
		}
	...
	PyObject * pOther )
{
	...
	{
		void deleteElements( PyArrayDataInstance & owner, 
				...
				int indexA, int indexB, DataType& elementType )
		{
			...
			for (size_t i = indexA; i < values.size(); ++i)
			{
				PyObjectPtr value = values[i];
				elementType.detach( values[i].get() );
				elementType.attach( values[i].get(), &owner,
					/*ownerRef=*/ i );
			}
		}
	...
	if (!pOther)
	{
		...
		helper.deleteElements( *this, values_, indexA, indexB,
	...
	helper.deleteElements( *this, values_, indexA, indexB, dataType );
	...
	for (size_t i = indexA + osz; i < values_.size(); ++i)
	{
		PyObjectPtr value = values_[i];
		dataType.detach( values_[i].get() );
		dataType.attach( values_[i].get(), this,
			/*ownerRef=*/ i );
	}
 ...
 */
PyObject * PyArrayDataInstance::index( PyObjectPtr pObject )
{
...
void ClassDataType::detach( PyObject * pObject )
{
	...
	MF_ASSERT( PyClassDataInstance::Check( pObject ) );
...
{
	MF_ASSERT( pDataType_ == other.pDataType_ );
...
public:
	...
	static int pyGetLength( PyObject* self );
 ...
 */
int PyFixedDictDataInstance::pyGetLength( PyObject* self )
{
...
/**
 *	Gets the value keyed by given key.
 */
...
FixedDictDataType::~FixedDictDataType()
{
	...
	// before Script::fini.
	MF_ASSERT( !pImplementor_ || !Script::isFinalised() );
}
...
void FixedDictDataType::detach( PyObject * pObject )
{
	...
	else
	{
		...
		// already been called.
		MF_ASSERT( PyFixedDictDataInstance::Check( pObject ) ||
					!Py_IsInitialized() );
...
UserDataType::~UserDataType()
{
	...
	// EntityDescription is destructed or EntityDescription::clear called on it
	// before Script::fini.
	MF_ASSERT( !pObject_ || !Script::isFinalised() );
}
