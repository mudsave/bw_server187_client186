...
bool MethodDescription::parseReturnValues( DataSectionPtr pSection )
{
	...
	while (iter != pSection->end())
	{
		...
		returnValues_.push_back(MethodDescription::ReturnValue(
			pDS->sectionName(), pDataType));
	...
	Mercury::Nub* pNub) const
{
	...
	// if we have return values, create and add a MethodResponse object to the args
	if (returnValues_.size() > 0)
	{
		if (replyID == -1)
		{
			// treat as one way by passing None to the function as the response object
			Py_INCREF( Py_None );
			PyTuple_SET_ITEM( pTuple, argAt++, Py_None );
		}
		else
		{
			PyObject* responseObj = new MethodResponse(
				replyID, *pReplyAddr, *pNub, *this);
	...
	{
		WARNING_MSG( "MethodDescription::getArgsAsTuple: CHEAT: "
