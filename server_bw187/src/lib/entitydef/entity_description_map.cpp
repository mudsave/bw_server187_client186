...
namespace // anonymous
{
std::vector< EntityDescriptionMap * >& s_mapCollection = *new std::vector< EntityDescriptionMap*>;
} // anonymous
...
EntityDescriptionMap::EntityDescriptionMap()
{
	...
	s_mapCollection.push_back( this );
}
...
{
	// NOTE: Not thread safe.
	s_mapCollection.erase(
			std::find( s_mapCollection.begin(), s_mapCollection.end(), this ) );
}
 ...
 *	method limits.
 */
bool EntityDescriptionMap::checkCount( char * description,
...
{
	MF_ASSERT( index < EntityTypeID(vector_.size()) );
