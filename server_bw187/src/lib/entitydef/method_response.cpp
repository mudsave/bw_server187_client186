...
void BlockingResponseHandler::handleMessage(
	...
	void* args)
{
	...
	if (err_ == Mercury::REASON_SUCCESS)
	{
 ...
 *	@param pType A pointer to the Python type.
 */
	...
	returnValueData_()
{
	...
	if (numReturnValues != returnValueData_.size())
	{
		ERROR_MSG("MethodResponse::MethodResponse(): "
			"Method description reports %d return values, but %d "
...
/**
 * Returns a borrowed reference to the value for attribute identified by name,
