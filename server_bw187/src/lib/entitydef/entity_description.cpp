...
{
	IFileSystem * pFS = BWResource::instance().fileSystem();
		...
		DataSectionPtr pSection, bool isFinal )
{
	if (!pSection)
	{
		std::string filename = "entities/defs/" + name + ".def";
...
#ifdef MF_SERVER
	hasCellScript_ = pythonScriptExists( "entities/cell/" + name_ );
	hasBaseScript_ = pythonScriptExists( "entities/base/" + name_ );
#else // not MF_SERVER
	// In the client, don't check for existence of base and cell script files,
	// just assume entities have cell and base scripts.
	hasCellScript_ = true;
	hasBaseScript_ = true;
#endif // MF_SERVER
	hasClientScript_ = pythonScriptExists( "entities/client/" + clientName_ );
	...
	if (pInterfaces)
	{
		...
		while (iter != pInterfaces->end())
		{
			...
			DataSectionPtr pInterface = BWResource::openSection(
					"entities/defs/interfaces/" + interfaceName + ".def" );
 ...
 */
void EntityDescription::addToDictionary( DataSectionPtr pSection,
	PyObject* pDict ) const
{
	...
	for(it = pSection->begin(); it != pSection->end(); it++)
	{
		...
		DataDescription* pDesc = this->findProperty(pDS->sectionName());

		if(!pDesc)
		{
			WARNING_MSG("EntityDescription %s has no property %s\n",
				this->name().c_str(),
				pDS->sectionName().c_str());
			continue;
		}

		PyObjectPtr pValue = pDesc->createFromSection( pDS );
...
bool EntityDescription::parseCellMethods( DataSectionPtr pMethods )
{
	...
	return cell_.init( pMethods, MethodDescription::CELL );
}
...
bool EntityDescription::parseBaseMethods( DataSectionPtr pMethods )
{
	...
	return base_.init( pMethods, MethodDescription::BASE );
}
		...
		int numDataSizes ) const
{
	...
	MF_ASSERT( (numDataSizes == 0) || (numDataSizes == actualPass) ||
 ...
 */
unsigned int EntityDescription::propertyCount() const
{
 ...
 */
DataDescription* EntityDescription::property( unsigned int n ) const
{
	MF_ASSERT(n < properties_.size());
 ...
 */
DataDescription*
EntityDescription::findProperty( const std::string& name ) const
{
...
{
	MF_ASSERT(n < clientServerProperties_.size());
...
bool DataLoDLevels::addLevels( DataSectionPtr pSection )
{
	...
	{
		MF_ASSERT( size_ <= int(sizeof( level_ )/sizeof( DataLoDLevel)) );
