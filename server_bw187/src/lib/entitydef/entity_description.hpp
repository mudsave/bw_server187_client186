...
public:
	...
	inline bool	readStream( BinaryIStream & stream,
				int dataDomains,
				PyObject * pDest ) const
	{
		return this->readStreamToDict( stream, dataDomains, pDest );
	}
	...
	DataDescription*		findProperty( const std::string& name ) const;
	...
	unsigned int			numEventStampedProperties() const;
...
//											bool isForServer );
	...
	/// Stores all properties associated with this entity type in order of their
	/// index.
