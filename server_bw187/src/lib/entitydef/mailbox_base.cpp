...
//#define CHECK_CLIENT_METHOD_CALL_LENGTH
...
PyObject * RemoteEntityMethod::pyCall( PyObject * args )
{
	...
	int limited = 0;
#ifdef CHECK_CLIENT_METHOD_CALL_LENGTH
	EntityMailBoxRef ref = PyEntityMailBox::reduceToRef( pMailBox_.getObject() );
	if (ref.component() == EntityMailBoxRef::CLIENT)
		limited = 512;
#endif
	...
	{
		MemoryOStream temp( 65536 );
		pMethodDescription_->addToStream( true, args, temp );
		if (temp.size() > limited)
		{
			PyErr_Format( PyExc_ValueError,
				"Message call to client method %s size %d exceeds limit of %d",
				pMethodDescription_->name().c_str(), temp.size(), limited );
			return NULL;
		}
		pBOS->transfer( temp, temp.size() );
	}
...
PyObject * PyEntityMailBox::pyRepr()
{
	...
	return PyString_FromFormat( "%s mailbox id: %ld type: %d addr: %s",
