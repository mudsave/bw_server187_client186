...
/**
 * Helper class for blocking returnvalue reply message handling.
...
public:
	...
	/**
	 * Returns the dictionary object, or NULL if the reply has not been received.
	 */
...
/**
 * This class is a helper class used to supply return values back to remote callers.
 * It has attributes that correspond to return values described by the
 * MethodDescription.
 *
 * @ingroup entity
 */
...
public:
	MethodResponse(int replyID,
		...
		Mercury::Nub& nub,
		const MethodDescription& methodDesc,
		PyTypeObject* pType=&MethodResponse::s_type_);
