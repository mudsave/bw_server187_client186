		...
		const DataDescription & dataDescription, EventNumber eventNumber )
{
	...
	MF_ASSERT( 0 <= index && index < (int)eventStamps_.size() );
...
/**
 *	This method returns the name of this entity description.
 */
INLINE const std::string& EntityDescription::name() const
{
	return name_;
}
