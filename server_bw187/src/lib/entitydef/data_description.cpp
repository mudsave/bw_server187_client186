...
bool DataType::initAliases()
{
	...
	DataSectionPtr pAliases =
		BWResource::openSection( "entities/defs/alias.xml" );
	...
	{
		WARNING_MSG( "Couldn't open entities/defs/alias.xml\n" );
	}
...
void DataType::clearStaticsForReload()
{
	...
	MF_ASSERT( s_singletonMap_ == NULL );
}
...
#ifdef MF_SERVER_DBMGR 
    /* huangshanquan  2009-12-28 add begin CSOL-7399*/
    std::string indextype = pSection->readString( "IndexType" ).c_str();
    if(indextype == "Unique")
    {
        IndexType_ = 4;
    }
