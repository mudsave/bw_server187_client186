 ...
 */
bool SpeedTreeThumbProv::isValid(const std::string& file)
{
	if (file.empty())
		return false;
 ...
 */
bool SpeedTreeThumbProv::prepare(const std::string& file)
{
	bool result = false;
	try
	{
		...
		tree_->load(resFile .c_str(), 1, NULL);
 ...
 */
bool SpeedTreeThumbProv::render(const std::string& file, Moo::RenderTarget* rt)
{
	if ( !tree_ )
		return false;
