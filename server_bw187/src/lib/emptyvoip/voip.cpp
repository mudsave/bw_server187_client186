...
/**
 *	This class provides a simple example implementation of the VOIPDataDict
 *	interface defined in voip.hpp. Internally it uses an STL map of string
 *	pairs and implements the interface to access this structure. The use of
 *	(VOIPDataDicts is expensive and so should be used very sparingly.\n
 *
 *	Example usage:\n
 *	@code
 * void VOIPClient::command( const VOIPDataDict * data )
 * {
 *     SimpleVOIPDataDict dd;
 *
 *     dd.dict_.insert( std::make_pair( std::string( "someKey" ), std::string( "someData" ) ) );
 *
 *     if( response_ )
 *         response_->response( 15, &dd );
 * }
 *	@endcode
 */
