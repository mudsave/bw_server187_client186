...
void DistortionMesh::createIndexBuffer()
{
	...
	HRESULT hr = Moo::rc().device()->CreateIndexBuffer( 
		nIndices_, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16,
		D3DPOOL_DEFAULT, &ib, NULL );
		
	if (SUCCEEDED(hr))
	{
		uint16* pIndices = NULL;
		if (SUCCEEDED(ib->Lock( 0, nIndices_, (VOID**)&pIndices, 0 )))
		{
			uint16 lastIndex = 0;
			for (int y = 0; y < nIndicesY_; y++)
			{
				...
				*(pIndices++) = lastIndex;
				*(pIndices++) = rowIndex + nVertsX_;
				for (int x = 0; x < nVertsX_; x++)
				{
					lastIndex = x + rowIndex;
					*(pIndices++) = lastIndex + nVertsX_;
					*(pIndices++) = lastIndex;
				}
			}
...
{
	Moo::rc().setIndices( indexBuffer().pComObject() );
}
