...
void Rain::tick( float dt )
{
	if ( rainSplashes_ != NULL )
	{
		...
		//
		// NOTE: Rain now is held by the PlayerAvatar's model. This means we
		// can no longer call its tick() or draw() methods.
		//
...
//	Moo::rc().setRenderState(D3DRS_CLIPPING, TRUE );
	...
	for ( int i = 0; i < numRainBits_; i++ )
	{
		...
		int illum = rand() % 64 + currIllumination;
		...
		verts_[2].uv_.x = particleScale;	verts_[2].uv_.y = 0.f;
		verts_[3].uv_.x = particleScale;	verts_[3].uv_.y = particleScale;		
...
void Rain::buildMaterial( void )
{
	...
	ts.pTexture( Moo::TextureManager::instance()->get( s_rainBmpName ) );
...
void Rain::update( const struct WeatherSettings & ws, bool outside )
{
	...
	wind_ = Vector3( ws.windX, 0, ws.windY ) / 50.f;
