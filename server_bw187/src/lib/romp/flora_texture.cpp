...
void FloraTexture::createUnmanagedObjects()
{
	...
	if ( textureMemory_ == 0 )
	{
		...
		HRESULT hr = Moo::rc().device()->CreateTexture( width_, height_,  smallMap_.numMipMaps(), 
			0, D3DFMT_DXT3, D3DPOOL_DEFAULT, &pTexture_, NULL );
		if ( SUCCEEDED( hr ) )
		{
			DX::Texture* sysCopy;
			if( SUCCEEDED( Moo::rc().device()->CreateTexture( width_, height_, smallMap_.numMipMaps(), 0, 
		...
		else
		{
			if (hr == D3DERR_INVALIDCALL)
				DEBUG_MSG( "FloraTexture::createUnmanagedObjects - could not create texture map ( invalid call )\n");
			else
				DEBUG_MSG( "FloraTexture::createUnmanagedObjects - could not create texture map ( error %lx )\n", hr );
		}
		
		textureSetter_ = new FloraTextureSetter( pTexture_ );
...
void FloraTexture::swapInBlock( int idx, Moo::BaseTexturePtr pSourceTexture )
{
	...
	{
		char buf[256];
		sprintf( buf, "C:\\floraTex%d.dds", s_shotID++ );
		D3DXSaveTextureToFile( buf, D3DXIFF_DDS, pTexture_, NULL );
		s_save = false;
	}
	...
	while ( numLevels )
	{
		...
		// we always divide by 2 (unlike the xbox version) because if we don't, they we get
...
void FloraTexture::drawDebug()
{
	...
	Moo::rc().setTexture( 0, this->pTexture_ );
 ...
 *
 *	@param	level[in]			The mip map level to retrieve.
 *	@param	ret [out]			The returned mip map data.
 */
 ...
 *	optional dds file header of 128 bytes. (or the tex header of
 *	40 bytes on the xbox).
 */
