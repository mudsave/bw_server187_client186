...
/**
 *	This method accepts a long string and the max width
 *	returns the result width & height ( in texels ) of the given string and the broken string lines
...
uint FontMetrics::stringWidth( const std::string& str )
{
	...
	wsprintfW( &wstr[0], L"%S\0", str.c_str() );
	...
	float* retH )
{
	...
	for ( int i=0; i<n; i++ )
	{
		pos.x += this->makeCharacter( &mesh[i*6], str[i], pos );
 ...
 */
void Font::drawConsoleString( const std::string& str, int x, int y )
{
	...
	int px = x * (charSizePx - effectsWidthInPixels);
	int py = y * charSizePy;
...
void Font::drawStringInClip( const std::wstring& wstr, const Vector3 & position )
{
	...
	if ( SUCCEEDED( Moo::rc().setFVF( Moo::VertexXYZDUV::fvf() ) ) )
	{
		...
		// Render without indices, because using drawIndexedPrimitiveUP was causing
		// a strange flicker in nVidia cards when rendering text.
		Moo::rc().drawPrimitiveUP( D3DPT_TRIANGLELIST, mesh.size() / 3,
			&mesh.front(), sizeof(Moo::VertexXYZDUV) );
	}
...
FontPtr	FontManager::get( const std::string& resourceName, bool htmlSupport, bool cached )
{
	...
	else
	{
		...
		if ( pSection.hasObject() )
		{
			...
			Moo::BaseTexturePtr pTex = Moo::TextureManager::instance()->get( mapName, false, false );
...
bool FontManager::createFont( DataSectionPtr pSection )
{
	...
	if ( pSect )
	{
		...
		char buf[256];
		sprintf( buf, "%s%s_%d.dds\0", s_fontRoot.value().c_str(),
