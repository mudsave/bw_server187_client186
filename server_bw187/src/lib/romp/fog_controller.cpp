...
void FogController::tick()
{
	...
	while( it != end )
	{
		...
		if ( distanceSq < e.outerRadiusSquared() )
		{
			...
			weightedFogColour += ( col * effectiveMultiplier );
 ...
 *	There are a few disparate fog settings in libraries, so
 *	we update all of them to keep them in synch.
 */
void FogController::commitFogToDevice()
{
	//set the fog onto the direct 3D device
	float farFog = Moo::rc().camera().farPlane() / multiplier_;
	float nearFog = -Moo::rc().camera().farPlane() * nearMultiplier_;
	...
	Moo::FogHelper::setFogColour( colour_ );
	Moo::FogHelper::setFogEnable( enabled_ );
	Moo::FogHelper::setFogStart( nearFog );
	Moo::FogHelper::setFogEnd( farFog );

	//make sure the render context has the correct settings
	Moo::rc().fogNear( nearFog );
	Moo::rc().fogFar( farFog );

	//make sure the visual context has the correct settings
	Moo::EffectVisualContext::instance().fogEnabled(enabled_);

	Moo::Material::standardFogColour( colour_ );
}
...
static PyObject * py_addFogEmitter( PyObject * args )
{
	...
	{
		PyErr_SetString( PyExc_TypeError, "py_addFogEmitter: Argument parsing error." );
		return NULL;
	}
...
static PyObject * py_delFogEmitter( PyObject * args )
{
	...
	{
		PyErr_SetString( PyExc_TypeError, "py_delFogEmitter: Argument parsing error." );
		return NULL;
	}
