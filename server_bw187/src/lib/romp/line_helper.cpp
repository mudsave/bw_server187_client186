 ...
 */
void LineHelper::addWorldSpace( const Vector3& start, const Vector3& end, 
 ...
 *	@param colour the colour of the line as a 32 bit ARGB value
 */
void LineHelper::addScreenSpace( const Vector4& start, const Vector4& end, 
...
void LineHelper::addBoundingBox( const BoundingBox& bb, const Matrix& world,
	Moo::PackedColour colour, uint32 life )
{
	...
	{
		addWorldSpace( points[i], points[i + 4], colour, life );
		uint32 ii = (i + 1) % 4;
		addWorldSpace( points[i], points[ii], colour , life );
		addWorldSpace( points[i + 4], points[ii + 4], colour, life );
	}
}
