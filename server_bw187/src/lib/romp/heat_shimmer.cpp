...
#endif
{
	...
	this->shimmerSettings_->addOption("ON", HeatShimmer::isSupported());
	this->shimmerSettings_->addOption("OFF", true);
...
void HeatShimmer::setRenderState()
{
	...
	if (s_debugTex != s_lastDebugTex)
	{
		...
		effectMaterial_->hTechnique( s_debugTex ? "debug" : "standard", 0 );
	}
}
