...
void EditConsole::update()
{
	...
	if (this->isCursorShowing())
	{
		...
		this->setCursor( 0, editLine_ );
		this->print( this->prompt() );
		this->setCursor( this->prompt().size(), editLine_ );
		this->print( lineEditor_.editString() );

		this->setCursor(
			this->prompt().size() + lineEditor_.cursorPosition(), editLine_ );
	}
}
...
protected:
		...
		const std::string & value )
	{
		...
		pDebugConsole_->lineColourOverride( pDebugConsole_->cursorY(),
			...
			(type == Watcher::WT_READ_WRITE)			? 0xffffffff :
			(type == Watcher::WT_DIRECTORY)				? 0xffffff20 :
														  0xff9f9f9f );
...
void DebugConsole::incrementLine( float increment )
{
	...
	if (assignmentLocation > 0)
	{
		...
		if (sscanf( valueStr.c_str(), "%f", &oldValue ) == 1)
		{
			...
			if (Watcher::rootWatcher().getAsString( NULL, path.c_str(), result, desc, type ))
			{
...
bool PythonConsole::init()
{
	...
	if (succeeded)
	{
		...
		char	initString[512];
		sprintf( initString, "Python %s on %s\n",
...
void PythonConsole::processLine( const std::string & line )
{
	...
	else
	{
		...
		std::string subbed =
			PyInputSubstituter::substitute( line, "keys", "macros" );
...
void PythonConsole::completeLine( int compIndex )
{
	...
	std::string subbedLine = PyInputSubstituter::substitute( 
			this->lastLineStart_, "keys", "macros" );
	...
	else if (filtered.size() > 1) 
	{
		...
		*/
		...
		useIndex += AUTO_INDEX_RESET;
		completed = buildCompletedLine( 
				headAndExp.first, objAndAttr.first, 
				filtered[ useIndex % filtered.size() ] );
	}
	// no match
	else 
	{
...
StringVector getAttributes( const std::string & object )
{
	...
	PyObject * list = Script::runString( buffer, false );
	if (PyErr_Occurred())
	{
		PyErr_Clear();
	}
