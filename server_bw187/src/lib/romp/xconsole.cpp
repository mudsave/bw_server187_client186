...
void XConsole::clear()
{
	...
	if (scrollOffset_ != 0)
	{
		sprintf( (char *)buffer_[0], "[%d More]", scrollOffset_ );
	}
}
...
void XConsole::draw( float dTime )
{
	...
	for ( int y = 0; y < this->visibleHeight(); y++ )
	{
		if (lineColours_[y].inUse)
		{
			uint32 lcol = lineColours_[y].colour;
		...
		std::string line = "";
		line.assign( (char*)( buffer_[y] ), this->visibleWidth_ );
		font_->drawConsoleString( line, 0, y );
	}
...
void XConsole::drawCursor( float dTime )
{
	...
	if (drawCursor)
	{
		...
		font_->drawConsoleString( "_", cursorX_, cursorY_ );
	}
}
...
void XConsole::newline()
{
	...
	if (cursorY_ - scrollOffset_ == this->visibleHeight_)
	{
		if (scrollable_)
		{
			...
			memset( &buffer_[this->visibleHeight_-1][0], 0, MAX_CONSOLE_WIDTH );
		}
		else if (shouldWrap_)
		{
			cursorY_ = 0;
		...
		{
			memset( buffer_[this->visibleHeight_-1], 0, MAX_CONSOLE_WIDTH );
