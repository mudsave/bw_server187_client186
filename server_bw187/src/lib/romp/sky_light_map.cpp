...
#endif
		if (Moo::EffectVisualContext::instance().isOutside() && active_)
	...
	const Vector2& strataMovement )
{
	...
	if ( this->beginRender() )
	{
		if (needsUpdate_)
		{
			...
			for (uint32 i=0; i<contributors_.size(); i++)
			{
				contributors_[i]->render(this, this->material_.get(), sunAngle);
			}		
			needsUpdate_ = false;
		}	
 ...
 */
void SkyLightMap::configureKeywordSetting(Moo::EffectMacroSetting & setting)
{
...
Moo::EffectMacroSetting * SkyLightMap::s_skyLightMapSetting = 
	new Moo::EffectMacroSetting(
		"SKY_LIGHT_MAP", "SKY_LIGHT_MAP_ENABLE",
		&SkyLightMap::configureKeywordSetting);