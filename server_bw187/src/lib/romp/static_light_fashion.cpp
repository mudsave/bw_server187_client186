	...
	SuperModel & sm, const DataSectionPtr modelLightingSection )
{
	char * pBytes = new char[ sizeof(StaticLightFashion)+
		(sm.nModels()-1)*sizeof(Model::StaticLighting*) ];
	...
	SuperModel & sm, const DataSectionPtr modelLightingSection )
{
	...
	if ((sectionName.length() > 9) && (sectionName.substr(sectionName.length() - 9) == ".lighting"))
	{
		...
		Model::StaticLightingPtr pSL = sm.topModel(0)->getStaticLighting( modelLightingSection );
		lighting_[0] = &*pSL;
		if (pSL)
		{
			pSL->incRef();
	...
	else
	{
		for (int i = 0; i < nModels_; i++)
		{
			...
			if(theSection)
			{
				Model::StaticLightingPtr pSL = sm.topModel(i)->getStaticLighting( theSection );
				lighting_[i] = &*pSL;
				if (pSL)
				{
					pSL->incRef();
