...
public:
			...
			const LensEffect& le )
	{
		...
		if ( difference.lengthSquared() >= (farPlane*farPlane) )
		{
			...
			//Otherwise, use the cloud layer
			return alpha_ > 0.4 ? SOLID : alpha_ > 0.1 ? DAPPLED : NONE;
		}
		else
			return NONE;
	}
 ...
 pBodyPix_( NULL )
{
	...
	ts1.pTexture( Moo::TextureManager::instance()->get(s_cloudEdgesBmpName) );
	...
	ts1.pTexture( Moo::TextureManager::instance()->get(s_cloudBodyBmpName));
...
{
	if (!pSpaceSettings->readBool( "sky", true ))
	{
		enabled_ = false;
		return;
	}
...
void Sky::prepareClouds( float dTime, Vector3 sunDir, uint32 sunCol, float sunAngle )
{
	...
	uint32 fogCol = Moo::Material::standardFogColour();
...
void Sky::render( SkyLightMap* lightMap, Moo::EffectMaterial* material, float sunAngle  )
{
	...
	if ( material->begin() )
	{
		for ( uint32 i=0; i<material->nPasses(); i++ )
		{
			...
			if ( bodis_.size() > 0 )
			{
				...
				vertexBase = vb.lockAndLoad( &bodis_.front(), bodis_.size() );
				vb.set( 0 );
...
void Sky::draw()
{
	...
	if ( s_drawEdges && edges_.size() > 0 )
	{
		uint32 vertexBase = 0;
		Moo::DynamicVertexBufferBase2<SkyVertex>& vb = Moo::DynamicVertexBufferBase2< SkyVertex >::instance();
		vertexBase = vb.lockAndLoad( &edges_.front(), edges_.size() );
		vb.set( 0 );
		Moo::rc().drawIndexedPrimitive(
			D3DPT_TRIANGLELIST, 
			vertexBase,
	...
	Moo::FogHelper::setFogColour( Moo::Material::standardFogColour() );
