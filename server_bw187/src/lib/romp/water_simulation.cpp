...
SimulationManager				SimulationManager::instance_;
float							SimulationManager::maxIdleTime_ = 5.0f;
int								SimulationManager::maxTextureBlocks_ = 4;
...
#define WATER_RENDER_TARGET_SIZE 256
...
{
	cookieTexture_ = Moo::TextureManager::instance()->get(s_simCookieMap);	
}
...
void SimulationManager::recreateBlocks()
{
	...
	if (Moo::RenderContext::instance().supportsTextureFormat( D3DFMT_A16B16G16R16F ))
	{
		for (uint i=0; i<blocks_.size(); i++)
		{
			blocks_[i].recreate();
...
void SimulationManager::init()
{
	...
	if (blocks_.size() == 0)
		blocks_.resize(maxTextureBlocks_);
 ...
 */
void SimulationManager::simulateBlock( Moo::EffectMaterialPtr effect, SimulationTextureBlock* block )
{
	...
	DX::Viewport d3dvp;
	d3dvp.X = 0; d3dvp.Y = 0;
	d3dvp.Width = size; d3dvp.Height = size;
	d3dvp.MinZ = 0.f; d3dvp.MaxZ = 1.f;

	Moo::rc().device()->SetViewport( &d3dvp );
	...
	Moo::rc().device()->DrawPrimitive(D3DPT_TRIANGLEFAN, 0, 2);
	effect->endPass();
	effect->end();
}
