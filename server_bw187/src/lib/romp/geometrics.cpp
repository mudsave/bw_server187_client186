 ...
 *	This is useful on the xbox when rendering to multiple viewports in a swizzled 
 *	render taret.
 */
...
void Geometrics::drawRect( const Vector2& topLeft,
		const Vector2& bottomRight,
		const Moo::EffectMaterial& material,
		bool linearUV )
{
	DX::Device* device_ = Moo::rc().device();
		...
		const Moo::Colour& colour, bool alwaysDraw )
{
    ...
    */
	...
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 4, &vertices[0], sizeof( Moo::VertexXYZL ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 4, &vertices[5], sizeof( Moo::VertexXYZL ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[10], sizeof( Moo::VertexXYZL ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[12], sizeof( Moo::VertexXYZL ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[14], sizeof( Moo::VertexXYZL ) );
	Moo::rc().drawPrimitiveUP( D3DPT_LINESTRIP, 1, &vertices[16], sizeof( Moo::VertexXYZL ) );
}
		...
		bool wire )
{
	...
	//fill the dynamic index buffer's vertices
	Moo::DynamicIndexBufferBase& indices = Moo::DynamicIndexBufferBase::instance(D3DFMT_INDEX16);
