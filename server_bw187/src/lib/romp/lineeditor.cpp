...
{
	editString_ = s.length() < uint(this->console_->visibleWidth()) - MAX_LINES_OFFSET
		? s
		: s.substr(0, this->console_->visibleWidth() - MAX_LINES_OFFSET);
...
{
	if (editString_.length() >= uint(this->console_->visibleWidth()) - MAX_LINES_OFFSET) 
	{
		return 0;
	}
...
void LineEditor::showHistory()
{
	...
	editString_ = history_[ historyShown_ ];
	cx_ = editString_.length();
	lastChar_ = 0;
}
