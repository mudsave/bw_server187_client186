 ...
 pRayleighMap_( NULL )
{
	...
	fogEmitter_.nearMultiplier_ = 0.f;
...
bool SkyGradientDome::loadTexture( const std::string & textureName )
{
    ...
    Moo::BaseTexturePtr newTexture = 
        Moo::TextureManager::instance()->get(textureName, true, false);
...
SkyGradientDome::update( float time )
{
	...
	fogEmitter_.maxMultiplier_ = 1.f;
}
...
void SkyGradientDome::draw( TimeOfDay* timeOfDay )
{
	...
	fogFactors_.w = 1.f -  min(totalOtherFogging, 1.f);
