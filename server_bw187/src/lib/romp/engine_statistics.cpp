...
double GraphTrack::period_ = stampsPerSecondD() * 2;
double GraphTrack::range_ = stampsPerSecondD() / 20;
...
void GraphTrack::draw()
{
	...
	uint64	doneTime = 0;
	uint64	uperiod = uint64(period_);
	for (int i = 1; i < 120 && doneTime < uperiod; i++)
	{
		tlv.pos_.x = float( double(int64(doneTime)) / period_ );
		tlv.pos_.x *= Moo::rc().screenWidth();
		tlv.pos_.x = Moo::rc().screenWidth() - tlv.pos_.x;

		tlv.pos_.y = float( double(int64(stat_.value( i ))) / range_ );
	...
	int indent, uint64 value, uint64 overall )
{
	...
	char	percent[16];
	sprintf( percent, "%4.1f%%", value * 100.f / overall );
...
{	
	...
	uint32 pgc = Moo::rc().primitiveGroupCount();
	uint32 vc  = Moo::Visual::drawCount_;
	Moo::Visual::drawCount_ = 0;

	_snprintf( statString, STR_SIZE - 1, "Visuals: %d PrimitiveGroups: %d pg/visual: %f    \n",
		vc, pgc, vc ? (float(pgc) / float(vc)) : 0.f );
	console.print( statString );
	_snprintf( statString, STR_SIZE - 1, "Primitives: %d    \n", Moo::rc().primitiveCount() );
	console.print( statString );
	uint32 rpgc = Moo::rc().resetPrimitiveGroupCount();
	if ( rpgc != 0 )
	{
		_snprintf( statString, STR_SIZE - 1, "PrimsPerPG: %d    \n",
			Moo::rc().primitiveCount() / rpgc );
		console.print( statString );
	}

	// Texture memory
	_snprintf( statString, STR_SIZE - 1, "TexMem   : %d    \n",
		Moo::TextureManager::instance()->textureMemoryUsed() );
	console.print( statString );
	_snprintf( statString, STR_SIZE - 1, "FrameTexMem: %.2fMB    \n\n",
		float(Moo::ManagedTexture::totalFrameTexmem_) / ( 1024.f * 1024.f ) );
	console.print( statString );
