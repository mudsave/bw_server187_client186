...
public:
	...
	Moo::EffectMaterial* getMaterial( const LensEffect & le );
	...
	void	preload( const LensEffect & le );
...
private:
	void	killOld( void );
	bool	flareVisible( LensEffect & l );
	...
	class Materials : public std::map< std::string, Moo::EffectMaterial * >
	{
	public:
		~Materials();
		void clear();
		iterator get( const std::string & resourceID );
	};
	...
	Vector4	lastFlareVisibleCheckSource_;
	bool			lastFlareVisibleCheckResults_[2];
};
