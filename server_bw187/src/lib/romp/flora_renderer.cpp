...
bool FloraRenderer::createVB( uint32 bufferSize, DWORD fvf, DWORD lockFlags )
{
	...
	HRESULT hr = Moo::rc().device()->CreateVertexBuffer( bufferSize, 
...
void FloraRenderer::clear( uint32 offset, uint32 size )
{
	if (pVB_)
	{
		...
		BYTE* pData;
        if (SUCCEEDED( pVB_->Lock( 0, 0, (void**)&pData, lockFlags_ )))
		{
			ZeroMemory( pData + offset, size );
			pVB_->Unlock();
		}
		else
		{
...
bool FloraRenderer::preDraw( EnviroMinder& enviro )
{
	...
	Moo::rc().device()->SetStreamSource( 0, pVB_, 0, sizeof( FloraVertex ) );	
