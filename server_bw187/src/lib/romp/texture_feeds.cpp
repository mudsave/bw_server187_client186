...
TextureFeeds TextureFeeds::instance_;
...
void TextureFeeds::addTextureFeed( const std::string& identifier, PyTextureProviderPtr texture )
{
	instance_.feeds_[ identifier ] = texture;
}
...
void TextureFeeds::delTextureFeed( const std::string& identifier )
{
	if (!instance_.feeds_.empty())
	{
		Feeds::iterator it = instance_.feeds_.find( identifier );
		if ( it != instance_.feeds_.end() )
			instance_.feeds_.erase( it );
	}
}
...
Moo::BaseTexturePtr TextureFeeds::get( const std::string& identifier ) 
{
	...
	{
		Feeds::iterator it = instance_.feeds_.find( identifier );
		if ( it != instance_.feeds_.end() && it->second )
