...
{
	TextureRenderers::iterator found = std::find(
		s_staggeredRenderers_.begin(), s_staggeredRenderers_.end(), pTR );

	if (found != s_staggeredRenderers_.end())
		s_staggeredRenderers_.erase( found );

	if ( pTR->cachable() )
	{	
		found = std::find( s_cachableRenderers_.begin(), s_cachableRenderers_.end(), pTR );
		if (found != s_cachableRenderers_.end())
			s_cachableRenderers_.erase( found );
	}
}
...
void TextureRenderer::updateCachableDynamics( float dTime )
{
	...
	for (uint i = 0; i < ss; i++)
	{
		TextureSceneInterface * pTR = s_cachableRenderers_[ i ];
		pTR->render( dTime );
	}
	clearCachable();
}
