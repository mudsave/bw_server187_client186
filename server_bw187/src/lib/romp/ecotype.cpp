...
{		
	...
	li->loadingTask_ = new BackgroundTask( &Ecotype::backgroundInit, li,
	...
	BgTaskManager::instance()->addTask( *li->loadingTask_ );
#else	
	//load ecotypes synchronously instead of async in the background thread.
	//currently the tools don't use BgTaskManager, so just do it synchronously
	backgroundInit( li );
	onBackgroundInitComplete( li );	
#endif
}
