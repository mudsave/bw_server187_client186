...
int s_allowedBlockMovesPerFrame = 50;
 ...
 */
bool Flora::init( DataSectionPtr pSection )
{
	...
	MF_ASSERT( ECOTYPES_PER_BLOCK_WIDTH == BLOCK_WIDTH / ECOTYPE_GRID );
	MF_ASSERT( ECOTYPES_PER_BLOCK == ECOTYPES_PER_BLOCK_WIDTH * ECOTYPES_PER_BLOCK_WIDTH );
...
void Flora::update( float dTime, EnviroMinder& enviro )
{
	...
	if ( fabsf( lastPos_.x - camPos2.x ) > 100.f )
	{
		cameraTeleport_ = true;
	}
	else if ( fabsf( lastPos_.y - camPos2.y ) > 100.f )
	{
		cameraTeleport_ = true;
	}
...
void Flora::draw( float dTime, EnviroMinder& enviro )
{
	...
	Moo::rc().addToPrimitiveGroupCount( s_numDrawPrimitiveCalls );
	Moo::rc().addToPrimitiveCount( s_numTris );
...
void Flora::calculateTransform( const FloraBlock& block )
{
	...
	Moo::TerrainBlock::FindDetails details = Moo::TerrainBlock::findOutsideBlock( blockPos );
...
Ecotype& Flora::ecotypeAt( const Vector2& p )
{
	if ( data_ )
	{
		...
		if (details.pBlock_)
		{
			...
			uint8 id = details.pBlock_->detailID( relPos.x + round, relPos.z + round );
 ...
 */
Moo::TerrainBlockPtr Flora::getTerrainBlock( const Vector3 & pos, Vector3 & relativePos, const Vector2 * refPt ) const
{
	if ( !refPt || *refPt != lastRefPt_ )
	{
		details_ = Moo::TerrainBlock::findOutsideBlock( pos );
		if ( refPt )
			lastRefPt_ = *refPt;
	}
...
void Flora::vbSize( uint32 numBytes )
{
	...
	numVerticesPerBlock_  = std::max(numVertices_ / blockSize, 3UL);
...
void FloraSettings::init(DataSectionPtr resXML)
{
	...
	if (resXML.exists())
	{
		...
		while (sectIt != sectEnd)
		{
			...
			{
				this->floraSettings_->addOption(label, true);
	...
	{
		this->floraSettings_->addOption("HIGHT", true);
