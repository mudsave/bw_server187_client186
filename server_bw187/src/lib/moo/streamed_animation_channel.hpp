...
public:
	...
	void load( const BinaryFile & bf, float totalTime,
...
private:
	...
	typedef std::vector< SmartPointer<StreamedAnimationChannel> > Channels;
	Channels			channels_;
	...
	struct StreamedBlock
	{
		...
		uint32						size_;
		CacheRecord *				data_;
		StreamedDataCache::Tracker*	reader_;
	};
...
struct ChannelBlock
{
	...
	const Vector3 * scaleKeyData() const
		{ return (const Vector3*)(
			((uint8*)(this+1)) + sHeader_.keyDataOffset_ * sizeof(uint32) ); }

	const Vector3 * positionKeyData() const
		{ return (const Vector3*)(
			((uint8*)(this+1)) + pHeader_.keyDataOffset_ * sizeof(uint32) ); }

	const Quaternion * rotationKeyData() const
		{ return (const Quaternion*)(
			((uint8*)(this+1)) + rHeader_.keyDataOffset_ * sizeof(uint32) ); }
...
public:
	...
	virtual bool		load( const BinaryFile & bf );
