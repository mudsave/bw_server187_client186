...
{
	SCOPEWATCH( "SetPixelShaderConstantB" );
...
{
	SCOPEWATCH( "SetPixelShaderConstantF" );
...
{
	SCOPEWATCH( "SetPixelShaderConstantI" );
...
{
	SCOPEWATCH( "SetVertexShaderConstantB" );
...
{
	SCOPEWATCH( "SetVertexShaderConstantF" );
	HRESULT hr;
...
{
	SCOPEWATCH( "SetVertexShaderConstantI" );
...
HRESULT __stdcall StateManager::SetTextureStageState( DWORD stage, D3DTEXTURESTAGESTATETYPE type,
	DWORD value )
{
	SCOPEWATCH( "SetTextureStageState" );
...
/*
 *	Get the instance of the statemanager singleton
 */
StateManager* StateManager::instance()
{
	static SmartPointer<StateManager> pInst = new StateManager;
	return pInst.getObject();
}
...
/*
 *	Dummy singleton instance of state recorder.
 *	This is only here as the Staterecorder derives from the
 *	state manager and the state manager has an instance.
 */
StateRecorder* StateRecorder::instance()
{
	CRITICAL_MSG( "StateRecorder::instance - no instance for the state recorder" );
	return NULL;
}
