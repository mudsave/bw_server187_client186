...
/**
 *  Vertices is the class used to load and access a vertex buffer, with one of several Vertex formats
 *  a Vertices object is created from a resourceID.  This in general is a reference
 *  to a subsection of a .primitives file ( for example "objects/MyBipedObject.primitive/FeetVertices" ).
 *  A Vertices object is created using the VerticesManager::get function.
 *
 *  Before rendering a primitive based on this set of Vertices, call the function setVertices, which will
 *  load the verts from the file if necessary, and present them to Direct3D as the current StreamSource.
 *  The format of the vertices is specified in the resource header (which is binary, not xml).  It is a string.
 *
 *  Currently the folowing formats are valid:
 *		- "xyznuv"			( xyz, normal, texture-coordinates )
 *		- "xyznuvtb"		( xyz, normal, texture-coordinates, tangent, binormal )
 *		- "xyznuviiiww"		( xyz, normal, texture-coordinates, node indices and weights )
 *		- "xyznuviiiwwtb"	( xyz, normal, texture-coordinates, node indices and weights, tangent, binormal )
 *		- "xyznuvitb"		( xyz, normal, texture-coordinates, index, tangent, binormal )
 *		- "xyznuvi"			( xyz, normal, texture-coordinates, index )
 *
 */
...
public:
	...
	DX::VertexBuffer*	vertexBuffer( ) const;
	...
	void				clearSoftwareSkinner() { pSkinnerVertexBuffer_ = NULL; }
...
protected:
	...
	DX::VertexBuffer*	pSkinnerVertexBuffer_;
	bool				vbBumped_;
...
public:
	...
	virtual void getVertexDepths( uint32 startVertex, uint32 nVertices, float* pOutDepths ) = 0;
	virtual uint32 setVertices( uint32 startVertex, uint32 nVertices, bool staticLighting ) = 0;
private:
};
