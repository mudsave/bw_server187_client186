...
class SpotLight : public ReferenceCount
{
public:
	SpotLight();
	SpotLight( const Colour& colour, const Vector3& position, const Vector3& direction, float innerRadius, float outerRadius, float cosConeAngle );
	~SpotLight();
...
//typedef std::vector< Moo::SpotLightPtr > SpotLightVector;
