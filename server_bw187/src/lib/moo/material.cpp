...
uint32 Material::standardFogColour_ = Material::initMaterialStuff();
...
{                                                                               
	{
		memoryCounterSub( material );
		memoryClaim( identifier_ );
	}
	identifier_ = id;
	{
		memoryCounterAdd( material );
		memoryClaim( identifier_ );
	}
}
	...
	if( rc.device() )
	{
		...
		if (fogged_)
		{
			if (destBlend == ONE)
				...
				FogHelper::setFogColour( standardFogColour_ );
		}
...
Material::readTextureStageSection( TextureStage& t, DataSectionPtr spSection )
{
    ...
    //initialize texture stage with values from file
...
uint32 Material::initMaterialStuff()
{
	...
	// return the default standard fog colour
	return 0x000000ff;
}
