...
public:
	...
	typedef std::vector<StringBoolPair> StringBoolPairVector;
	...
	int addOption(const std::string & label, bool isSupported);
	const StringBoolPairVector & options() const;
	...
	static void read(DataSectionPtr section);
...
private:
	bool selectOptionPriv(int optionIndex);

	std::string           label_;
	StringBoolPairVector  options_;
	int                   activeOption_;
	bool                  delayed_;
	bool                  needsRestart_;
...
/**
 *	Implement the GraphicsSetting class, providing a callback hook that
 *	Is called whenever the active option changes. This class provides the
 *	basic graphics settings functionality that is needed by most client
 *	subsystems (see romp/flora.cpp for an example).
 */
...
public:
	...
	CallbackGraphicsSetting(
			...
			bool                delayed,
			bool                needsRestart) :
		GraphicsSetting(label, activeOption, delayed, needsRestart),
		instance_(instance),
		function_(function)
	{}
