...
inline void interpolateKeys(
	...
	const InterpolatedAnimationChannel::IndexVector & index,
	Interpolation interpolate,
	float time,
	KeyVal& out )
{
	//std::map< float, KeyVal >::const_iterator itFirst = keyFrames.upper_bound( time );
	uint32 hint = index.upper_bound( int( time ) );

	MF_ASSERT((hint - 1u) < keyFrames.size() &&
			  "Corrupt animation data has been encountered, deleting all .anca files should resolve the problem.")
	while ( hint > 0 && keyFrames[hint-1].first > time )
	{
		hint--;
	}

	std::vector< std::pair< float, KeyVal >  >::const_iterator
		itFirst = keyFrames.begin() + hint;

	if( itFirst != keyFrames.begin() )
	{
		std::vector< std::pair< float, KeyVal >  >::const_iterator
			itLast = itFirst--;
		if( itLast != keyFrames.end() )
		{
			float time1 = itFirst->first;
			float time2 = itLast->first;
			float t = ( time - time1 ) / ( time2 - time1 );
			interpolate( &out, &itFirst->second, &itLast->second, t );
		}
		else
		{
			out = itFirst->second;
...
void InterpolatedAnimationChannel::reduceRotationKeys( float cosAngleError, Moo::NodePtr node )
{
	...
	if (node)
	{
		float angleError = acosf( cosAngleError );
 ...
 *	Load method.
 */
bool InterpolatedAnimationChannel::load( const BinaryFile & bf )
{
...
int InterpolatedAnimationChannel::type() const
{
	...
	// We're being run in modelviewer, so save with the compression factors
	if (inhibitCompression_)
		return 4;
	// We've read in a file with compression factors and compressed on load,
	// so just save the compressed data out raw for the xbox

	// Either that or we read in an old style file without compression factors
...
int InterpolatedAnimationChannel::size() const
{
	return
		...
		// assume indicies are 8 bits on xbox
