...
{
	memoryCounterAdd( indices );
	memoryClaim( primitives_ );
}

PrimitiveManager::~PrimitiveManager()
{
	memoryCounterSub( indices );
	while (!primitives_.empty())
	{
		memoryClaim( primitives_, primitives_.begin() );
...
void PrimitiveManager::addInternal( Primitive * pPrimitive )
{
	...
	memoryCounterAdd( indices );
	memoryClaim( primitives_, --primitives_.end() );
}
