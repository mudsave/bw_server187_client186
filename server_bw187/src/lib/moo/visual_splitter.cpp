...
void VisualSplitter::save( const std::string& resourceName )
{
	...
	std::string primitivesResource = BWResource::removeExtension(resourceName) + ".primitives";
	DataSectionPtr pPrimitives = new BinSection( primitivesResource, new BinaryBlock( 0, 0 ) );
	...
	virtual void save( DataSectionPtr pVisualSection, DataSectionPtr pPrimitives )
	{
		if (pRenderSet_ && pPrimitives_)
		{
			...
			for (uint i = 0; i  < geometrySections.size(); i++)
			{
                ...
                pPrimitives->writeBinary( verts, new BinaryBlock( pVerts->data(), pVerts->len() ) );
                pPrimitives->writeBinary( prim, new BinaryBlock( pPrims->data(), pPrims->len() ) );
			}
		}
	}
private:
...
/*
 *	Structures used by the vertices and indices
 */
...
public:
	...
	public:
			...
			const std::vector<Type>& in, const std::string& formatName )
		{
			...
			sprintf( pVH->vertexFormat, "%s", formatName.c_str() );
			pVH->nVertices = in.size();
			memcpy( pVertices, &in.front(), sizeof(Type) * in.size() );
			BinaryPtr pData = new BinaryBlock( data, len );
			delete [] data;
			return pData;
		}
	private:
	...
	public:
		IndexHolder( const uint16* pIndices, uint32 nIndices, 
			const PrimGroup* pPrimGroups, uint32 nPrimGroups )
		{
		...
		{
			const PrimGroup& group = primitiveGroups_[pg];
	...
	private:
		...
		std::vector< PrimGroup > primitiveGroups_;
	};
	...
	public:
			...
			std::string identifier, int channelIndex)
		{
			...
			if (pMT)
				...
				::MessageBox( 0, "Error: Failed to add morph target", 
					"Visualexporter: Morph target failure", MB_ICONERROR );
		}
	...
	private:
		...
		void loadMorphTargets( const MorphHeader* pMH )
		{
			...
			for (int i = 0; i < pMH->nMorphTargets_; i++)
			{
				...
				if (pMT)
					...
					::MessageBox( 0, "Error: Failed to load morph target", 
						"Visualexporter: Morph target failure", MB_ICONERROR );
										  ...
										  const MorphTargetBase::MorphVertex* pMV )
		{
			...
			{
				MF_ASSERT(false &&
					"Moo::RenderSetSplit::MorphTargetsHolder::createMorphTarget: "
	...
	public:
		...
		/*
		 *	This method creates a list of node indices by combining relationships until
		 *	the list contains upto a specified number of nodes. Relationships that are
		 *	added to the nodelist will be removed.
	...
	public:
			...
			pMaterialSection_( pMaterialSection )
		{
			...
			// the split index.
			char postfix[32];
			sprintf( postfix, "%d%c", pgIndex, 'a' + char(splitIndex_) );
	...
	private:
		...
		void writeIndices( DataSectionPtr pPrimitives )
		{
			...
			uint32 indsSize = indices_.size() * sizeof(uint16) + sizeof(IndexHeader) + sizeof(PrimGroup);
			...
			PrimGroup* pPG = reinterpret_cast<PrimGroup*>(pIndices + indices_.size());

			//Fill in the index header.
			sprintf( pIH->indexFormat, "list" );
			pIH->nIndices = indices_.size();
			pIH->nTriangleGroups = 1;
			...
			pPrimitives->writeBinary( indsName_, new BinaryBlock(data, indsSize));
		...
		void writeMorphTargets(MorphTargetsHolder::MorphTargetsHolderPtr pMTH, DataSectionPtr pPrimitives)
		{
			...
			// Allocate data
			BinaryPtr dataBlock = new BinaryBlock(NULL, totalDataSize);
...
#define CREATE_HOLDER( name, format, container ) \
	if (std::string(pVh->vertexFormat) == #name)\
    {\
		pHolder = new container<format>( \
		reinterpret_cast< const format* >( pVh + 1 ), pVh->nVertices, #name );\
    }
	...
	IndexHolder* readIndices( BinaryPtr pIndexData )
	{
		...
		const PrimGroup* pPrimGroups =
			reinterpret_cast<const PrimGroup*>( pIndices + pIH->nIndices );
		return new IndexHolder( pIndices, pIH->nIndices, 
			pPrimGroups, pIH->nTriangleGroups );
	}
	...
	MorphTargetsHolder* readMorphTargets(BinaryPtr pVerts)
	{
		...
		int nVertices = pVHeader->nVertices;
		
		// Get the stride of the vertices
		uint vertexStride;
		if (std::string( pVHeader->vertexFormat ) == "xyznuvi")
		{
			vertexStride = sizeof(VertexXYZNUVI);
		}
		else if (std::string( pVHeader->vertexFormat ) == "xyznuvitb")
		{
			vertexStride = sizeof(VertexXYZNUVITB);
		}
		else if (std::string( pVHeader->vertexFormat ) == "xyznuviiiww")
		{
			vertexStride = sizeof(VertexXYZNUVIIIWW);
		}
		else if (std::string( pVHeader->vertexFormat ) == "xyznuviiiwwtb")
		{
		...
		return new MorphTargetsHolder( pMH, std::string(pVHeader->vertexFormat) );
	}
