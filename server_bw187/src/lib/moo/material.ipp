...
{
	MF_ASSERT( mode < Material::LAST_UV2_GENERATOR );
	uv2Generator_ = mode;
}
...
{
	MF_ASSERT( mode < Material::LAST_UV2_ANGLE );
	uv2Angle_ = mode;
}
...
INLINE uint32 Material::standardFogColour() // static method
{
	return standardFogColour_;
}

INLINE void Material::standardFogColour( uint32 colour )
{
	standardFogColour_ = colour;
}
