...
int Toolbar::getImageIndex( ItemPtr item )
{
	if( imageIndices_.find( item.getObject() ) == imageIndices_.end() )
	{
		...
		BitmapPtr bitmap = BitmapManager::instance()->get( (*item)["imageNormal"], transparency );
		result_ = ImageList_Add( normalImageList_, (*bitmap), NULL );
		if( item->exist( "imageHot" ) )
			ImageList_Add( hotImageList_,
				*BitmapManager::instance()->get( (*item)["imageHot"], transparency ), NULL );
		else
			ImageList_Add( hotImageList_,
				*BitmapManager::instance()->get( (*item)["imageNormal"], transparency, "HOVER" ), NULL );

		if( item->exist( "imageDisabled" ) )
			ImageList_Add( disabledImageList_,
				*BitmapManager::instance()->get( (*item)["imageDisabled"], transparency ), NULL );
		else
			ImageList_Add( disabledImageList_,
				*BitmapManager::instance()->get( (*item)["imageNormal"], transparency, "DISABLED" ), NULL );
