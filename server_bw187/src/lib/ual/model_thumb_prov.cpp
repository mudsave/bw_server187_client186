...
bool ModelThumbProv::isValid( const std::string& file )
{
	if ( file.empty() )
		return false;
...
bool ModelThumbProv::needsCreate( const std::string& file, std::string& thumb, int& size )
{
...
bool ModelThumbProv::prepare( const std::string& file )
{
...
bool ModelThumbProv::render( const std::string& file, Moo::RenderTarget* rt )
{
	if ( !visual_ )
		return false;
