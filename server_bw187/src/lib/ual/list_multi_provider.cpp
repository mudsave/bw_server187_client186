...
ListMultiProvider::ListItem::ListItem( ListProvider* provider, int index ) :
	provider_( provider ),
	index_( index ),
	inited_( false )
{
...
void ListMultiProvider::getThumbnail( int index, CImage& img, int w, int h,
 ...
 */
void ListMultiProvider::addProvider( ListProvider* provider )
{
	if ( !provider )
		return;
...
void ListMultiProvider::fillItems()
{
		...
		p != providers_.end(); ++p )
	{
		...
		ListProvider* listProv = (*p).getObject();
