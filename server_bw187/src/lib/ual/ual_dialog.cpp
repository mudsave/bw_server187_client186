...
static const char* DEFAULT_SEARCH_IDLE_TEXT = "Asset Name";
...
UalDialog::UalDialog( const std::string& configFile )
	...
	, splitterBar_( 0 )
	, dlgShortCaption_( "Assets" ) , dlgLongCaption_( "Asset Browser" )
...
void UalDialog::loadMain( DataSectionPtr section )
{
	...
	searchIdleText_ = section->readString( "searchIdleText", DEFAULT_SEARCH_IDLE_TEXT );
}
...
BOOL UalDialog::OnInitDialog()
{
	...
	if( toolTip_.CreateEx( this, 0, WS_EX_TOPMOST ) )
	{
		...
		toolTip_.AddTool( &search_, "Type in the partial name of the asset" );
		toolTip_.AddTool( &searchFilters_, "Click to show/hide the search filters" );
...
void UalDialog::favouritesChanged()
{
	...
	if ( smartList_.getProvider() == &favouritesListProvider_ )
		smartList_.refresh();
}
...
void UalDialog::historyChanged()
{
	...
	if ( smartList_.getProvider() == &historyListProvider_ )
		smartList_.refresh();
}
...
void UalDialog::folderTreeItemDelete( VFolderItemData* data )
{
	...
	if ( data->getProvider().getObject() == historyFolderProvider_.getObject() )
	{
		...
		{
			if ( MessageBox(
					"Are you sure you want to clear the entire asset history?",
					"Clear Asset History",
			...
			UalHistory::instance()->clear();
		}
		else
			UalHistory::instance()->remove( data->assetInfo() );
	}
	else if ( data->getProvider().getObject() == favouritesFolderProvider_.getObject() )
	{
		if ( data->isVFolder() )
		{
			...
			UalFavourites::instance()->clear();
		}
		else
			UalFavourites::instance()->remove( data->assetInfo() );
	}
}
...
void UalDialog::showItemContextMenu( UalItemInfo* ii )
{
	...
	// List Styles submenu
	menu.startSubmenu( "List View Styles" );

	std::string check;
	check = smartList_.getStyle() == SmartListCtrl::LIST ? "##" : "";
	menu.addItem( check + "List", listViewCmd );

	check = smartList_.getStyle() == SmartListCtrl::SMALLICONS ? "##" : "";
	menu.addItem( check + "Small Icons", smallViewCmd );

	check = smartList_.getStyle() == SmartListCtrl::BIGICONS ? "##" : "";
	menu.addItem( check + "Big Icons", bigViewCmd );
	...
	if ( ii )
	{
		...
		{
			if ( smartList_.getProvider() == &favouritesListProvider_ )
				menu.addItem( "Remove from Favourites", removeFromFavCmd );
			else if ( smartList_.getProvider() == &historyListProvider_ )
				menu.addItem( "Remove from History", removeFromHistCmd );

			if ( smartList_.getProvider() != &favouritesListProvider_ )
				menu.addItem( "Add to Favourites", addToFavCmd );
		}
		...
		if ( !ii->getNext() )
		{
			...
			if ( paths.size() == 1 )
			{
				menu.addItem( "Open folder in Explorer", openExplorerCmd );
				menu.addItem( "Copy path to clipboard", copyPathCmd );
			}
			else
			{
				for( int i = 0; i < (int)paths.size() && i < openExplorerCmdRange; ++i )
				{
					...
					{
						menu.addItem( 
								std::string("Open '") + paths[i] + "' in Explorer",
								openExplorerCmd + i );
					}
				}
				...
				for( int i = 0; i < (int)paths.size() && i < copyPathCmdRange; ++i )
				{
					...
					{
						menu.addItem( 
								std::string("Copy '") + paths[i] + "' to clipboard",
								copyPathCmd + i );
					}
				}
			}
		...
		result == removeFromHistCmd )
	{
		...
		while ( ii )
		{
			...
			{
				UalFavourites::instance()->add(
					ii->assetInfo() );
			}
			else if ( result == removeFromFavCmd )
			{
				UalFavourites::instance()->remove(
					ii->assetInfo() );
			}
			else if ( result == removeFromHistCmd )
			{
				UalHistory::instance()->remove(
					ii->assetInfo() );
			}
			ii = ii->getNext();
		}
...
void UalDialog::showContextMenu( VFolderItemData* data )
{
	if ( !data || data->isVFolder() )
	{
		...
		// common menu items
		menu.addItem( "Change this panel's title...", renameCmd );
		menu.addItem( "Reload default folders", defaultFoldersCmd );

		if ( data )
		{
			std::string remove = std::string("Remove '") + data->assetInfo().text() + "'";
			menu.addItem( remove, removeFolderCmd );
		}
		if ( !plainVFolder )
			menu.addItem( "Clear contents", 100 );
		...
		if ( result == removeFolderCmd && data )
		{
			...
			{
				ThumbnailManager::instance()->resetPendingRequests( &folderTree_ );
		...
		{
			if ( MessageBox( "Are you sure you want to reload the default folders for this dialog?",
...
void UalDialog::listItemSelect()
{
	...
	else
	{
		...
		char num[100];
		sprintf( num, "%d", numSel );
		txt = num;
		sprintf( num, "/%d", smartList_.GetItemCount() );
		txt += num;
		txt += " items";

		if ( numSel > 10 )
			txt += " : (many items selected)";
		else
		{
			txt += " : ";
			int item = -1;
...
{
	if ( smartList_.getProvider() == &historyListProvider_ ||
		smartList_.getProvider() == &favouritesListProvider_ )
	{
...
void UalDialog::handleDragMouseMove( UalItemInfo& ii, const CPoint& screenPt )
{
	CPoint pt = screenPt;
	ClientToScreen( &pt );
		...
		!itemInfo.isFolder_ )
	{
		...
		else
		{
			...
			if ( doAdd ) 
			{
				...
				while ( ii )
				{
					UalFavourites::instance()->remove( ii->assetInfo() );
					UalFavourites::instance()->addAt(
						ii->assetInfo_,
						dropAssetInfo );
					ii = ii->getNext();
				}
				...
				if ( doAdd )
				{
					...
					while ( ii )
					{
						if ( !data->isVFolder() )
						{
							...
							UalFavourites::instance()->remove( ii->assetInfo_ );
						}
						// add to favourites
						if ( !UalFavourites::instance()->getItem( ii->assetInfo_ ) )
							UalFavourites::instance()->addAt(
								ii->assetInfo_,
								data->assetInfo() );
						else
							UalFavourites::instance()->add( ii->assetInfo_ );
						ii = ii->getNext();
					}
				}
			}
