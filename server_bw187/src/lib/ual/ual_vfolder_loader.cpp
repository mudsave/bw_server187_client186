	...
	bool addToFolderTree )
{
	...
	VFolderFileProvider* prov = new VFolderFileProvider( type, paths, extensions, includeFolders, excludeFolders, flags );
	...
	bool addToFolderTree )
{
	...
	bool showItems = section->readBool( "showItems", false );
	std::string path = section->readString( "path" );
	UalHistory::instance()->setPath( path );
	int maxItems = section->readInt( "maxItems", UalHistory::instance()->getMaxItems() );
	if ( maxItems < 0 )
		error( dlg, "Wrong History/maxItems. Must be greater or equal to zero." );
	else
		UalHistory::instance()->setMaxItems( maxItems );
	...
	bool addToFolderTree )
{
	...
	bool showItems = section->readBool( "showItems", false );
	std::string path = section->readString( "path" );
	UalFavourites::instance()->setPath( path );
