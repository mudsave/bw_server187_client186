...
SmartListCtrl::SmartListCtrl() :
	...
	maxItems_( 200 )
{
	thumbWidth_ = ThumbnailManager::instance()->size();
	thumbHeight_ = ThumbnailManager::instance()->size();
}

SmartListCtrl::~SmartListCtrl()
{
	ThumbnailManager::instance()->resetPendingRequests( this );
...
void SmartListCtrl::init( ListProvider* provider, XmlItemVec* customItems, bool clearSelection )
{
	provider_ = provider;
...
ListProvider* SmartListCtrl::getProvider()
{
	return provider_;
}
...
void SmartListCtrl::getData( int index, std::string& text, int& image, bool textOnly /*false*/ )
{
	...
	if ( item )
	{
		...
		if ( !textOnly && thumbWidthCur_ && thumbHeightCur_ )
		{
			...
			// cache miss
			CImage img;
			ThumbnailManager::instance()->create( item->assetInfo().thumbnail(),
	...
	if ( !textOnly && thumbWidthCur_ && thumbHeightCur_ )
	{
		...
		// cache miss
		CImage img;
		provider_->getThumbnail( index, img, thumbWidthCur_, thumbHeightCur_, this );
