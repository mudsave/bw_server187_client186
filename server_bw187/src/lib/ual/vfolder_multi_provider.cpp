...
VFolderItemDataPtr VFolderMultiProvider::getNextChild( CImage& img )
{
	if ( iter_ == providers_.end() )
		return NULL;
	...
	while ( (data = (*iter_)->getNextChild( img )) == NULL )
	{
	...
	if ( data != NULL )
	{
		...
		(*iter_)->getThumbnail( data, img );
	}

	return data;
}

void VFolderMultiProvider::getThumbnail( VFolderItemDataPtr data, CImage& img )
{
	if ( !data || !data->getProvider() )
		return;

	// load the item's thumbnail from it's provider
	data->getProvider()->getThumbnail( data, img );
}
...
const std::string VFolderMultiProvider::getDescriptiveText( VFolderItemDataPtr data, int numItems, bool finished )
{
	...
	if ( data->isVFolder() || !data->getExpandable() )
	{
		...
		desc += std::string( num ) + " items";

		if ( !finished )
		{
			// if not finished, put some feedback in the string
			desc += " and loading...";
		}
		desc += ")";
	}
