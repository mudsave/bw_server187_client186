...
public:
	...
	virtual void getThumbnail( int index, CImage& img, int w, int h,
		ThumbnailUpdater* updater );

	virtual void filterItems();

	// additional interface
	void addProvider( ListProvider* provider );
...
private:
	...
	{
	public:
		ListItem( ListProvider* provider, int index );

		ListProvider* provider() const { return provider_; }
		int index() const { return index_; }

		const char* text() const;

	private:
		ListProvider* provider_;
		int index_;
