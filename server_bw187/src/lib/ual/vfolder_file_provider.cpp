...
{
	thumbnailPostfix_ = ThumbnailManager::instance()->postfix();
	init( "", "", "", "", "", 0 );
}
	...
	int flags )
{
	...
	StringUtils::toLowerCase( pathsL );
	...
	StringUtils::toLowerCase( includeFoldersL );
	...
	StringUtils::toLowerCase( excludeFoldersL );
...
VFolderItemDataPtr VFolderFileProvider::getNextChild( CImage& img )
{
	if ( finderStack_.empty() )
		return 0;
					...
					StringUtils::matchSpec( (LPCTSTR)finder->files.GetRoot(), includeFolders_ ) ) )
			{
					...
					&& finder->files.GetFileName().Find( ".thumbnail.bmp" ) == -1 )
				{
						...
						!PathFileExists( BWResource::changeExtension( filePath, ".tga" ).c_str() ) ) )
					{
						...
						ThumbnailManager::instance()->create( filePath, img, 16, 16, folderTree_ );
						found = true;
					}
				}
			}
...
void VFolderFileProvider::getThumbnail( VFolderItemDataPtr data, CImage& img )
{
	if ( !data )
		return;

	ThumbnailManager::instance()->create( data->assetInfo().longText(), img, 16, 16, folderTree_ );
}
...
const std::string VFolderFileProvider::getDescriptiveText( VFolderItemDataPtr data, int numItems, bool finished )
{
	...
	{
		char num[100];
		sprintf( num, "%d", numItems );
		std::string txt = getPathsString() + " (" + num + " items";
		if ( !finished )
			txt += " and loading...)";
		else
			txt += ")";
		...
		((VFolderFileItemData*)data.getObject())->getItemType() == VFolderFileItemData::ITEMDATA_FOLDER )
	{
		char num[100];
		sprintf( num, "%d", numItems );
		std::string txt = data->assetInfo().longText();
		txt += " (";
		txt += num;
		txt += " items";
		if ( !finished )
	...
	bool& retItemClicked )
{
	...
	if ( !fullPath.empty() )
	{
		...
		((ListFileProvider*)listProvider_)->init(
			getType(),
			fullPath,
			getExtensionsString(),
			getIncludeFoldersString(),
