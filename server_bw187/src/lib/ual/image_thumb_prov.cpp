...
bool ImageThumbProv::isValid( const std::string& file )
{
	if ( file.empty() )
		return false;
...
bool ImageThumbProv::prepare( const std::string& file )
{
	...
	int s = ThumbnailManager::instance()->size();

	int origW = imgInfo.Width;
	int origH = imgInfo.Height;
	ThumbnailManager::instance()->recalcSizeKeepAspect( s, s, origW, origH );
...
bool ImageThumbProv::render( const std::string& file, Moo::RenderTarget* rt )
{
	if ( !pTexture_ )
		return false;
