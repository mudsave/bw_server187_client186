...
VFolderItemDataPtr VFolderXmlProvider::getNextChild( CImage& img )
{
	if ( itemsItr_ == items_.end() )
		return 0;

	if ( !(*itemsItr_)->thumbnail().empty() )
		ThumbnailManager::instance()->create( (*itemsItr_)->thumbnail(), img, 16, 16, folderTree_ );
	else
		ThumbnailManager::instance()->create( (*itemsItr_)->longText(), img, 16, 16, folderTree_ );
...
void VFolderXmlProvider::getThumbnail( VFolderItemDataPtr data, CImage& img )
{
	if ( !data )
		return;
	...
	if ( !xmlData->thumb().empty() )
		ThumbnailManager::instance()->create( xmlData->thumb(), img, 16, 16, folderTree_ );
	else
		ThumbnailManager::instance()->create( xmlData->assetInfo().longText(), img, 16, 16, folderTree_ );
}
	...
	bool& retItemClicked )
{
	...
	((ListXmlProvider*)listProvider_)->init( getPath() );
