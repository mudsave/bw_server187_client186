...
bool Pickler::init()
{
	PyObject* pModule = PyImport_ImportModule( "cPickle" );

	if (pModule != NULL)
	{
		if (!s_pPickleMethod)
		{
			s_pPickleMethod = PyObject_GetAttrString( pModule, "dumps" );
			if (!s_pPickleMethod)
			{
		...
		{
			s_pUnpickleMethod = PyObject_GetAttrString(pModule, "loads");
	...
	else
	{
		...
		ERROR_MSG( "Failed to import cPickle module. "
			...
			"\tThis requires entities/common/lib-dynload/cPickle.so to be "
