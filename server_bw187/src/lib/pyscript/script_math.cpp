 ...
 *  mprod.b = matrix2
 *
 *  # create a Matrix from the MatrixProduct
 *  m = Math.Matrix( mprod )
 *
 *  # print the yaw
 *  print m.yaw
...
PyObject * PyVector<V>::pyGetAttribute( const char * attr )
{
	...
	{
		char * pPos = strchr( memNames, attr[0] );
...
template <class V> char * PyVector_fullName();
 ...
 */
template <> char * PyVector_fullName< Vector2 >() { return "Math.Vector2"; }
 ...
 */
template <> char * PyVector_fullName< Vector3 >() { return "Math.Vector3"; }
 ...
 */
template <> char * PyVector_fullName< Vector4 >() { return "Math.Vector4"; }

template <class V>
char * PyVector_baseName()
{
	char * path = PyVector_fullName<V>();
	char * pBase = strrchr( path, '.' );
...
public:
	...
	PY_AUTO_MODULE_STATIC_METHOD_DECLARE( RETDATA, getRegister, ARG( uint8, END ) )
