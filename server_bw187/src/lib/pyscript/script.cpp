...
#define PATHPREFIX "../res/"
	...
	sysPaths = PATHPREFIX+pythonPaths+";"PATHPREFIX"entities/common"
		";"PATHPREFIX"entities/common/Lib"
#ifdef _WIN32
		";"PATHPREFIX"entities/common/DLLs"
#else
		";"PATHPREFIX"entities/common/lib-dynload"
#endif
		;
...
PyThreadState* Script::createInterpreter()
{
	...
	if (pNewInterpreter)
	{
		...
		// TODO: __kyl__ (26/10/2005) s_pOurInitTimeModules is only available if
		// WITH_THREAD is defined. Should probably have another flag to indicate
		// that we want to have s_pOurInitTimeModules. libpyscript is currently
		// compiled with WITH_THREAD so this works for now.
		...
		MF_ASSERT( pSwapped == pNewInterpreter );
	}
...
void Script::destroyInterpreter( PyThreadState* pInterpreter )
{
	...
	MF_ASSERT( pInterpreter != PyThreadState_Get() );
...
{
	MF_ASSERT( s_defaultContext == NULL );
...
{
	MF_ASSERT( s_defaultContext == PyThreadState_Get() );
...
void Script::releaseLock()
{
	...
	MF_ASSERT( oldState == s_defaultContext )
}
 ...
 *	Any error generates an exception, which is printed out if printException
 *	 is true, otherwise it is left as the set python exception.
 ...
 *	the stack, and we clear the error ( e.g. PyObject_GetAttrString( func )
	...
	const char * errorPrefix, bool okIfFunctionNull, bool printException )
{
	...
	MF_ASSERT(!PyErr_Occurred() || (okIfFunctionNull && pFunction == NULL));
	...
	else
	{
		if (pArgs == NULL || !okIfFunctionNull)
		{
			PyErr_Format( PyExc_ValueError,
				...
				"\tFunction at 0x%08x, Args at 0x%08x",
				errorPrefix, unsigned(pFunction), unsigned(pArgs) );
		}
		else
		{
	...
	if (printException)
	{
		...
		if (pErr != NULL)
		{
			...
			ERROR_MSG( "%sScript execution returned the error '%s'\n",
				errorPrefix,
				PyString_AsString( pErrStr ) );
			Py_DECREF( pErrStr );
...
memoryCounterDefine( python, Entity );
	...
	const char * varName )
{
	...
	if (PyLong_Check( pObject ))
	{
		...
		if (!PyErr_Occurred())
		{
			WARNING_MSG( "Assigning value %lu (greater than 2147483647) to an "
