...
memoryCounterDefine( bsp, Geometry );
...
BSP::~BSP()
{
	memoryCounterSub( bsp );
	memoryClaim( triangles_ );
}
...
{
	MF_ASSERT( triangles.size() == polygons.size() );
...
bool BSP::save( FILE * pFile, const WorldTriangle * pFront ) const
{
	...
	{
		ERROR_MSG( "BSP::save: There are too many triangles. %d >= %d\n",
...
{
	MF_ASSERT( sizeof( WorldTriangle ) == 40 );
	bool result = false;
	...
	if (pRoot_ == NULL)
	{
		...
		{
			MF_ASSERT( unsigned( bspFile.size() ) >=
			...
			MF_ASSERT(bspFile.size() >= size);
			char * data = new char[size];
			bspFile.read(data, sizeof(char), size);			
			this->setUserData(type, new BinaryBlock(data, size));
			delete [] data;
		}
...
bool BSPTree::save( const std::string & filename ) const
{
	...
	if (triangles_.size() > 0xffff)
	{
		ERROR_MSG( "BSPTree::save: "
				"Tree size (%d) is bigger than max size (%d)\n",
...
/**
 *	For viewing the bsp tree
 */
void BSPTree::createVertexList( Moo::VertexXYZL * & verts, int & nVerts,
	uint32 colour) const
{
	nVerts = 0;
    ...
    verts = new Moo::VertexXYZL[nVerts];

	Moo::VertexXYZL * currentVert = verts;
		...
		it++)
	{
		...
		currentVert->colour_ = colour;
		(currentVert++)->pos_ = tri.v0();
		currentVert->colour_ = 0xFFFFFFFF;
		(currentVert++)->pos_ = tri.v1();
		currentVert->colour_ = colour;
		(currentVert++)->pos_ = tri.v2();
	}
}
#endif
