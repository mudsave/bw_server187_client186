	...
	if ( pTarget_ && pSource_ )
	{
		...
		*/
				...
				limitVelocity_)
		{
			...
			if ( !limitVelocity_ )
			{
				float dist = Math::decay( oldDist, newDist,
					this->movementHalfLife(), deltaTime );
				cameraPosInWS_ = positionOnEntity + (dist/newDist) * (idealPosInWS - positionOnEntity);
			}
			else
			{
...
PY_BEGIN_ATTRIBUTES( CursorCamera )
	...
	/*~ atrribute CursorCamera.pivotMinDist
	 *
	...
	/*~ atrribute CursorCamera.terrainMinDist
	 *
	 *	This attribute has been deprecated, and no longer performs a useful
	 *	function.
	 *
	 ...
	 *	Vector4Provider and use the x,y values as pitch,yaw adjustments.
