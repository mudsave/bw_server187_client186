...
/**
 *	This struct records the state of a direction cursor at a point in time
 */
struct DCState
{
	DCState() {}
	DCState( Angle p, Angle y, Angle r ) :
	...
	DCState operator+( const DCState & a ) const
		{ return DCState( pitch + a.pitch, yaw + a.yaw, roll + a.roll ); }

	DCState operator-( const DCState & a ) const
		{ return DCState( pitch - a.pitch, yaw - a.yaw, roll - a.roll ); }

	DCState operator*( float c ) const
		{ return DCState( pitch * c, yaw * c, roll * c ); }
};


static AnnalSynchronised< DCState, TweenNumericAlt<DCState> >	s_dcAnnal;
...
void DirectionCursor::tick( float deltaTime )
{
	...
	{
		s_dcAnnal.push( DCState( pitch_, yaw_, roll_ ) );
	}
