...
//for the background loading task.
...
MPSBackgroundTask::MPSBackgroundTask( const std::string& filename, PyObject * callback ):
	BackgroundTask( &doBackgroundTask, this, &doMainThreadTask, this ),
	filename_( filename ),
	pCallback_( callback ),
	pSystem_( NULL )
{
 ...
 */
void MPSBackgroundTask::doBackgroundTask( void * self )
{
	MPSBackgroundTask * ths = static_cast<MPSBackgroundTask*>(self);
	ths->pSystem_ = new MetaParticleSystem();
	ths->pSystem_->load( ths->filename_, "" );	
}
 ...
 *	@param	mgr		The background task manager instance
 */
void MPSBackgroundTask::doMainThreadTask( void * self )
{
	MPSBackgroundTask * ths = static_cast<MPSBackgroundTask*>(self);

	PyMetaParticleSystem* pMPS = new PyMetaParticleSystem( ths->pSystem_ );
	Py_INCREF( ths->pCallback_.getObject() );		
	Script::call( &*(ths->pCallback_),
...
{	
	MPSBackgroundTask* pTask = new MPSBackgroundTask( filename, callback );
	BgTaskManager::instance()->addTask(*pTask);
	Py_Return;
}
