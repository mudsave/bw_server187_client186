...
public:
	...
	// accessors for editor gizmos
	Vector3 start() const { return origin_; }
	void start(const Vector3 & pos) { origin_ = pos; }

	Vector3 end() const { return origin_ + direction_; }
	void end(const Vector3 & end) { direction_ = end - origin_; }
