...
std::string BarrierPSA::reactionTypeNames_[BarrierPSA::REACTION_MAX] = { "Bounce", "Remove", "Allow" };
		...
		float dTime )
{
	...
	while ( current != particleSystem.end() )
	{
		...
		Vector3 posAfter = particle.position() + dTime * particle.getTempVelocity();
		xDiff = positionOfCylinder.x - posAfter.x;
		zDiff = positionOfCylinder.z - posAfter.z;
		float afterSquared = xDiff * xDiff + zDiff * zDiff;

		if ( ( beforeSquared > radiusSquared ) ^ ( afterSquared > radiusSquared ) )
		{
			switch ( reaction_ )
			{
			case REMOVE:
		...
		float dTime )
{
	...
	while ( current != particleSystem.end() )
	{
		...
		Vector3 velocity;
		particle.getVelocity(velocity);
		Vector3 posAfter = posNow + dTime * velocity;
		...
		float dTime )
{
	...
	while ( current != particleSystem.end() )
	{
		...
		if ( ( distSqrNow > radiusSquared ) ^ ( distSqrAfter > radiusSquared ) )
		{
			switch ( reaction_ )
			{
			case REMOVE:
 ...
 *	@return A new BarrierPSA object.
 */
