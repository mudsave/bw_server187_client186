 ...
 *
 *	@param	particleSystem	A reference to the particle system that will be
 *							drawn.
 */
...
void PointSpriteParticleRenderer::realDraw( const Matrix& worldTransform, Particles::iterator beg, Particles::iterator end )
{
	...
	Moo::rc().setVertexDeclaration( pVertexDeclaration_->declaration() );
	
	Moo::rc().setPixelShader( NULL );

	//vertex shader constants (transforms and lighting)
	Vector4 constants( 0.f, 0.5f, 1.f, 2.f );
	device->SetVertexShaderConstantF( 0, static_cast<const float*>( constants ), 1 );
	...
	device->SetVertexShaderConstantF( 1, (float*)&vp, 4 );

	//5 - fog
	device->SetVertexShaderConstantF(  5,
	...
	device->SetVertexShaderConstantF(  6, (const float*)Vector4(Moo::rc().screenHeight(),0.f,0.f,0.f ), 1  );
...
/**
 *	This method creates the vertex shader required for
 *	drawing point sprites that have individual sizes.
 */
void PointSpriteParticleRenderer::createUnmanagedObjects()
{
	...
	if( pVertexShader )
	{
		...
		if (FAILED( Moo::rc().device()->CreateVertexShader( (DWORD*)pVertexShader->data(), &vertexShader_ ) ) )
		{
			CRITICAL_MSG( "PointSpriteParticleRenderer::createShaders - couldn't create vertexshader!\n" );
		}
	}

	pVertexDeclaration_ = Moo::VertexDeclaration::get( "particle" );
}


/**
 *	This method frees the vertex shader.
 */
void PointSpriteParticleRenderer::deleteUnmanagedObjects()
{
