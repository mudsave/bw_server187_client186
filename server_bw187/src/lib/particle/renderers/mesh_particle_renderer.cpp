...
static AutoConfigString s_solidEffect("system/meshParticles/solid");
static AutoConfigString s_additiveEffect("system/meshParticles/additive");
static AutoConfigString s_blendedEffect("system/meshParticles/blended");
...
public:
	...
	{
		MF_ASSERT(i<PARTICLE_MAX_MESHES);
		transforms_[i] = orig;
		Matrix m;
...
bool MeshParticleRenderer::beginDraw(const BoundingBox & inbb)
{
	...
	{		
		...
		{			
			...
			{
				ERROR_MSG( "MeshParticleRenderer::drawOptimisedVisual: Couldn't set primitives\n" );
				return false;
			}
...
void MeshParticleRenderer::draw( const BoundingBox& bb, int primGroupIndex )
{
	...
	Moo::rc().addToPrimitiveGroupCount( 1 );
...
void MeshParticleRenderer::updateMaterial()
{
	...
	{
		std::string effectName( s_solidEffect );
		switch ( materialFX_ )
		{
		/*case FX_SOLID:
			effectName = s_solidEffect;
			break;*/
		case FX_ADDITIVE:
			effectName = s_additiveEffect;
			break;
		case FX_BLENDED:
			effectName = s_blendedEffect;
			break;			
		}
