...
memoryCounterDefine( chunk, Scene );
...
std::vector<Chunk::TouchFunction> * Chunk::touchType_ = NULL;
/**
 *	Constructor.
 */
...
// Editor will call this when it's already loaded to recreate the chunk
	...
	// then read out our transform and bounding box
	...
	if( !isOutsideChunk() )
	{
		...
		{
			ChunkItemFactory::Result res = this->loadItem( shellSection );
			good &= bool(res);
		}
		if (!good)
		{
		...
		if( !good )
		{
			ERROR_MSG( "Chunk::load: Failed to load shell mode for chunk %s\n", identifier_.c_str() );
			loaded_ = true;
			return false;
		}
	}
	...
	{
		boundingBox_.setBounds( pSection->readVector3( "boundingBox/min" ),
								pSection->readVector3( "boundingBox/max" ) );
		boundingBox_.transformBy( pMapping_->mapper() );

		centre_ = boundingBox_.centre();

		localBB_ = boundingBox_;
		localBB_.transformBy( transformInverse_ );
	...
	if ( !this->loadBoundaries( pSection ) )
	{
		good = false;
...
#ifdef EDITOR_ENABLED
	// the first object in the item is the shell model
	if (!isOutsideChunk_ && !selfItems_.empty() )
	{
		selfItems_.front()->edBounds( localBB_ );
		this->visibilityBox_ = localBB_;
	}
#endif//EDITOR_ENABLED
	...
	for (int i = 0; i < Chunk::nextCacheID_; i++)
	{
		...
		(*Chunk::touchType_->operator[](i))( *this );
...
bool Chunk::loadInclude( DataSectionPtr pSection, const Matrix & flatten, std::string* errorStr )
{
	...
	for (DataSectionIterator it = pSection->begin(); it != end; it++ )
	{
		...
		else if (stype == "model")
		{
			...
			{
				std::string resourceID = (*it)->readString( "resource" );
				ERROR_MSG( "Exception occurs in chunk %s loading model %s reason %s.\n", identifier_.c_str(), resourceID.c_str(), err.what() );
			}
			catch (...)
			{
				std::string resourceID = (*it)->readString( "resource" );
				ERROR_MSG( "Exception occurs in chunk %s loading model %s.\n", identifier_.c_str(), resourceID.c_str() );
			}
		}
		else
		{
 ...
 */
bool Chunk::loadBoundaries( DataSectionPtr pSection )
{
	std::vector<DataSectionPtr>		bsects;

	bool good = true;

	pSection->openSections( "boundary", bsects );
		...
		bit++)
	{
		...
		for (uint i=0; i < (*bit)->unboundPortals_.size(); i++)
		{
			...
			if (p.hasChunk() && p.pChunk->mapping()->condemned())
			{
				...
				MF_ASSERT( pOthMapping != pMapping_ );
				MF_ASSERT( !p.pChunk->ratified() );	// since condemned
			...
			if (!p.hasChunk())
			{
				...
				// if we didn't find anything there, then leave it
				//  unconnected (this is bad ... all should be loaded)
				if (pFound == NULL)
				{
					WARNING_MSG( "Chunk::bind(form): "
						"Cannot find any chunk to connect to at (%f,%f,%f) column @ 0x%08X\n",
						conPt[0], conPt[1], conPt[2], pCol );
					continue;
				}
				...
				{
					DEBUG_MSG( "bind(form) in %s could not connect "
						"portal at (%f,%f,%f) to %s\n",
						identifier_.c_str(),
						p.centre[0], p.centre[1], p.centre[2],
						pFound->identifier_.c_str() );
					continue;
				}
			...
			if (p.pChunk->online())
			{
				...
				// let it know we're online
				ChunkBoundary::Portal* portal = pOnlineChunk->bind( this );

				if (portal)
				{
					// and move it to the bound portals list
					(*bit)->bindPortal( i-- );
					this->onBound( pOnlineChunk, portal );
				}
		...
		for (uint i=0; i < (*bit)->boundPortals_.size(); i++)
		{
			...
			if (cut)
			{
				if (!this->isOutsideChunk() && p.pChunk->isOutsideChunk())
					...
					p.pChunk = NULL;
				// note: bounds_ not updated

				DEBUG_MSG( "loose(cut) in %s disconnected "
					"own portal at (%f,%f,%f) to state %d\n",
					identifier_.c_str(),
					p.centre[0], p.centre[1], p.centre[2],
					p.pChunk );
			}
 ...
 */
ChunkBoundary::Portal* Chunk::bind( Chunk * pChunk )
{
	// go through all our boundaries
	for (ChunkBoundaries::iterator bit = joints_.begin();
		bit != joints_.end();
		bit++)
	{
			...
			pit != (*bit)->unboundPortals_.end(); pit++)
		{
			...
			{				
				ChunkBoundary::Portal* pPortal = *pit;
...
/**
 *	Private function called when a two way portal has been bound
 */
void Chunk::onBound( Chunk* other, ChunkBoundary::Portal* portal )
{
#if UMBRA_ENABLE
	// Create umbra portal
	portal->createUmbraPortal( other );
#endif
	other->notifyCachesOfBind( false );
}

namespace // anonymous
{
	/**
	 * Calculate the world space bounding box containing all the points in the portal,
	 * and 5cm to either side of the portals plane
	 */
	BoundingBox portalBounds( ChunkBoundary::Portal& portal, Chunk* chunk )
	{
		BoundingBox bb( portal.centre, portal.centre );

		ChunkBoundary::V2Vector::iterator i = portal.points.begin();
		for (; i != portal.points.end(); ++i)
		{
			Vector2 v = *i;
			Vector3 localV = (portal.uAxis * v.x) + (portal.vAxis * v.y) + (portal.plane.normal() * portal.plane.d());
			Vector3 worldV = chunk->transform().applyPoint( localV );
			bb.addBounds( worldV );
		}

		// Now expand the bb by 5cm on either side wrt to the plane normal
		Vector3 worldPlaneNormal = chunk->transform().applyVector( portal.plane.normal() );

		bb.addBounds( portal.centre + worldPlaneNormal * .05f );
		bb.addBounds( portal.centre + worldPlaneNormal * -.05f );
	...
	bool contains( const BoundingBox& a, const BoundingBox& b )
	{
		...
		return (
			(a.minBounds().x - epsilon < b.minBounds().x) &&
			(a.minBounds().y - epsilon < b.minBounds().y) &&
			(a.minBounds().z - epsilon < b.minBounds().z) &&
			(a.maxBounds().x + epsilon > b.maxBounds().x) &&
			(a.maxBounds().y + epsilon > b.maxBounds().y) &&
			(a.maxBounds().z + epsilon > b.maxBounds().z)
			);
	}
	...
	{
		MF_ASSERT( chunkA != chunkB );

		// ensure both the portals are available (ie, not heaven, earth, or invasive)
		if (portalA.pChunk != NULL || portalB.pChunk != NULL)
			return false;

		// for now, just compare their centres. in the
		//  future we'll need a much better way to do this!
		//return ((portalA.centre - portalB.centre).lengthSquared() < 0.04f);

		/*	// Early out, not really needed
		...
		if ((n1 + n2).length() > .001f)
			return false;
		*/

		BoundingBox bbA = portalBounds( portalA, chunkA );
		BoundingBox bbB = portalBounds( portalB, chunkB );

		return contains( bbA, bbB ) || contains( bbB, bbA );
	}
} // namespace anonymous
...
bool Chunk::formPortal( Chunk * pChunk, ChunkBoundary::Portal & oportal )
{
			...
			bit++)
		{
				...
				pit != (*bit)->unboundPortals_.end(); pit++)
			{
				if ( canBind( oportal, **pit, pChunk, this ) )
				{
					...
					DEBUG_MSG( "formPortal: "
						"Chunk %s connecting portal at (%f,%f,%f) "
						"to portal in %s at (%f,%f,%f)\n",
						identifier_.c_str(), (*pit)->centre[0],
						(*pit)->centre[1], (*pit)->centre[2],
						pChunk->identifier_.c_str(), oportal.centre[0],
						oportal.centre[1], oportal.centre[2] );
	...
	DEBUG_MSG( "formPortal: "
		"Chunk %s forming a new internal portal to connect "
		"to portal in %s at (%f,%f,%f)\n",
		identifier_.c_str(), pChunk->identifier_.c_str(),
		oportal.centre[0], oportal.centre[1], oportal.centre[2] );
			...
			ppit != (*bit)->boundPortals_.end(); ppit++)
		{
			...
			if (pit->pChunk == pChunk)
			{
				...
				if (cut)
				{
					...
					if (pit->internal)
					{
						DEBUG_MSG( "loose(%s,cut) in chunk %s deleted internal "
							"portal at (%f,%f,%f) under orders\n",
							pChunk->identifier_.c_str(), identifier_.c_str(),
							pit->centre[0], pit->centre[1], pit->centre[2] );
					...
					DEBUG_MSG( "loose(%s,cut) in chunk %s disconnected "
						"portal at (%f,%f,%f) under orders\n",
						pChunk->identifier_.c_str(), identifier_.c_str(),
						pit->centre[0], pit->centre[1], pit->centre[2] );
				}
...
void Chunk::jogForeignItems()
{
	...
	for (int i = 0; i < diSize; i++)
	{
		...
		// Keep a reference to this object so it can't be deleted inside nest.
		ChunkItemPtr cip = (*it); 
		cip->nest( pSpace_ );
...
void Chunk::drawBeg()
{
	...
	if (drawSelf)
	{
		...
		++ChunkManager::s_chunksVisible;
			...
			contractBox.expandSymmetrically(offset, offset, offset);
		...
		)
	{
		...
		if (Moo::rc().reflectionScene())
		{
			...
			++ChunkManager::s_chunksReflected;
				...
				float offset = -10.0f * std::min(7, ChunkManager::s_drawPass);
				refectedtBox.expandSymmetrically(offset, offset, offset);
...
{
	if (touchType_ == NULL) touchType_ = new std::vector<TouchFunction>;
	touchType_->push_back( tf );
...
void Chunks_drawCullingHUD_Priv()
{
	#define DRAW_VBOXES(containter, colour)							\
	{																\
		BBoxVector::const_iterator travIt  = containter.begin();	\
		BBoxVector::const_iterator travEnd = containter.end();		\
	...
	DRAW_VBOXES(s_traversedChunks, Moo::Colour(0.5, 0.5, 0.5, 1.0));
	DRAW_VBOXES(s_visibleChunks, Moo::Colour(1.0, 0.0, 0.0, 1.0));
	DRAW_VBOXES(s_reflectedChunks, Moo::Colour(0.0, 0.0, 1.0, 1.0));
