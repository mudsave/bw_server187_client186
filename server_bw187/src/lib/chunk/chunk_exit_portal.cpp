...
#if UMBRA_ENABLE
	  ChunkItem( 3 ),	//need tick for Umbra
#else
	  ChunkItem( 1 ),	//just draw without Umbra
#endif
 ...
 *	be a code error, for example BigBang saving out ChunkExitPortals
...
{
	MF_ASSERT( "No!  Don't load these from file.. they should only be created"
				" by chunks, when an exit portal is bound.");
	return false;
}
