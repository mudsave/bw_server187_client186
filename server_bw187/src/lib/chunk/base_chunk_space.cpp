...
memoryCounterDefine( focusGrid, Substance );
...
BaseChunkSpace::~BaseChunkSpace()
{
	...
	// chunk in the currentChunk's last vector and delete it.  This will in
...
Chunk * BaseChunkSpace::findOrAddChunk( Chunk * pChunk )
{
	...
	if (found != currentChunks_.end())
	{
		for (uint i = 0; i < found->second.size(); i++)
		{
			...
			{
				if (pChunk != found->second[i]) delete pChunk;
...
void BaseChunkSpace::delChunk( Chunk * pChunk )
{
	...
	if (found != currentChunks_.end())
	{
		...
		if (found->second.empty()) currentChunks_.erase( found );
	}
}
...
BaseChunkSpace::Column::~Column()
{
	...
	{
		memoryClaim( holdings_[i] );
		delete holdings_[i];
	}
	memoryClaim( holdings_ );
...
void BaseChunkSpace::Column::delDynamicObstacle( const ChunkObstacle & obstacle)
{
	...
	*iter = heldObstacles_.back();
	...
	MF_ASSERT( found2 != elements.rend() );
...
void BaseChunkSpace::emulate( SmartPointer<BaseChunkSpace> pRightfulSpace )
{
	...
	MF_ASSERT( pRightfulSpace->currentChunks_.empty() );
}
