...
memoryCounterDeclare( focusGrid );
 ...
 */
PhotonOccluder::Result ChunkObstacleOccluder::collides(
	...
	const LensEffect& le )
{
	...
	else
	{
		if (obVisitor.hitBlended())
		{
			...
			result = PhotonOccluder::DAPPLED;
		}
		else
		{
			//sun is completely unobstructed
			result = PhotonOccluder::NONE;
		}
	}
...
float ChunkRompTerrainCollider::ground( const Vector3 & pos )
{
	...
	ChunkSpace * pCS = &*ChunkManager::instance().cameraSpace();
	if (pCS != NULL)
		dist = pCS->collide( dropSrc, dropMax, ClosestTerrainObstacle::s_default );
	if (dist < 0.f) return RompCollider::NO_GROUND_COLLISION;
...
ClosestTerrainObstacle ClosestTerrainObstacle::s_default;
