...
/*virtual*/ bool ChunkStationNode::isValid(std::string &failureMsg) const
{
    if (id() == UniqueID())
    {
        failureMsg = "Invalid id";
        return false;
    }

    if (graph() == NULL)
    {
        failureMsg = "Not connected to a graph";
        return false;
    }

    if (links_.size() != preloadLinks_.size())
    {
        failureMsg = "links_.size() != preloadLinks_.size()";
        return false;
    }
    ...
    for (size_t i = 0; i < links_.size(); ++i)
    {
        ...
        {
            failureMsg = "link doesn't match end items";
            return false;
        }
        if (myself != this)
        ...
        {
            failureMsg = "preload is missing link";
            return false;
        }
        ...
        if (owns)
        {
            ...
            {
                failureMsg = "link direction != preload direction";
                return false;
            }
            if 
            (
    ...
    )
    {
        ...
        if (!ok)
        {
            failureMsg = "Link in preload_ not in links_";
            return false;
        }
    }
    ...
    {
        failureMsg = "Node not registered to graph";
        return false;
    }

    if (chunk() == NULL)
    {
        failureMsg = "Node not in chunk";
        return false;
    }
