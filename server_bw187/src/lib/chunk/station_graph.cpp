...
bool StationGraph::save()
{
	...
	//sure BigBang (the only one using the save() method) turns
	...
	if (pSect_ && isReady_)
	{
        if ( nodes_.empty() )
		{
			...
			// TODO: add file deleting to BWResource
		...
		else
		{
			...
			while (it != end)
			{
				...
				if (edNode->chunk())
				{
					DataSectionPtr pNodeSect = pSect_->openSection( edNode->id(), true );
...
/*virtual*/ bool StationGraph::isValid( std::string &failureMsg ) const
{
    ...
    {
        failureMsg = "edNodes_.size() != nodes_.size()";
        return false;
    }
...
bool StationGraph::Node::load( DataSectionPtr pSect, const Matrix& mapping )
{
	...
	this->id_ = UniqueID( pSect->sectionName() );
