...
public:
	...
	void stop( const Model::Action * pActionSource );
...
private:
	...
	void endPlayingAction( AQElement & aqe, float dTime, float endat );
	void tickPromotion( AQElement & aqe, float timeNow );
	bool mollifyWorldTransform();
...
public:
	...
	PY_AUTO_METHOD_DECLARE( RETOWN, action, ARG( std::string, END ) )


	PY_RO_ATTRIBUTE_DECLARE( daction()->pFirstAnim->duration_, duration );
	PY_RO_ATTRIBUTE_DECLARE( daction()->pSource->blendInTime_, blendInTime );
	PY_RO_ATTRIBUTE_DECLARE( daction()->pSource->blendOutTime_, blendOutTime );
	PY_RO_ATTRIBUTE_DECLARE( daction()->pSource->track_, track );
	PY_RO_ATTRIBUTE_DECLARE( daction()->pSource->filler_, filler );
	PY_RO_ATTRIBUTE_DECLARE( (daction()->pSource->track_ == -1), blended );
