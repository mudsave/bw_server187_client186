...
PY_BEGIN_METHODS( SkeletonCollider )
	 ...
	 *
	 *	attempt to collide with this collider
...
bool SkeletonCollider::collide( const Vector3& start, const Vector3& direction, float& distance )
{
	...
	while (it != end)
	{
		if ((*it)->collide( start, direction, distance ))
		{
			ret = true;
			impactCollider_ = *it;
		}
	...
	if (ret)
	{
		impactReflection_ = Vector3(0,0,0) - direction;
		impactPoint_ = start + direction * distance;
	}
	return ret;
}
