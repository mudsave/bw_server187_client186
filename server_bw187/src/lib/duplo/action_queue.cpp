...
{ void callNextFrame(
	PyObject * fn, PyObject * args, const char * reason, double age = 0.0 ); }
#endif

DECLARE_DEBUG_COMPONENT2( "Model", 0 )

#define myCounterConditionalPre( vec, statements )						\
	bool vec##_willGrow = (vec.size() == vec.capacity());				\
	if (vec##_willGrow) { memoryCounterSub( superModel ); statements }	\

#define myCounterConditionalPost( vec, statements )						\
	if (vec##_willGrow) { memoryCounterAdd( superModel ); statements }	\

memoryCounterDeclare( superModel );
...
/// Constructor
ActionQueue::ActionQueue( PyModel & model ) :
	model_( model ),
...
ActionQueue::~ActionQueue()
{
	...
	memoryClaim( actionState_ );
	memoryClaim( fashionState_ );
}
...
void ActionQueue::flush()
{
	...
	{
		if (iter->action) iter->action->finish = -1.f;
	}
	actionState_.clear();
	...
	CueResponses * responses, bool promoteMotion, SuperModelActionPtr link )
{
	if (link)
	{
		...
		if (aqe == NULL) for (uint i = 1; i < actionState_.size(); i++)
		{
			...
			if (look->action == link && head.action->passed <
				head.action->finish - head.action->pSource->blendOutTime_ )
			{
				haqe = &head;
				aqe = look;
				break;
		...
		if (aqe != NULL)
		{
			...
			haqe->action->finish = action->pSource->filler_ ?
				-1.f : haqe->action->finish + action->pFirstAnim->duration_;
			return;
		}
	}
	...
	action->passed = -afterTime;
	action->played = 0.f;
	action->finish = action->pSource->filler_ ?
		-1.f : action->pFirstAnim->duration_;
	action->lastPlayed = 0.f;

	myCounterConditionalPre( waiting_, memoryClaim( waiting_ ); )
	waiting_.push_back( newAQE );
	myCounterConditionalPost( waiting_, memoryClaim( waiting_ ); )

	MF_ASSERT( responses == NULL || !responses->empty() );
 ...
 */
void ActionQueue::stop( const Model::Action * pActionSource )
{
	for (uint i = 1; i < actionState_.size(); i++)
	{
		AQElement & aqe = actionState_[i];
		if (aqe.action->pSource == pActionSource)
		{
			float newFinish = aqe.action->passed +
				aqe.action->pSource->blendOutTime_;
			if (aqe.action->finish < 0.f || newFinish < aqe.action->finish)
			{
				//DEBUG_MSG( "Setting finish on action '%s' to %g\n",
				//	aqe.action->actionName().c_str(), aqe.finish );
				aqe.action->finish = newFinish;
			}
		}
	}
	...
	float actionTime, bool promoteMotion )
{
	...
	// if there are other primary actions around, then this must be NULL
	for (uint i=1; i < actionState_.size(); i++)
	{
		AQElement	& aqe = actionState_[i];
		SuperModelActionPtr ia = aqe.action;
		if (ia->pSource->track_ == 0 && (ia->finish < 0.f ||
			(ia->passed + dTime) < 
			(ia->finish - ia->pSource->blendOutTime_)))
		{
			action = NULL;
			actionTime = -1.f;
			lastMatchAccepted_ = false;
	...
	if (matchState.action)
	{
		...
		if (matchState.action->pSource->isMovement_ &&
			matchState.action->pFirstAnim->looped_)
		{
			cycleParam = fmodf( matchState.action->played /
				matchState.action->pFirstAnim->duration_, 1.0 );
		}

		matchState.action->finish = matchState.action->passed +
			matchState.action->pSource->blendOutTime_ *
			matchState.action->blendRatio();

		// now whack it on the back
		myCounterConditionalPre( actionState_, memoryClaim( actionState_ ); )
		myCounterConditionalPre( fashionState_, memoryClaim( fashionState_ ); )
		...
		myCounterConditionalPost( actionState_, memoryClaim( actionState_ ); )
		myCounterConditionalPost( fashionState_, memoryClaim( fashionState_ ); )
	...
	if (action)
	{
		...
		float playedTime = action->pSource->isMovement_ ?
			cycleParam * action->pFirstAnim->duration_ : 0.f;
		...
		newMatchState.action->played = playedTime;
		newMatchState.action->passed = passedTime;
		newMatchState.action->finish = -1.f;	// these are all like fillers
		newMatchState.action->lastPlayed = playedTime;
		...
		if (eraseIdx > 0)
		{
			MF_ASSERT( eraseIdx < actionState_.size() &&
...
float ActionQueue::matchCycleCount()
{
	...
	if (action)
	{
		return action->played / action->pFirstAnim->duration_;
	}
	return 0;
}
 ...
 */
void ActionQueue::tick( float dTime )
{
	...
	if (needsMollifying_)
	{
		needsMollifying_ = this->mollifyWorldTransform();
	}
	...
	for (uint i = 0; i < waiting_.size(); i++)
	{
		...
		if (aqe.action->passed + dTime >= 0)
		{
			// pretend it's played for that long too
			aqe.action->played = aqe.action->passed;

			// if we're not on track -1, first stop any
			//  actions already playing on the same track
			if (aqe.action->pSource->track_ != -1)
			{
				int track = aqe.action->pSource->track_;
				for (uint j = 1; j < actionState_.size(); j++)
				{
					AQElement & jaqe = actionState_[j];
					if (jaqe.action->pSource->track_ == track)
					{
						float newFinish = jaqe.action->passed +
							jaqe.action->pSource->blendOutTime_;
						float & oldFinish = jaqe.action->finish;
						if (oldFinish < 0.f || newFinish < oldFinish)
						{
							//DEBUG_MSG( "Setting finish on action '%s' to %g\n",
							//	jaqe.action->pSource->name_.c_str(), newFinish );
							oldFinish = newFinish;
						}
					}
				}
				// don't forget the matched action
			...
			//	aqe.action->pSource->name_.c_str(),
			//	aqe.action->finish );
		...
		else
		{
			...
			// passed time
			aqe.action->passed += dTime;
		}
	}

	MF_ASSERT( Moo::CueShot::s_cueShots_.empty() );
	...
	for (uint i = 1; i < actionState_.size(); i++)
	{
		...
		aqe.action->passed += dTime;
		aqe.action->played += dTime;

		// move on the links while they're being used up
		while (aqe.link != NULL && aqe.action->played >
			aqe.action->pFirstAnim->duration_)
		{
			// set up link
			float odur = aqe.action->pFirstAnim->duration_;
			aqe.link->action->passed = aqe.action->passed;
			aqe.link->action->played = aqe.action->played - odur;
			aqe.link->action->finish = aqe.action->finish;
			aqe.link->action->lastPlayed = 0.f;

			//DEBUG_MSG( "Following link from action '%s' to '%s',\n"
			//	" passed %g, finish %g, played %g\n",
			//	aqe.action->pSource->name_.c_str(),
			//	aqe.link->action->pSource->name_.c_str(),
			//	aqe.action->passed, aqe.action->finish,
			//	aqe.action->played - odur );

			// end old action
			this->endPlayingAction( aqe, dTime, odur );
		...
		if (aqe.action->finish >= 0.f && aqe.action->finish < aqe.action->passed)
		{
			//DEBUG_MSG( "Stopping action '%s', finish %g\n",
			//	aqe.action->pSource->name_.c_str(), aqe.action->finish );

			float overShot = (aqe.action->passed-aqe.action->finish);
			this->endPlayingAction( aqe, dTime,
			//	aqe.action->pFirstAnim->duration_ );
				aqe.action->played - overShot );
	...
	{
		AQElement & aqe = actionState_[i];
		aqe.action->tick( *model_.pSuperModel(), dTime );
	...
	CueResponse::respond( 0, responses_, &model_ );
 ...
 */
void ActionQueue::endPlayingAction( AQElement & aqe, float dTime, float endat )
{
	// find out how much we overshot by
	float overShot = aqe.action->played - endat;

	// tick it internally up until the end
	float rplay = aqe.action->played;
	aqe.action->played = endat;
	aqe.action->tick( *model_.pSuperModel(), dTime - overShot );
	aqe.action->played = rplay;
	...
	if (aqe.promoteMotion)
	{
		...
		aqe.action->played -= overShot;
		this->tickPromotion( aqe, dTime - overShot );
 ...
 */
void ActionQueue::tickPromotion( AQElement & aqe, float dTime )
{
	SuperModelActionPtr action = aqe.action;

	// what proportion had we added last time?
	float lastProp = (action->played - dTime) / action->pFirstAnim->duration_;
	if (lastProp < 0) lastProp = 0;

	// what proportion do we want to make it now?
	float thisProp = action->played / action->pFirstAnim->duration_;
	...
	if (action->pSource->isMovement_)
	{
		Matrix move = action->pFirstAnim->flagFactorBit( 0 );
		move.invert();
	...
	else if (action->pSource->isImpacting_)
	{
		...
		padd[0][0] = lastProp * action->pFirstAnim->duration_;
		action->pFirstAnim->flagFactor( 4, padd );
		//padd.preMultiply( promoter );

		nadd[0][0] = thisProp * action->pFirstAnim->duration_;
		action->pFirstAnim->flagFactor( 4, nadd );
		//nadd.preMultiply( promoter );

		/*!
		float	lastFrame = lastProp * action->pFirstAnim->duration_ * action.frameRate() +
			action.firstFrame();

		float	thisFrame = thisProp * action->pFirstAnim->duration_ * action.frameRate() +
			action.firstFrame();
 ...
 */
bool ActionQueue::mollifyWorldTransform()
{
	...
	// Get the old world
	const Matrix oldWorld = model_.worldTransform();
	...
	delRot.minimise();
	float dangle = fabsf( acosf( delRot.w ) );
	...
	model_.worldTransform( newWorld );
...
void ActionQueue::debugTick( float dTime )
{
	...
	for (uint i = ibegin; i < iend; i++)
	{
		...
		for (j = 0; j < s_aqhist.size(); j++)
		{
			...
			if (s_aqhist[j].smap_->pSource == aqe.action->pSource)
...
void ActionQueue::debugDraw()
{
	...
	for (uint i = 0; i < s_aqhist.size(); i++)
	{
		...
		myfont->drawString( aqeh.smap_->pSource->name_,
 ...
 *	self.model.action("Climb")(0.5, self.onClimbFinished, 1)
 *	@}
 *
...
PyObject * ActionQueuer::pyCall( PyObject * args, PyObject * kwargs )
{
	...
	int promoteMotion = action_->pSource->isImpacting_;
 ...
 *	by starting to blend them out (excepting matched actions)
 */
/// This method stops all instances of this action in the action queue
PyObject * ActionQueuer::py_stop( PyObject * pArgs )
{
	SuperModelActionPtr smap = this->daction();

	model_->actionQueue().stop( smap->pSource );
...
/// Get the action of the given name
PyObject * ActionQueuer::action( const std::string & aname, bool errIfNotFound )
{
	...
	if (link_ && smap->pSource->track_ != link_->pSource->track_)
	{
		PyErr_Format( PyExc_AttributeError,
			"ActionQueuer.%s cannot continue from %s "
			"because it is on track %d and the earlier one is on %d",
			aname.c_str(), link_->pSource->name_.c_str(),
			smap->pSource->track_, link_->pSource->track_ );
		return NULL;
	}
...
/// Special get method for Python attribute 'displacement'
PyObject * ActionQueuer::pyGet_displacement()
{
	SuperModelActionPtr smap = this->daction();

	Vector3	ret( 0.f, 0.f, 0.f );
	if (smap->pSource->isMovement_)
	{
		ret = model_->initialItinerantContextInverse().applyVector(
			smap->pFirstAnim->flagFactorBit( 0 ).applyToOrigin() )*-1.f;
	}
	else if (smap->pSource->isImpacting_)
	{
		Matrix imEnd;
		imEnd[0][0] = smap->pFirstAnim->duration_;
		smap->pFirstAnim->flagFactor( 4, imEnd );

		ret = model_->initialItinerantContextInverse().applyVector(
			imEnd.applyToOrigin() );
	}
	else if (smap->pSource->isCoordinated_)
	{
		ret = model_->initialItinerantContextInverse().applyVector(
			smap->pFirstAnim->flagFactorBit( 1 ).applyToOrigin() );
	}
...
PyObject * ActionQueuer::pyGet_rotation()
{
	...
	if (smap->pSource->isMovement_)
	{
		q = Quaternion( smap->pFirstAnim->flagFactorBit( 0 ) );
	}
	else if (smap->pSource->isImpacting_)
	{
		Matrix imEnd;
	...
	{
		q = Quaternion( smap->pFirstAnim->flagFactorBit( 1 ) );
	}

	Vector3	ax( q.v() );	ax.normalise();
	Vector4 ret( ax[0], ax[1], ax[2], 2 * acosf( q.w ) );
	return Script::getReadOnlyData( ret );
}
...
/**
 *	Special get method for Python attribute 'seek'
 *
...
/**
 *	Special get method for Python attribute 'seekInv'
 *
 *	This attribute returns the seek position for another entity
...
Vector4 ActionQueuer::getSeek( const Matrix & worldIn, bool invert )
{
	...
	Matrix	coord = smap->pFirstAnim->flagFactorBit( 1 );
	/*
	coord.setRotate( smap->pSource->coordRotation() );
	if (invert)
	{
		coord.invert();
		coord.postTranslateBy( smap->pSource->coordDisplacement()*-1.f );
	}
	else
	{
		coord.preTranslateBy( smap->pSource->coordDisplacement() );
	}
	*/
...
/**
 *	Special get method for Python attribute 'impact'
 *
...
PyObject * ActionQueuer::pyGet_impact()
{
	...
	Matrix	impact;
	impact[0][0] = smap->pFirstAnim->duration_;
	smap->pFirstAnim->flagFactor( 4, impact );
/*
	impact.setRotate( smap->pSource->impactRotation() );
	impact.invert();

	impact.postTranslateBy( smap->pSource->impactDisplacement() * -1.f );
*/
