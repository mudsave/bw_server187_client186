...
static float ACCELERATION_TIME = 0.5f;
...
TrackerNodeInfo::TrackerNodeInfo( PyModel *pOwner,
		...
		float angularVelocity,
		PyTypePlus *pType ) :
	BaseNodeInfo(minPitch, maxPitch, minYaw, maxYaw, angularVelocity, pType ),
	pOwner_( pOwner ),
...
bool TrackerNodeInfo::isPrimaryNodeAnimated()
{
    ...
    if ( pPrimaryNode_->blend( Model::s_blendCookie_ ) <= 0.0f )
	{
		return false;
	}
	else
...
PyObject * TrackerNodeInfo::pyNew( PyObject * args )
{
	...
	if (!PyArg_ParseTuple( args, "OsO!sfffff",
		&pModelObj,
		&primaryNodeName,
		&PyList_Type,
		&secondaryNodesList,
		...
		&minYaw,
		&maxYaw,
		&angularVelocity) || !PyModel::Check( pModelObj ))
	{
		PyErr_SetString( PyExc_TypeError, "TrackerNodeInfo() expects: "
			...
			"float minYaw, float maxYaw, float angularVelocity" );
		return NULL;
	}
...
bool ArmTrackerNodeInfo::isPrimaryNodeAnimated()
{
	// Check if the node has been animated or not. If not, just return
	// and do nothing further.
    if ( pForeArmNode_->blend( Model::s_blendCookie_ ) <= 0.0f )
	{
		return false;
	}
	else
...
PyObject * ArmTrackerNodeInfo::pyNew( PyObject * args )
{
	...
	// first parse the easy arguments
	if (!PyArg_ParseTuple( args, "Ossffffffffffffif",
		&pModelObj,
		&upperArmNodeName,
		&foreArmNodeName,
		&minShoulderPitch,
...
bool OrthogonalNodeInfo::isPrimaryNodeAnimated()
{
	...
	// nothing further.
    if (pDirectionNode_->blend( Model::s_blendCookie_ ) <= 0.0f)
	{
		return false;
	}
	else
...
PyObject * OrthogonalNodeInfo::pyNew( PyObject * args )
{
	...
	// first parse the easy arguments
	if (!PyArg_ParseTuple( args, "Osssffffiif",
		&pModelObj,
		&pitchNodeName,
		&yawNodeName,
		&directionNodeName,
...
{
	char msg[256];
	sprintf(msg, "Tracker(0x%08x):maxNodeInfoSize=%d, nodeInfo(0x%08x):size=%d",
