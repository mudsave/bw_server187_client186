...
public:
	...
	static void onLoadCompleted( void * sModelLoader );
	static void createSuperModel( void * sModelLoader );

private:
	BackgroundTask * loadingTask_;
...
public:
	...
	PY_AUTO_METHOD_DECLARE( RETOWN, getSound,
		ARG( std::string, END ) );

	PyObject* playSound( const std::string& s );
	PY_AUTO_METHOD_DECLARE( RETOWN, playSound,
		ARG( std::string, END ) );
...
public:
	...
	PY_AUTO_METHOD_DECLARE( RETOK, cue, ARG( std::string,
...
//	void				removeFromScene();
	...
	PY_AUTO_METHOD_DECLARE( RETOK, origin, ARG( std::string, END ) )
