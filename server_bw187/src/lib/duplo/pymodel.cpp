...
#define fashionsCounterAdd	\
{ memoryCounterAdd( superModel ); memoryClaim( fashions_ ); }
#define fashionsCounterSub	\
{ memoryCounterSub( superModel ); memoryClaim( fashions_ ); }
...
PY_BEGIN_ATTRIBUTES( PyModel )
	 ...
	 *	of attachments (or allowing selective control of their individual visibility). Only meaningfull
	 ...
	 *	returns a PyMatrixProvider.  The matrix specifies what transformation
	 ...
	 *
	 *	This PyMatrixprovider can be used with the BoundingBoxGUIComponent to
...
{
	loadingTask_ = new BackgroundTask( &ScheduledModelLoader::createSuperModel, this,
						&ScheduledModelLoader::onLoadCompleted, this );
	BgTaskManager::instance()->addTask( *loadingTask_ );
}

void ScheduledModelLoader::createSuperModel( void * sModelLoader )
{
	ScheduledModelLoader * sml = (ScheduledModelLoader *) sModelLoader;
	MEM_TRACE_BEGIN( sml->modelNames_[0] )
	sml->pSuperModel_ = new SuperModel( sml->modelNames_ );
}

void ScheduledModelLoader::onLoadCompleted( void * sModelLoader )
{
	TRACE_MSG( "ScheduledModelLoader::onLoadCompleted\n" );
	ScheduledModelLoader * sml = (ScheduledModelLoader *) sModelLoader;

	// remove background task
	if (sml->loadingTask_)
	{
		delete sml->loadingTask_;
		sml->loadingTask_ = NULL;
	}

	bool successfullLoad =
		sml->modelNames_.size() == sml->pSuperModel_->nModels();

	if (successfullLoad)
	{
		MEM_TRACE_END();
		sml->pSuperModel_->checkBB( false );
		PyModel * model = new PyModel( sml->pSuperModel_ );
		Py_INCREF( sml->pBgLoadCallback_.getObject() );
		Script::call( &*(sml->pBgLoadCallback_),
	...
	else
	{
		ERROR_MSG( "Model(): Did not found all models "
			...
			sml->pSuperModel_->nModels(), sml->modelNames_.size() );
		...
		delete sml->pSuperModel_;
		sml->pSuperModel_ = NULL;
		MEM_TRACE_END()
	}
	delete sml;
}
...
static Moo::NodePtr s_getRootNode()
{
	if (!s_globalRootNode)
	{
		s_globalRootNode = Model::findNodeInCatalogue( "Scene Root" );
 ...
 *  returns an ActionQueuer object if it finds it.  Otherwise None is returned
 *	and a Python error is thrown.
 *
 *	@param	actionName		A string which is the name of the action to search
 *							for.
 *
 *	@return					The ActionQueuer object named actionName, or None
...
PyObject* PyModel::getSound( const std::string& s )
{
	...
	PySound *pSound = new PySound( pEvent );
...
#if FMOD_SUPPORT
	// TODO: Check if this is really necessary, as tick() has this too
	if (pSounds_ &&
		!pSounds_->update( this->worldTransform().applyToOrigin() ))
	{
		ERROR_MSG( "PyModel::pySet_position: "
...
void PyModel::draw( const Matrix & worldTransform, float lod )
{
	...
	{
		MF_ASSERT( WaterSceneRenderer::currentScene() );
		if ( WaterSceneRenderer::currentScene()->shouldReflect( worldTransform.applyToOrigin(), this ) )
...
{
	class NodesIDs : public Moo::ChannelDrawItem
	{
	public:
		NodesIDs()
		{
		...
		virtual void draw()
		{
			...
			StrVecVector::const_iterator nodIt  = this->nodeNLabels_.begin();
			StrVecVector::const_iterator nodEnd = this->nodeNLabels_.end();
			while (nodIt != nodEnd)
			{
				font->draw3DString(nodIt->first, nodIt->second);
				++nodIt;
			}
		}
	...
	NodesIDs * nodesIDs = new NodesIDs;
	for (int i=0; i<pSuperModel_->nModels(); ++i)
	{
		Model * model = pSuperModel_->curModel( i );
		if (model != NULL)
...
void PyModel::drawDebugInfo()
{
	...
	if (!initted)
	{
		...
		FontPtr xf = FontManager::instance().get( "system_small.font" );
...
void PyModel::drawAttachments()
{
	...
	else
	{
		...
		if (!knownNodes_.empty())	// traverse this node for blank models
		{
			...
			pSceneRoot->blendClobber( Model::s_blendCookie_, Matrix::identity );
