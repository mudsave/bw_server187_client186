...
public:
	...
	~ChunkDynamicObstacles() { MF_ASSERT( cdos_.empty() ); }
...
ChunkDynamicObstacle::ChunkDynamicObstacle( PyModelObstacle * pObstacle ) :
	ChunkDynamicEmbodiment( pObstacle, 3 | (4 << 8) )
{
			...
			if( pColumn )
			{
				BoundingBox columnBB(	Vector3((x+0) * GRID_RESOLUTION,
										...
										Vector3((x+1) * GRID_RESOLUTION,
												0.0f,
												(z+1) * GRID_RESOLUTION ) );
				columnBB.addYBounds( -2000000.0f );
				columnBB.addYBounds( +2000000.0f );
...
ChunkStaticObstacle::ChunkStaticObstacle( PyModelObstacle * pObstacle ) :
	ChunkStaticEmbodiment( pObstacle, 3 ),
...
PY_BEGIN_ATTRIBUTES( PyModelObstacle )
	 ...
	 *
	 *	This is the set of .model resources specified upon creation ( see BigWorld.PyModelObstacle() ).
	 */
