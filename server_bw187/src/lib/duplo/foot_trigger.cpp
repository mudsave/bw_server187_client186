...
static const int MAX_FOOTPRINTS = 32;
...
/**
 *	This class stores and renders footprints
 */
class FootPrints : public MainLoopTask
{
public:
	FootPrints() :
		footPrintMat_( NULL ),
		...
		enabled_(true)
	{
		MainLoopTasks::root().add( this, "Flora/FootPrints", ">App", NULL );
	}
	 ...
	 */
	void setFootPrintOption(int selectedOption)
	{
		this->enabled_ = selectedOption == 0;
	}

	//~FootPrints()
	//{ MainLoopTasks::root().del( this, "Flora/FootPrints" ); }
	...
	virtual bool init()
	{
		memoryCounterAdd( detailObjects );

		footPrintMat_ = new Moo::Material();
		footPrintMat_->load( s_mfmName );
		memoryClaim( footPrintMat_ );

		pStorage_ = new Vector3[ 4 * MAX_FOOTPRINTS ];
		memoryClaim( pStorage_ );
		...
		GraphicsSettingPtr footPrintSettings =
			Moo::makeCallbackGraphicsSetting(
				"FOOT_PRINTS", *this,
				&FootPrints::setFootPrintOption,
				-1, false, false);

		footPrintSettings->addOption("ON", true);
		footPrintSettings->addOption("OFF", true);
	...
	virtual void fini()
	{
		...
		if (footPrintMat_ != NULL)
		{
			memoryClaim( footPrintMat_ );
			delete footPrintMat_;
			footPrintMat_ = NULL;
		}
	}
	...
	/**
	 *	Store another footprint
	 */
	Vector3 * storeFootPrint()
	{
		int idx = cursor_ % MAX_FOOTPRINTS;

		if (++cursor_ >= MAX_FOOTPRINTS * 2) cursor_ = MAX_FOOTPRINTS;

		return &pStorage_[ 4 * idx ];
	}

private:
	Moo::Material	* footPrintMat_;
	int				  cursor_;
	Vector3			* pStorage_;
	bool               enabled_;
...
void FootPrints::draw()
{
	...
	int numPrints = cursor_ < MAX_FOOTPRINTS ? cursor_ : MAX_FOOTPRINTS;
	if (numPrints <= 0) return;

	vxs_.clear();
	for (int i=0; i < numPrints; i++)
	{
		Vector3 * fp = &pStorage_[ 4*i ];
		...
		vxs_.push_back( quad[0] );
		vxs_.push_back( quad[2] );
		vxs_.push_back( quad[1] );

		vxs_.push_back( quad[1] );
		vxs_.push_back( quad[2] );
		vxs_.push_back( quad[3] );
	}
...
static FootPrints s_footprints;
 ...
 */
 FootTrigger::FootTrigger( bool odd, std::string soundTagPrefix, PyTypePlus * pType ) :
	...
	lastMaterialKind_( 0 )
{
	FootTrigger::instanceCouter_++;
	this->plotIndex_ = FootTrigger::instanceCouter_;
}
	...
	if (dist >= 0.f)
	{
		...
		if (fabs(worldFoot.y - groundFoot.y) < 0.40f)
		{
			...
			if (MaterialKinds::instance().isValid( material ))
			{
				...
				// If we have a valid sound event to play, do it.  Note how much
				// Python type juggling crap I have to go through here.  Think
				// how much easier this would have been if it had been done in
				// script instead of C++.
				if (pSound_ != NULL)
				{
					PyObject *p = NULL;
			...
			if (onTerrain)
			{
				...
				Vector3	* footPrint = s_footprints.storeFootPrint();
				for (int j=0; j<4; j++)
				{
					...
					footPrint[j] = Vector3( footCorner.x,
						peq.y( footCorner.x, footCorner.z ) + 0.001f, footCorner.z );
				}
...
int  FootTrigger::instanceCouter_ = 0;
