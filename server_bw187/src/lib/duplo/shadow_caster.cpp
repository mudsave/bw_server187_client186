 ...
 */
bool ShadowCaster::init( ShadowCommon* pCommon, bool halfRes, Moo::RenderTarget* pDepthParent )
{
	bool success = false;
...
{
	ID3DXEffect* pEffect = effect.pEffect(0)->pEffect();
	...
	SAFE_SET( Texture, "depthTexture", pShadowTarget_->pTexture() );
	SAFE_SET( Float, "shadowTexelSize", 1.f / float(shadowBufferSize_) );
	SAFE_SET( Float, "shadowIntensity", pCommon_->shadowIntensity() * angleIntensity_ );
	SAFE_SET( Matrix, "shadowProj", &shadowProjection_ );
	SAFE_SET( Float, "shadowDistance", pCommon_->shadowDistance() );
	SAFE_SET( Float, "shadowFadeStart", pCommon_->shadowFadeStart() );
	SAFE_SET( Int, "shadowQuality", pCommon_->shadowQuality() );
	SAFE_SET( Vector, "shadowDir", &Vector4( view_[0][2], view_[1][2], view_[2][2], 0 ) );
}

/**
 *	This methods sets everythign up for rendering the shadow 
...
void ShadowCaster::beginReceive( bool useTerrain /* = true */ )
{
	...
	setupConstants( pReceiverOverride->pHardSkinnedEffect_ );
	...
	if (useTerrain)
	{
		Moo::TerrainRenderer::instance().draw(pCommon_->pTerrainReceiver());		
	}

	// This must currently be called even if useTerrian is false.  Because
 ...
 */
void ShadowCommon::init( DataSectionPtr pConfigSection )
{
