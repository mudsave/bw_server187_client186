...
FashionPtr PyDye::fashion()
{
		...
		it++)
	{
		...
		{
			(*it)->output( val );
			pDye_->properties[ it - dynamicPropValues_.begin() ].value_ = val;
		}
	}
	...
	if (i >= 0)
	{
		if (uint(i) < pDye_->properties.size())
		{
			...
			else
			{
				...
				return Script::setData(
					value, pDye_->properties[i].value_, propDesc );
			}
		}
...
{
	for (uint i = 0; i < other.pDye_->properties.size(); i++)
	{
		const Model::DyePropSetting & now = other.pDye_->properties[i];
		for (uint j = 0; j < pDye_->properties.size(); j++)
		{
			if (pDye_->properties[j].index_ == now.index_)
			{
				// ok, we have a match, copy the value
				pDye_->properties[j].value_ = now.value_;
