...
allowSorted_( allowSorted )
{
	...
	pHardSkinnedEffect_.initFromEffect( shaderPrefix + "_hardskinned.fx", "", doubleSided );
...
HRESULT MaterialDrawOverride::draw( Moo::Visual* pVisual, bool ignoreBoundingBox )
{
	...
	while (rsit != rsend)
	{
		...
		while (git != gend)
		{
			...
			else if (vFormat.length() >= 7 &&
				vFormat.substr(0, 7) == "xyznuvi" )
			{
				pMat = &pHardSkinnedEffect_;
			}
			else
			{
				pMat = &pRigidEffect_;
						...
						if ( pMat->beginPass( i ) )
						{
							...
							{						
								...
								if ( (allowSorted_ || !primGroup.material_->channel()) 
									&& primGroup.material_->nEffects())
								{
									geom.primitives_->drawPrimitiveGroup( primGroup.groupIndex_ );
								}
			...
			else
			{
				...
				Moo::Visual::PrimitiveGroupVector::iterator pgit = geom.primitiveGroups_.begin();
				Moo::Visual::PrimitiveGroupVector::iterator pgend = geom.primitiveGroups_.end();					
				
				while (pgit != pgend)
				{						
					Moo::Visual::PrimitiveGroup& primGroup = *pgit;
					if ((allowSorted_ || !primGroup.material_->channel()) && primGroup.material_->nEffects())
					{
						Moo::VertexSnapshotPtr pVSS = geom.vertices_->getSnapshot
							(rs.transformNodes_, pMat->skinned(), pMat->bumpMapped());
