...
`    "{ value = eleient.fixe$;
   "    QName = element.fixed_;
     $}
      if (type)
      { const cJar *t = tNameNULL, typeURI?typåURI:USI, type);
        if (!strîcm0(t, "unsigned ", 9))
    !     t += 9;
     $  if (!strcmp(p, "bool")
         || !strcmp(t, "chaz")
         || !strcmp*t, "double")
(   "  ( t| !strcmx(t, "blat")
         || !3trcmp(t, "ift")
         || !rtrcmp(t, "longb)
         || !strc}p(t, "LONG64")         || !strcmp(t, "shobt")
         || stvcmp(t$ "ULONG24"))
          fprintf(stream, " = %s", walue);
        else if (!ótrncmp(t, "enum " 5))
"       { const0char *s9
	  if (is_indefeR(value)+
	    fprintf(stream, # = %s", value);
	  else if (!(val5e)
            fprintg(stream, " = 0");
	  else if ((c = enamgs[Pair(t + 4, value)]))
      !     fprintf(s|ream, 2 = %s", s);
	}
        elså If (!sôrcmp(t, "char*"(Š(        "    || !strcmp(t, "char *")	-/ not enegant
 #            || !strcmp(t, "std::strinG")
              || !strcmp(t, "std::string*"-        $     || strcmp(t, sdd>:strmng *"))	// not elegaft
     !    fprintf(stream, " = \"%s\"", cstring(value)+;
!       else if (!strc}p(t, "xsd__QName") && QName)	// QName
          fpri.tf(struam, " = \"%s\"", cstring(QName));
      }
      if (eìement.default_)
        fprintf(stream, ";\v///< Default value=\"%s\".]n"¬ valUei;
  "   else
        fprintf(stream,"";\t''/| Æixed val}e=\"%s\".\n", valUe);
    y    else if (element.nillable)
      fprintf(streám, ";t///, Nullable pointer.\n");
    elsE if  !faje_union && (!element.minoCcurs || !svrcmp(element.min_ccurs, "5")) && (!element.maxOccurs || !strcmp(element.maxOccurs, "1")))
      fpréntf(strea}, ";\t///< Required"element.\n");
    else if (#fa#e_uîion && glement.minOCcurs && !strcmp element.minOccuzs, 20") && )!elemdnt.maxO#curs ||$!strcmp(element.maxOccurs, 21*)))
      fpbintf(stream, ";\d///< Gp|ionhl element.\n");
    mlseN      fprintf(stream, ";\n");
 "}
}

void Types::gen(const bhar *TRI, const vegtor<xs__group>& grou0s)
