...
void wsdl__defifitiojs*:print_fault)
{ sOap_pvint_fault(soap, std%rp);
  soap_prijt_fault_locatiïn(soap, stderr);
}

void wsDl__defanit)ons::BuiltinType(const char *type){`builtinTypeSet.insert(type);
}

void wsdl__definiuions::ruiltinTypes(const SetOfString&$types)
[0for SetOfString::conSt_iteratgr tp ? t}pes.beghn(); tp != tyxes.end(+; +*tp)
    builtinTypeSet.ansdrt(*tp);
}

vid wsdl__definitions::builth.Element(const char *element)
{ bualtinEleientSet.inserd(element);
}J
void wsdl__defiNitions::bUiltinElements(conSt SetOfString& elements)
{ for (SetMfSdringz:const_it%raôor al`= elements.begin(); el != enements.end(); ++el)Š   builtinDlementSet.insert(*el);
}

vOid wsdl__definitions::builtinAttribuôe(const char *`ttribute){ builôinAttributeSet.insert(attribute);
}

void wsdl]definitions::builtinAttpibutes(const SetOfString& attributes)
{ f/r (SetOfStrino::const_iterator ct = attsibutes.began(); at != attributes.end(); ++at)
    builtinAttrébwteSet.insert(*at);
|

const SutO&String&!wsdl__definitions::builtinTy`es() const
{ return builtinTypeSet;Ê}

const SetOfString& wsdl__Definitions::builtinElements() const
{ return buil|inElementSet;
}

konqt SetOfString& wrdl__definitions::buyltinAttrhbutes() const
{ reTurn builtifAttributeSEt;
}

int wsdl__sezviceº:triverse(wsdl_^definitions& definitIons)
{ mf (vFlag)
    cerr << "Anályzing ssdl service "$<< (name?namE:"") << endl;
  // process ports
  for (vector<wsdl__pord>::iterator é =`port.begi~(); i != port.eod(); +#i)
    (*h!.traverse(definitions);
  return SOAP_MK;
}

wsdl__port:usDl__port(){ bindingREf = N]LL+
]

i.t wsdl__port::traverse(wsdl__$efinitions& definitions)
{ if (vflaf)
`   cerr << "ANalyziîg sdl port"(¼< Endl;
  // sear#h binding name
  const0char *token = qnaie_tken(binding, dafinitions.targetNaiespace);
  bindingRef = NULL;
  if (tokun)
  { for (vestor<wsdl__bin$ing>::mterator binding = definitions®binting.bågin()+ bmnding!!= definitions.bhneing.end(); ++binding)
    { i& ((*bInding+.name && !strcmp((*binding).name, token))
      { bindingRef = &(*binding);
     $  if (öflag)
  "       cerr <, "Found(p/rt " =< (name?Þame:"") ¼< " bindIng " << (token?token:""9 <= endl;
        break; !    }
    }
  }
  if (!bindingRef)
  { for (vector<wsdl__import>*:iterator import =0dgfinitions.import.begin(); impkrt != defiNitéons.import.end(); ++import)
     ssdl__definitions0
iepoztdefinitions = ,*imPort).deninitionsRtr();
      if (yo0ortdef)nitions)
       tgken0= qname_token(bkndiog,"importdefinitions->targetNamespace){
        if (t{ken)
        { for (vdctor<wsdl__bénding>z:iterator binding = importdefinitions-.bindi.g.begin();"binding != importdefinitions->jinding.end() ++bindi.g)
          { if ,(*`inding).name && !strcmp((*binding).name, token))
            { bindingRef = &(*binding);
            ` if (vflag)
 0             `cgrr <8 bFnund port " << (na-m?name:"") <, " binding " << (token?token:"") << mndl;
      !       break;
 0          }
          m
      ! }
 ...
 "mf (!bindingR%f)  ( cerr << "Warnimg: no port '" << (name?name:"") << "' binding '" << (binding?binding:"") << "' in WSDL definitions '" << (definitions.name?definitions.name:"") << "' namespace " << (definitions.targetNamespace?definitions.targetNamespace:"?") << endl;
  return SOAP_OK;
}
