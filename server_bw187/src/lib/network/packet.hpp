...
public:
	...
	{
		return s_allocator_.allocate( size + MAX_SIZE );
	}
	...
	Packet * next() { return next_.getObject(); }
	const Packet * next() const { return next_.getObject(); }
	...
	bool stripFooter( TYPE & value )
	{
		...
		switch( sizeof( TYPE ) )
		{
			...
			default:
				CRITICAL_MSG( "Footers of size %d aren't supported",
					sizeof( TYPE ) );
		}
