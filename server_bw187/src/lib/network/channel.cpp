...
void Channel::staticInit()
{
	...
	MF_WATCH( "internalSendWindowSizeThreshold",
		s_sendWindowWarnThresholds_[ 0 ] );

	MF_WATCH( "indexedSendWindowSizeThreshold",
...
Channel::~Channel()
{
	...
	this->reset( Address::NONE, /*warnOnDiscard=*/ true, 
		/*shouldSendDelayed=*/ false );
...
void Channel::addOverflowPacket( UnackedPacket * pPacket )
{
	...
	uint numOverflowPackets = seqMask( largeOutSeqAt_ - smallOutSeqAt_ );
	...
	if (maxOverflowPackets != 0)
	{
		...
		{
			CRITICAL_MSG( "Internal %s channel overflow (%s): "
						  "largeOutSeqAt_=%d smallOutSeqAt_=%d, "
						  "oldestUnackedSeq_=%d windowSize_=%d, "
						  "numOverflowPackets=%u, maxOverflowPackets=%u\n",
						  isIrregular_ ? "irregular" : "regular",
						  this->c_str(), largeOutSeqAt_, smallOutSeqAt_,
						  oldestUnackedSeq_, windowSize_,
						  numOverflowPackets, maxOverflowPackets );
		}
		...
		if (numOverflowPackets > (maxOverflowPackets / 2))
		{
			if (!hasSeenOverflowWarning_)
			{
				WARNING_MSG( "Channel::addOverflowPacket: Overflow packet "
					"list size (%d) exceeding safety threshold (%d).\n",
					numOverflowPackets, (maxOverflowPackets / 2) );
		...
		const ReliableOrder * roBeg, const ReliableOrder * roEnd )
{
	...
	if (!overflowPackets_.empty() || unackedPackets_[ seq ] != NULL)
	{
		WARNING_MSG( "Channel::addResendTimer( %s ):"
						"Window size exceeded, buffering #%d\n",
		...
		if (!isIrregular_ &&
				(lastReliableResendTime_ + stampsPerSecond() < now))
		{
			this->resend( oldestUnackedSeq_ );
		}
	}
	else
	{
...
bool Channel::delResendTimer( SeqNum seq )
{
	...
	if (seqMask( seq ) != seq)
	{
		ERROR_MSG( "Channel::delResendTimer( %s ): "
			"Got out-of-range seq #%d (outseq: #%d)\n",
	...
	if (seqMask( (smallOutSeqAt_-1) - seq ) >= windowSize_)
	{
		if (this->isInternal())
		{
			WARNING_MSG( "Channel::delResendTimer( %s ): "
				"Called for seq #%d outside window #%d (maybe ok)\n",
	...
	if (oldestUnackedSeq_ != SEQ_NULL)
	{
		...
		while (seqLessThan( seq, smallOutSeqAt_ ) && unackedPackets_[ seq ])
		{
			...
			if (now - unacked.lastSentTime_ > thresh)
			{
				...
				{
					WARNING_MSG( "Channel::checkResendTimers( %s ): "
						"Resending unacked packet #%d due to inactivity "
...
std::pair< Packet*, bool > Channel::queueAckForPacket( Packet *p, SeqNum seq,
	const Address & srcAddr, bool shouldSendAck, bool & didDiscard )
{
	didDiscard = false;
	...
	if (seqMask( seq ) != seq)
	{
		ERROR_MSG( "Channel::queueAckForPacket( %s ): "
			"Got out-of-range incoming seq #%d (inSeqAt: #%d)\n",
	...
	if (addr_ != srcAddr)
	{
		if (shouldAutoSwitchToSrcAddr_)
		{
			...
			if (seqLessThan( p->channelVersion(), version_ ))
			{
				WARNING_MSG( "Channel::queueAckForPacket( %s ): "
					"Dropping packet from old addr %s (v%d < v%d)\n",
		...
		{
			ERROR_MSG( "Channel::queueAckForPacket( %s ): "
				"Got packet #%d from wrong source address: %s\n",
		...
		seqLessThan( version_, p->channelVersion() ))
	{
		...
		WARNING_MSG( "Channel::queueAckForPacket( %s ): "
			"Updating to v%d without changing address\n",
			this->c_str(), version_ );
	}

	// Is it above the window?
	if (seqLessThan( inSeqAt_, seq ) && 
			seqMask(seq - inSeqAt_) > windowSize_)
	{
		DEBUG_MSG( "Channel::queueAckForPacket( %s ): "
				"Sequence number #%u is way above window #%u!\n",
			this->c_str(), seq, inSeqAt_ );
	...
	// The calling code may not want us to ack - this is used to stop ACKing
	// packets that contain piggybacks that we have discarded that we want
	// resent later.
	if (shouldSendAck)
	{
		...
		if (pushUnsentAcksThreshold_ && acksOnPacket >= pushUnsentAcksThreshold_)
		{
			WARNING_MSG( "Channel::queueAckForPacket( %s ): "
					"Pushing %u unsent ACKs due to inactivity\n",
...
const char * Channel::c_str() const
{
	...
	if (this->isIndexed())
	{
		length += bw_snprintf( dodgyString + length,
			sizeof( dodgyString ) - length,	"/%ld", id_ );
	}
 ...
 */
int Channel::handleTimeout( int, void * arg )
{
		...
		bool shouldSendDelayed )
{
	...
	// Send it now if the nub has it registered for delayed sending. 
	if (shouldSendDelayed)
	{
		pNub_->sendIfDelayed( *this );
	}
...
int Channel::latestAckedPacketAge() const
{
	...
	return (int)windowSize_ + seqMask( largeOutSeqAt_ - smallOutSeqAt_ );
}
