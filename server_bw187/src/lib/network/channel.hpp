...
public:
	...
	std::pair< Packet*, bool > queueAckForPacket(
		Packet * p, SeqNum seq, const Address & srcAddr, bool shouldSendAck, 
		bool & didDiscard );
...
private:
	...
	public:
		...
		static uint32 s_numInstances() { return s_allocator_.totalInstances(); }
...
public:
				...
				minInactivityResendDelay, pFilter ) )
	{
		...
		MF_ASSERT( traits == Channel::EXTERNAL ||
			(minInactivityResendDelay == DEFAULT_INACTIVITY_RESEND_DELAY &&
				 pFilter == NULL) );
	}
