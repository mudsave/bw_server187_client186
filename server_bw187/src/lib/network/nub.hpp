...
public:
	...
	TimerID registerTimer( int microseconds, TimerExpiryHandler * handler,
			void* arg = NULL );
	int registerCallback( int microseconds, TimerExpiryHandler * handler,
			void * arg = NULL );
	void cancelTimer( TimerID id );
	int cancelTimers( TimerExpiryHandler * pHandler );
	uint64   timerDeliveryTime( int id ) const;
	uint64 & timerIntervalTime( int id );
	...
	void pBundleEventHandler( BundleEventHandler * handler );
...
private:
	...
	{
	public:
		ReplyHandlerElement(	int replyID_,
		...
		const int						replyID_;
		int								timerID_;
		ReplyMessageHandler * const		pHandler_;
		void * const					arg_;
		Channel * const					pChannel_;

		virtual int handleTimeout( int id, void * nubArg );
		void handleFailure( Nub & nub, bool isTimeout );
	};
...
public:
	...
	void shouldAutoCreateAnonymousChannels( bool b )
	{	shouldAutoCreateAnonymousChannels_ = b; }

	bool shouldAutoCreateAnonymousChannels() const
	{	return shouldAutoCreateAnonymousChannels_; }

	void sendIfDelayed( Channel & channel );

private:
	int onceOffPacketCleaningTimerID_;
...
private:
	...
	bool shouldAutoCreateAnonymousChannels_;
	bool isVerbose_;
};
