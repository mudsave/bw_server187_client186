...
void MGMPacket::read( MemoryIStream &is )
{
	...
	while (is.remainingLength())
	{
		...
		char *msgdata = (char*)is.retrieve( msglen );
		if (is.error())
		{
...
bool MGMPacket::write( MemoryOStream &os ) const
{
	...
	if (isOversized)
	{
		ERROR_MSG( "MGMPacket::write: Wrote MGMPacket %d bytes long/%d "
...
void WholeMachineMessage::write( BinaryOStream &os )
{
	...
	{
		const InterfaceStats &is = ifStats_[i];
...
{
	bw_snprintf( MachineGuardMessage::s_buf_, sizeof(MachineGuardMessage::s_buf_), "WholeMachineMessage" );
	return MachineGuardMessage::s_buf_;
}
...
{
	bw_snprintf( MachineGuardMessage::s_buf_, sizeof(MachineGuardMessage::s_buf_),
		"CreateMessage: %s/%s (uid:%d)",
		config_.c_str(), name_.c_str(), uid_ );
	return MachineGuardMessage::s_buf_;
}
...
const char *MachinedAnnounceMessage::c_str() const
{
	...
	{
		bw_snprintf( MachineGuardMessage::s_buf_, sizeof(MachineGuardMessage::s_buf_), "MachinedBirth: %lu", count_ );
	}
	else
	{
