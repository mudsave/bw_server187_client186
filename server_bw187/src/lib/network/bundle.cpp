...
void Bundle::finalise()
{
	...
	// now go through all the packets and turn on their 'reliable' flags
	//  if any were reliable (requires turning on the 'has seq #' flag too)
	// msgReliable_ is only set here if no msgs (only footers) on bundle,
	//  but the setter wants to indicate that it should still be reliable
	if (reliableOrders_.size() || msgReliable_ || numReliableMessages_ > 0)
	{
		for (Packet * p = firstPacket_.getObject(); p; p = p->next())
		{
			...
			p->enableFlags(
				Packet::FLAG_HAS_SEQUENCE_NUMBER | Packet::FLAG_IS_RELIABLE );
		}
	}
}
	...
	Packet *p )
{
	...
	// If the original packet specified ordering, this needs to be preserved too
	flags |= (p->flags() & Packet::FLAG_ON_CHANNEL);
...
UnpackedMessageHeader & Bundle::iterator::unpack( const InterfaceElement & ie )
{
	...
	else
	{
		...
		{
			ERROR_MSG( "Bundle::iterator::unpack( %s ): "
				"Not enough data on stream at %d for request ID and NRO "
				"(%d left, needed %d)\n",
