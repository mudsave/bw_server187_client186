...
{
	if ((socket_ != -1) && Endpoint::isHijacked())
	{
...
INLINE int Endpoint::getInterfaceFlags( char * name, int & flags )
{
	...
	if (ioctl( socket_, SIOCGIFFLAGS, &request ) != 0) return -1;
...
INLINE int Endpoint::getInterfaceAddress( const char * name, u_int32_t & address )
{
	...
	else
	{
		...
		strncpy( request.ifr_name, name, IFNAMSIZ );
		if (ioctl( socket_, SIOCGIFADDR, &request ) != 0) return -1;
	...
	u_int32_t & netmask )
{
	struct ifreq request;
	strncpy( request.ifr_name, name, IFNAMSIZ );
	if (ioctl( socket_, SIOCGIFNETMASK, &request ) != 0) return -1;
