...
namespace
{
	void HEX_DUMP( char *p, int len )
	{
		...
		{
			bw_snprintf( s, lineSize, "%02x ", (unsigned char)p[i] ); s += 3;
			lineSize -= 3;
			if (i > 0 && i % 20 == 19)
			{
				DEBUG_MSG( "%s\n", line );
	...
	void HEX_DUMP_PACKET( Packet *p )
	{
		HEX_DUMP( p->data(), p->totalSize() );
		DEBUG_MSG( "Packet length is %ld\n", p->totalSize() );
	}
} // anonymous namespace
#endif
...
Nub::Nub( uint16 listeningPort, const char * listeningInterface ) :
	...
	shouldAutoCreateAnonymousChannels_( !isExternal_ ),
	isVerbose_( true )
{
...
Nub::~Nub()
{
	...
	{
		INFO_MSG( "Nub::~Nub: Num pending reply handlers = %d\n",
	...
	// Delete any anonymous channels registered here.  We can't condemn them
	// because they won't have a Nub with which to resend.
		...
		!drainSocketInput_)	// line above only to keep queue small
	{
		...
		if (tqe->state != TimerQueueElement::STATE_CANCELLED)
		{
			...
			try
			{
				...
				tqe->handler->handleTimeout( (int)tqe, tqe->arg );
			}
			catch (...)
			{
...

	{
		...
		for (Packet * p = bundle.firstPacket_.get(); p; p = p->next())
		{
			...
			if (p->hasFlags( Packet::FLAG_HAS_PIGGYBACKS ))
			{
				...
				for (Bundle::Piggybacks::iterator it = bundle.piggybacks_.begin();
					 it != bundle.piggybacks_.end(); ++it)
				{
					Bundle::Piggyback &pb = **it;
					...
					*(Packet::Flags*)pdata = pb.flags_;
					pdata += sizeof( Packet::Flags );

					// Stream on the reliable messages
					for (ReliableVector::iterator rvit = pb.rvec_.begin();
					...
					*(SeqNum*)pdata = BW_HTONL( pb.seq_ );
					...
					{
						Field & subPiggies = pb.pPacket_->piggyFooters();
		...
		if (packet->seq() != Channel::SEQ_NULL)
		{
			if (packet->channelID() != CHANNEL_ID_NULL)
			{
				DEBUG_MSG( "Nub::rescheduleSend: "
					"dropped packet #%d to %s/%d due to artificial loss\n",
					packet->seq(), addr.c_str(), packet->channelID() );
			}
			else
			{
				DEBUG_MSG( "Nub::rescheduleSend: "
					"dropped packet #%d to %s due to artificial loss\n",
...
int Nub::RescheduledSender::handleTimeout( int, void * )
{
 ...
 */
void Nub::sendIfDelayed( Channel & channel )
{
	DelayedChannels::iterator iter = delayedChannels_.find( &channel );

	if (iter != delayedChannels_.end())
	{
		ChannelPtr pChannel = *iter;
		delayedChannels_.erase( iter );
...
Reason Nub::processPacket( const Address & addr, Packet * p )
{
	...
	Channel * pChannel = this->findChannel( addr,
			shouldAutoCreateAnonymousChannels_ && p->shouldCreateAnonymous() );
...
Reason Nub::processFilteredPacket( const Address & addr, Packet * p, 
		bool * pShouldAckPiggybackParent /*=NULL*/ )
{
	if (pPacketMonitor_)
	{
	...
	if (p->hasFlags( Packet::FLAG_HAS_CHECKSUM ))
	{
		...
		*(Packet::Checksum*)p->back() = 0;
		...
		// on.
		*(Packet::Checksum*)p->back() = p->checksum();
	...
	// If we have piggybacks, there's a chance we don't process them
	// immediately and discard them, and should wait for the resend of the
	// top-level packet containing those discarded piggybacks before we send an
	// ACK for it. We use shouldAck to see if recursive calls to
	// processFilteredPacket() on piggybacked packets have discarded any that
	// we want resent at a later time.
		...
		while (!done)
		{
			...
			try
			{
				this->processFilteredPacket( addr, piggyPack.get(), 
					pShouldAckPiggybackParent != NULL ? 
						pShouldAckPiggybackParent : &shouldSendAck );
			}
	...
	if (!pChannel && p->hasFlags( Packet::FLAG_ON_CHANNEL ))
	{
		...
		if (!pChannel)
		{
			// __kyl__ (2008-3-12) Disabled because clients don't support
			// dev asserts properly.
	...
	if (p->hasFlags( Packet::FLAG_IS_RELIABLE ))
	{
		...
		if (pChannel)
		{
			...
			// See whether we discarded the packet because it was above the
			// receive window.
			bool didDiscard = false;

			// Add this packet to the receive window.
			std::pair< Packet*, bool > result = pChannel->queueAckForPacket( 
				p, p->seq(), addr, shouldSendAck, didDiscard );

			if (didDiscard && pShouldAckPiggybackParent)
			{
				// If the channel discarded that packet because it is above the
				// recieve window, and it was a piggybacked packet, we
				// shouldn't ACK the containing packet.
				*pShouldAckPiggybackParent = false;
			}
			...
			if (result.first == NULL)
			{
				if (result.second)
				{
					// The packet was not corrupted, and either:
					// - is below the receive window, and has already been
					// processed
					// - is within the receive window, and has been buffered
					// - is above the receive window, and so is discarded
					//
					// In the last case, if the discarded packet A was
					// piggybacked onto a later packet B, we should not send an
					// ACK for B, so A will be resent along with B when we are
					// ready for it.
					//
					// Otherwise, we have sent an ACK for it, and we should
					// send on the channel if it is established.

					++numDuplicatePacketsReceived_;

					if (!didDiscard && shouldSendAck && 
							pChannel->isEstablished() && shouldSendChannel)
					{
						pChannel->send();
					}
					return REASON_SUCCESS;
				...
				{
					// The packet seems to be corrupted.
					RETURN_FOR_CORRUPTED_PACKET();
				}
			}
		}
...
#	define SOURCE_STR (pChannel ? pChannel->c_str() : addr.c_str())
	...
	if (p->hasFlags( Packet::FLAG_IS_FRAGMENT ))
	{
		...
		if (pFragments == NULL)
		{
			...
			if (pChannel && p->seq() != p->fragBegin())
			{
				ERROR_MSG( "Nub::processOrderedPacket( %s ): "
					"Bundle (#%d,#%d) is missing packets before #%d\n",
		...
		if (pFragments->lastFragment_ != p->fragEnd())
		{
			...
			if (!p->hasFlags( Packet::FLAG_IS_RELIABLE ))
			{
				...
				WARNING_MSG( "Nub::processPacket( %s ): "
					"Discarding unreliable fragment #%d (#%d,#%d) while "
					"waiting for reliable chain (#%d,#%d) to complete\n",
		...
		for (walk = pFragments->pChain_.get(); walk; walk = walk->next())
		{
			...
			{
				WARNING_MSG( "Nub::processPacket( %s ): "
					"Discarding duplicate fragment #%d\n",
					SOURCE_STR, p->seq() );
	...
	while (iter != end && !breakBundleLoop_)
	{
		...
		// __glenc__ TODO: Change MemoryIStream to take a const char *
		MemoryIStream mis = MemoryIStream( (char*)msgData, header.length );

		numMessagesReceived_++;
		ie.incMessagesReceived();
		ie.incBytesReceived( header.length );
		...
		if (mis.remainingLength() != 0)
		{
			WARNING_MSG( "Nub::processOrderedPacket( %s ): "
				"Handler for message index %d did not consume all data\n",
				SOURCE_STR, header.identifier );
		}
	}
...
void Nub::cancelReplyMessageHandler( ReplyMessageHandler * pHandler )
{
	...
	while (iter != replyHandlerMap_.end())
	{
		...
		{
			pElement->handleFailure( *this, /*isTimeout:*/false );
		}
	}
 ...
 *	in timestamps.
 */
uint64 Nub::timerDeliveryTime( int id ) const
{
	TimerQueueElement * tqe = (TimerQueueElement*)id;
 ...
 */
uint64 & Nub::timerIntervalTime( int id )
{
	TimerQueueElement * tqe = (TimerQueueElement*)id;
	...
	InputNotificationHandler * handler )
{
	...
	if (fd >= FD_SETSIZE)
	{
		ERROR_MSG( "Nub::registerFileDescriptor: "
			"Tried to register fd %d >= FD_SETSIZE (%d)\n",
			fd, FD_SETSIZE );
 ...
 */
void Nub::pBundleEventHandler( BundleEventHandler * pHandler )
{
 ...
 */
int Nub::handleTimeout( int id, void * arg )
{
...
void Nub::expireOnceOffResendTimer( OnceOffPacket & oop )
{
	...
	else
	{
		...
		{
			ERROR_MSG( "Nub::expOnceOffResendTimer( %s ): "
 ...
 * 	Constructor
 */
Nub::ReplyHandlerElement::ReplyHandlerElement(	int replyID,
 ...
 * 	the user.
 */
int Nub::ReplyHandlerElement::handleTimeout( int /*id*/, void * nubArg )
{
	this->handleFailure( *static_cast< Nub * >( nubArg ), true );
	return 0;
}
 ...
 */
void Nub::ReplyHandlerElement::handleFailure( Nub & nub, bool isTimeout )
{
	...
	NubException e( isTimeout ? REASON_TIMER_EXPIRED : REASON_CHANNEL_LOST );
