...
LoggerMessageForwarder::~LoggerMessageForwarder()
{
 	...
 	{
		// Temporary work-around for nub being an invalid reference on client.
		// Both nub_ and this are destroyed in static destruction time.
#ifdef MF_SERVER   
...
LoggerMessageForwarder* LoggerMessageForwarder::pInstance()
{
	return pInstance_;
}


void LoggerMessageForwarder::registerAppID( int id )
{
	appID_ = id;
...
void LoggerMessageForwarder::init()
{
	...
	TRACE_MSG( "Total # loggers on network: %d\n", loggers_.size() );
...
void LoggerMessageForwarder::addLogger( const Mercury::Address & addr )
{
	...
	INFO_MSG( "LoggerMessageForwarder::addLogger: Added %s. # loggers = %d\n",
...
void LoggerMessageForwarder::sendAppID( const Mercury::Address &addr )
{
	MF_ASSERT( appID_ != 0 );
	...
	int messagePriority, const char * format, va_list argPtr )
{
	...
	ForwardingStringHandler * pHandler = NULL;
	...
	{
		pHandler = new ForwardingStringHandler( format,
			this->isSuppressible( format ) );
...
int LoggerMessageForwarder::handleTimeout( TimerID id, void * arg )
{
		 ...
		 iter != recentlyUsedHandlers_.end(); ++iter)
	{
		...
		if (this->isSpamming( pHandler ))
		{
			this->parseAndSend( &spamHandler_, 0, MESSAGE_PRIORITY_WARNING,
...
LoggerMessageForwarder* LoggerMessageForwarder::pInstance_ = NULL;
