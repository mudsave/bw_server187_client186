...
PySound::PySound( SoundManager::Event *pEvent, PyTypePlus* pType ) :
	PyObjectPlus( pType ),
	pEvent_( pEvent ),
	pModel_( NULL ),
	pPosition_( NULL ),
...
float PySound::duration()
{
	...
	FMOD_RESULT result = pEvent_->getInfo( NULL, NULL, &info );
	...
	return info.lengthms / (float)1000.0;
}
