 ...
 *	Constructor.
 */
PositionGizmo::PositionGizmo( int disablerModifiers, MatrixProxyPtr matrixProxy, MatrixProxyPtr visualOffsetMatrix, float radius ) 
:	active_( false ),
	...
	radius_( radius )
{
	...
	//HACK : this is probably better done somewhere else, but there's not
	//enough time right now to :
	//a) investigate whether gizmos are better off encapsulated inside
	//   tool functors / views, etc.
	/*
	Matrix currTransform;
	pMatrix->getMatrix( currTransform );
	Options::setOptionFloat( "itemEditor/lastY", currTransform.applyToOrigin().y );
	*/
}
...
void PositionGizmo::click( const Vector3& origin, const Vector3& direction )
{
	...
	if (currentPart_ != NULL) 
	{
		...
		/*if ( pView )
		{
			...
			{
				BigBang::instance().tool()->addView( "chunkViz", pView );
			}
		}*/
	}
}
