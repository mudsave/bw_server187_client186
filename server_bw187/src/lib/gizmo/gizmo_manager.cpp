...
PY_MODULE_STATIC_METHOD( GizmoManager, gizmoUpdate, BigBang )
PY_MODULE_STATIC_METHOD( GizmoManager, gizmoClick, BigBang )
...
PyObject* GizmoManager::py_gizmoUpdate( PyObject* args )
{
	...
	{
		PyErr_SetString( PyExc_TypeError, "BigBang.gizmoUpdate expects a Vector3" );
		return NULL;
	}
	if (Script::setData( PyTuple_GetItem(args, 0 ), ray, "BigBang.gizmoUpdate argument" ) != 0 )
	{
		return NULL;
	}
...
PyObject* GizmoManager::py_gizmoClick( PyObject* args )
{
	...
	{
		PyErr_SetString( PyExc_TypeError, "BigBang.gizmoClick takes no arguments" );
		return NULL;
	}
	if (!GizmoManager::instance().click())
	{
		PyErr_SetString( PyExc_TypeError, "BigBang.gizmoClick no gizmo to click" );
		return NULL;
	}
	Py_Return;
}
