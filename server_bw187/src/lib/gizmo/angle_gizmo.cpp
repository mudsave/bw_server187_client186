...
void AngleGizmo::drawZBufferedStuff( bool force )
{
	...
	if (ind.size())
	{
		...
		Moo::VertexXYZND* verts = Moo::rc().mixedVertexProcessing()	?	Moo::DynamicSoftwareVertexBuffer< Moo::VertexXYZND >::instance().lock( coneMesh.verts().size() )
																	:	Moo::DynamicVertexBuffer< Moo::VertexXYZND >::instance().lock( coneMesh.verts().size() );
		if (verts)
		{
			uint32 vbLockIndex = 0;
			memcpy( verts, &coneMesh.verts().front(), sizeof( Moo::VertexXYZND ) * coneMesh.verts().size() );
			...
			rc.drawIndexedPrimitive( D3DPT_TRIANGLELIST, vbLockIndex, 0, coneMesh.verts().size(),
				ibLockIndex, coneMesh.indices().size() / 3 );
		}
	}
...
bool AngleGizmo::draw( bool force )
{
	...
	if (ind.size())
	{
		...
		if (verts)
		{
			...
			pDev->SetStreamSource( 0, dvb.pVertexBuffer(), 0, sizeof( Moo::VertexXYZND ) );
			dib.indexBuffer().set();
