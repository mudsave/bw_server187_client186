 ...
 *	Constructor.
 */
MatrixScaler::MatrixScaler( MatrixProxyPtr pMatrix, float scaleSpeedFactor, FloatProxyPtr scale, PyTypePlus * pType ) :
	ToolFunctor( pType ),
	pMatrix_( pMatrix ),
 ...
 *	Constructor.
 */
PropertyScaler::PropertyScaler( float scaleSpeedFactor, FloatProxyPtr scale, PyTypePlus * pType ) :
	ToolFunctor( pType ),
	scale_( scale ),
	scaleSpeedFactor_(scaleSpeedFactor)
{
	...
	if (tool.locator())
	{
		...
		for (; i != props_.end(); ++i)
		{
			...
			for( int i = 0; i < 3; ++i )
			{
				...
				if( scale + 1 <= scaleEpsilon )
					continue;

				if( fabs( scale ) > fabs( maxScale ) )
					maxScale = scale;
...
void MatrixRotator::update( float dTime, Tool& tool )
{
	...
	if (tool.locator())
	{
		...
		if (!almostEqual( grabOffset_, offset ))
		{
			Vector3 n;
