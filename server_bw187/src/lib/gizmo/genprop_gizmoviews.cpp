...
private:
	...
	static struct ViewEnroller
	{
		ViewEnroller()
		{
			LinkProperty::registerViewFactory(
				GizmoViewKind::kindID(), &create );
		}
	}	s_viewEnroller;
};
...
int LinkPropertyView::s_count_ = 0;
GizmoPtr LinkPropertyView::s_pGizmo_ = NULL;
...
private:
	...
	static struct ViewEnroller
	{
		ViewEnroller()
		{
			GenRotationProperty::registerViewFactory(
				GizmoViewKind::kindID(), &create );
		}
	}	s_viewEnroller;
};
	...
	virtual void EDCALL elect()
	{
		if (s_count_++ == 0)
		{
			...
			s_pUniformScaleGizmo_ = new RadiusGizmo( new UniformScaleFloatProxy(),
				...
				"Uniform Scale",
				0xffffffff,
// Raymond : for bug 4734, make the gizmo radius bigger so we have more margin to scale
				8.f,
//				1.5f,
				MODIFIER_ALT,
...
private:
	...
	static struct ViewEnroller
	{
		ViewEnroller()
		{
			GenScaleProperty::registerViewFactory(
				GizmoViewKind::kindID(), &create );
		}
	}	s_viewEnroller;
};
...
private:
	...
	static struct ViewEnroller
	{
		ViewEnroller()
		{
			GenRadiusProperty::registerViewFactory(
				GizmoViewKind::kindID(), &create );
		}
	}	s_viewEnroller;
};
...
private:
	...
	static struct ViewEnroller
	{
		ViewEnroller()
		{
			AngleProperty::registerViewFactory(
				GizmoViewKind::kindID(), &create );
		}
	}	s_viewEnroller;
};
