...
PY_MODULE_STATIC_METHOD( GeneralEditor, getCurrentEditors, BigBang )
PY_MODULE_STATIC_METHOD( GeneralEditor, setCurrentEditors, BigBang )
...
/**
 *	This static method gets the sequence of current editors
 *
 *	I considered doing this with a PySTLSequence, but concluded that
 *	there was no point - in fact this way is better in some respects
 */
PyObject * GeneralEditor::py_getCurrentEditors( PyObject * args )
{
	if (PyTuple_Size(args))
	{
		PyErr_SetString( PyExc_TypeError, "BigBang.getCurrentEditors "
...
/**
 *	This static method sets the sequence of current editors
 */
PyObject * GeneralEditor::py_setCurrentEditors( PyObject * args )
{
	...
	for (int i = 0; i < na; i++)
	{
		...
		if (pItem == NULL || !GeneralEditor::Check( pItem ))
		{
			...
			PyErr_SetString( PyExc_TypeError, "BigBang.setCurrentEditors "
	...
	// Record the current position when ever the selection changes
	if (!CurrentPositionProperties::properties().empty())
	{
		Options::setOptionFloat( "itemEditor/lastY", CurrentPositionProperties::centrePosition().y );
	}
