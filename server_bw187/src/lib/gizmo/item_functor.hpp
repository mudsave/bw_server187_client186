...
public:
	...
	/*
	void setUndoName( std::string name ) { undoName_ = name; }
	PY_AUTO_METHOD_DECLARE( RETVOID, setUndoName,
		ARG( std::string, END ) )
	*/
...
/**
 *	This class is a functor that make sth. snap on the surface under
 *	the current mouse cursor
 */
/*class MatrixSnapper : public ToolFunctor
{
	Py_Header( MatrixMover, ToolFunctor )
public:
	MatrixMover( MatrixProxyPtr pMatrix, PyTypePlus * pType = &s_type_ );
 ...
 *	else that can return a matrix.
 */
class MatrixScaler : public ToolFunctor, public Aligned
{
	Py_Header( MatrixScaler, ToolFunctor )
public:
	MatrixScaler( MatrixProxyPtr pMatrix, float scaleSpeedFactor = 1.f, FloatProxyPtr scale = NULL, PyTypePlus * pType = &s_type_ );
...
class PropertyScaler : public ToolFunctor, public Aligned
{
	Py_Header( PropertyScaler, ToolFunctor )
public:
	PropertyScaler( float scaleSpeedFactor = 1.f, FloatProxyPtr scale = NULL, PyTypePlus * pType = &s_type_ );
 ...
 *	else that can return a matrix.
 */
class MatrixRotator : public ToolFunctor, public Aligned
{
 ...
 */
class DynamicFloatDevice : public ToolFunctor, public Aligned
{
 ...
 */
class WheelRotator : public ToolFunctor, public Aligned
{
