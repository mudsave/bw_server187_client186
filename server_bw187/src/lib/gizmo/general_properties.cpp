...
PyObject * EDCALL ChoiceProperty::pyGet()
{
	...
	{
		if ((*it)->asInt() == v)
			return PyString_FromString( (*it)->sectionName().c_str() );
	}
	...
	if (pChoices_->countChildren() != 0)
	{
		return PyString_FromString(
			pChoices_->openChild(0)->sectionName().c_str() );
	}
...
int EDCALL ChoiceProperty::pySet( PyObject * value, bool transient /* = false */ )
{
	...
	{
		choiceStr += pChoices_->openChild( index )->sectionName();
