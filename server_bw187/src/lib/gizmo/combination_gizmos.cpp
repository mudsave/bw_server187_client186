...
//	rc.setRenderState( D3DRS_FILLMODE, D3DFILL_WIREFRAME );
	...
	if (ind.size())
	{
		...
		Moo::VertexXYZND* verts = Moo::rc().mixedVertexProcessing()	?	Moo::DynamicSoftwareVertexBuffer< Moo::VertexXYZND >::instance().lock( mesh_.verts().size() )
																	:	Moo::DynamicVertexBuffer< Moo::VertexXYZND >::instance().lock( mesh_.verts().size() );
		if (verts)
		{
