...
/*virtual*/ void LinkView::render(Tool const &tool)
{
    ...
    bool   canLink   = linkProxy_->canLinkAtPos(tool.locator());
	...
	if (linkDrawMesh_)
	{
		...
		pLC->ambientColour( canLink ? canLinkClr : cannotLinkClr );
	...
	if (canLink)
	{
		rc->setRenderState( D3DRS_TEXTUREFACTOR, canLinkClr );
	}
	else
	{
		rc->setRenderState( D3DRS_TEXTUREFACTOR, cannotLinkClr );
	}
    ...
    (
        ...
        canLink ? canLinkClr : cannotLinkClr
    );
