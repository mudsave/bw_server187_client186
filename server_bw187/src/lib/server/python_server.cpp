...
PY_BEGIN_METHODS( PythonServer )
	 ...
	 *
	 * 	@param	string	A Python tuple containing a single string argument
	 */
	PY_METHOD(write)
...
PY_BEGIN_ATTRIBUTES( PythonServer )
	 ...
	 *  that instances of PythonServer can be used as streams.
...
PyObject* PythonServer::py_write(PyObject* args)
{
	...
	// First print it as normal.
	// dprintf("%s", msg);
	PyObject * pResult =
		PyObject_CallMethod( prevStdout_, "write", "O", args );
	if (pResult)
	{
		Py_DECREF( pResult );
	}
