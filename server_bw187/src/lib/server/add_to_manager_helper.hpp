 ...
 *  sending and resending the add message until it succeeds or fails.
 *
...
public:
	...
	/// The maximum number of times we'll let the BaseAppMgr tell us it's not
	/// ready to add BaseApps yet.
...
private:
	...
	int handleTimeout( int timerID, void * arg );

	/// The nub that's being used for comms.
	Mercury::Nub & nub_;

	/// The number of times we've sent the add message to the manager.
	int retries_;
};
