...
static GoboComponent::TextureSetter s_diffuseMapSetter_;
static GoboComponent::TextureSetter s_blurMapSetter_;
...
void GoboComponent::setConstants()
{
	...
	s_diffuseMapSetter_.map( texture_ );
	if (Bloom::isSupported() && blurTexture_)
		s_blurMapSetter_.map( blurTexture_ );
	else
		s_blurMapSetter_.map( texture_ );
	*pDiffuseMap_ = &s_diffuseMapSetter_;
	*pBlurMap_ = &s_blurMapSetter_;
 ...
 */
void GoboComponent::buildMaterial( void )
{
	if ( !material_ )
	{
	...
	MF_ASSERT( material_->pEffect() );
	MF_ASSERT( material_->pEffect()->pEffect() );
