...
// INLINE void SimpleGUIComponent::inlineFunction()
// {
// }
...
{
	if ( state == widthInClipCoords_ )
		return;
	
	widthInClipCoords_ = state;
	
	if ( state )
	{
		//convert current pixel width to clip  width
		width_ = width_ / Moo::rc().halfScreenWidth();
	}
	else
	{
		//convert current clip height to pixel height
		width_ = width_ * Moo::rc().halfScreenWidth();
	}
}
...
{
	if ( state == heightInClipCoords_ )
		return;
	
	heightInClipCoords_ = state;
	
	if ( state )
	{
		//convert current pixel width to clip  width
		height_ = height_ / Moo::rc().halfScreenHeight();
	}
	else
	{
		//convert current clip height to pixel height
