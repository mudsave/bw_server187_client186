	...
	alwaysDisplayChildren_( false )
{
	for (int i = 0; i < 4; i++)
	{
		...
		corners_[i]->widthInClipCoords( 0 );
		corners_[i]->heightInClipCoords( 0 );
	}
...
void BoundingBoxGUIComponent::update( float dTime )
{
	dTime_ = dTime;
 ...
 */
void BoundingBoxGUIComponent::preDraw()
{
	...
	else
	{
		...
		SimpleGUI::instance().pixelRangesToClip(
			SimpleGUIComponent::width(), SimpleGUIComponent::height(), &w, &h );
	...
	corners_[0]->update( dTime_ );
	corners_[1]->update( dTime_ );
	corners_[2]->update( dTime_ );
	corners_[3]->update( dTime_ );
