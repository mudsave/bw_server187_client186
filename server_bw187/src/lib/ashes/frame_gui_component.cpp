...
:SimpleGUIComponent( backgroundTextureName, pType )
{
	...
	for ( int i = 0; i < 4; i++ )
	{
		...
		corners_[i]->widthInClipCoords( true );
		corners_[i]->heightInClipCoords( true );
		edges_[i] = new SimpleGUIComponent( edgeTextureName );
		edges_[i]->widthInClipCoords( true );
		edges_[i]->heightInClipCoords( true );
	}
 ...
 * tile size in pixels.
 */
void FrameGUIComponent::update( float dTime )
{
	...
	//if ( ts.pTexture() )
	{
		...
		for ( int i = 0; i < 4; i++ )
		{
			...
			corners_[i]->update( dTime );
			edges_[i]->update( dTime );
		}
