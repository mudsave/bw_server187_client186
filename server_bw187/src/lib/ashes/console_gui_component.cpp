...
PY_RW_ACCESSOR_ATTRIBUTE_DECLARE( int, pConsole_->le().cursorPosition, editCol )
 ...
 */
void ConsoleGUIComponent::update( float dTime )
{
	dTime_ = dTime;
}
...
void ConsoleGUIComponent::save( DataSectionPtr pSect, SaveBindings & bindings )
{
	...
	for (int i = 0; i < MAX_CONSOLE_HEIGHT; i++)
	{
		...
		pSect->newSection( "line" )->setString( tline );
	}
}
