...
public:
	...
	SmartPointer<SimpleGUIComponent> child( const std::string& name );
...
protected:
	void				layout( float x, float y, float width, float height, float& newX, float& newY );
	void				tile();

	void				cleanMesh();
	void				cleanMaterial();

	void				updateChildren( float dTime );

	void				drawSelf( bool overlay );
	...
	static void SimpleGUIComponent::setupTechniqueTable( Moo::EffectMaterial* material );
	...
	///the position, in clip coords, of the component.
	///the z coordinate is ignored
	Vector3				position_;	//in clip coords ( -1 .. +1 )
	///the width, in pixels, of the component
	float				width_;		//in pixels
	bool				widthInClipCoords_;	//or clip coords, if this set
	///the height, in pixels, of the component
	float				height_;	//in pixels
	bool				heightInClipCoords_;	//or clip coords, if this set
	///tiled or not?
	bool				tiled_;
