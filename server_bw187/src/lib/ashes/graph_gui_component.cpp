...
PY_BEGIN_ATTRIBUTES( GraphGUIComponent )
	 ...
	 *	The Vector4Provider is represented as a continuous line graph 
	 *	of the x and y components.  As a Provider, the display is updated 
	 *	whenever the contents of the Provider changes.
	 */
	PY_ATTRIBUTE( input )
...
:SimpleGUIComponent( "", pType ),
 ...
 frequency_( 0.033f ),
 head_( 0 ),
 mesh_( D3DPT_LINESTRIP ),
 accumTime_( 0.f )
{	
	materialFX( FX_ADD );
	yValues_ = new float[nPoints_];
	memset( yValues_, 0, nPoints_ * sizeof(float) );
	mesh_.resize( nPoints_ );
}
...
void GraphGUIComponent::draw( bool overlay )
{
	...
	if ( input_ )
	{
		...
		{
			v.uv_.set( 1.f-((float)i / (float)nPoints_), 1.f - (yValues_[idx] - minY_) / yRange );
			v.pos_.set( x, (yValues_[idx] - minY_) * yScale + yMin, position_.z );
			mesh_[i] = v;
