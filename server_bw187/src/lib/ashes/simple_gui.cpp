...
public:
	...
	{
		Vector2 pos;
		if ( MouseCursor::instance().isActive() )
			pos = MouseCursor::instance().position();
		else
...
SimpleGUI::~SimpleGUI()
{
	...
	{
		ERROR_MSG( "SimpleGUI::~SimpleGUI: "
			"SimpleGUI::fini should have been called\n" );
	}
}


void SimpleGUI::init( DataSectionPtr config )
{
	SimpleGUIComponent::init(config);
	inited_ = true;
}


/**
 *	This method cleans up the SimpleGUI singleton instance. It should be called
 *	before Script::fini.
 */
void SimpleGUI::fini()
{
...
{
	dTime_ = dTime;
	MouseCursor::instance().tick( dTime );

	static bool lastMouseActive = false;

	if ( MouseCursor::instance().isActive() != lastMouseActive )
	{
		...
		lastMouseActive = MouseCursor::instance().isActive();
	}
}
...
static SmartPointer<AshesProjSetter> s_pProjSetter = new AshesProjSetter;
...
static SmartPointer<TFactorSetter> s_pTFactorSetter = new TFactorSetter;


void SimpleGUI::setConstants( DWORD colour )
{
	...
	s_pTFactorSetter->colour( colour );
...
/**
 *	This method returns the current draw order count and increments it.
 *
 *	@return		current draw order count, before incrementing.
 */
uint32 SimpleGUI::stepDrawOrderCount()
{
	return drawOrderCount_++;
}
...
{
	//Update
	Moo::rc().device()->GetViewport( &originalView_ );

	Vector2 newResolution( Moo::rc().screenWidth(), Moo::rc().screenHeight() );
	...
	Moo::rc().device()->SetViewport( &originalView_ );
	MF_ASSERT( clipStack_.size() == 1 );
...
{
	MF_ASSERT( pixelToClipX_ != 0.f );
	MF_ASSERT( pixelToClipY_ != 0.f );
...
{
	bool handled = false;
	Vector2 mousePos = MouseCursor::instance().position();
	SimpleGUIKeyEvent keyEvent( event, mousePos );

	if (MouseCursor::instance().isActive() && isMouseKeyEvent( event ))
	{
...
{
	if ( !inited_ || !MouseCursor::instance().isActive() )
		return;
...
void SimpleGUI::checkCursorChanged()
{
	...
	if ( MouseCursor::instance().isActive() != s_lastMouseActive )
	{
		// make sure the EnterLeaveHandler sends enter/leave events when
		// the mouse cursor is hidden/shown.
		...
		s_lastMouseActive = MouseCursor::instance().isActive();
	}
}
