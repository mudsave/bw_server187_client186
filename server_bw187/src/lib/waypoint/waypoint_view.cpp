	...
	bool removeAllOld )
{
	...
	if (!removeAllOld && pChunkBin)
	{
		...
		{
			BinaryPtr combo =
				new BinaryBlock( NULL, pNewNavmesh->len() + keepSize );
	...
	if (!pChunkBin)
	{
		DataSectionPtr pTemp = new BinSection( "", new BinaryBlock( NULL, 0 ) );
	...
	float defaultGirth )
{
	...
	BinaryPtr dataBlock = new BinaryBlock( NULL, dataSize );
