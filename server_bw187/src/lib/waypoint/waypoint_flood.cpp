...
bool WaypointFlood::checkBounds( Vector3 src, Vector3 dst )
{
	...
	for (int i = 0; i < 16; ++i)
	{
		...
		// raise y a bit to avoid collision error
		isrc.y += pPhysics_->getScrambleHeight();
		idst.y += pPhysics_->getScrambleHeight();

		// make sure isrc and idst in current chunk.
		// not using pChunk->contains because it doesn't take into account interior chunks.
		if (pChunk_->owns( isrc ) &&
			pChunk_->owns( idst ))
		{
			return true;
	...
	while ( !floodStack.empty() )
	{
		...
		for ( angle = 0; angle < 8; ++angle )
		{
			...
			*/
			...
			log.println( 1, "adjGrids_[dst_grid][dst_index]: %u  src_grid: %d", adjGrids_[dst_grid][dst_index].all, src_grid );
